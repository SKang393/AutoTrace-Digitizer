...
| Marker center detection | Line-aware revision v1 P2 preregistered | Project-owned Apache-2.0 source and new procedural families only; no Chandler, private article, downloaded weights, or prior-candidate bytes | P1 produced no checkpoint. P2 has no output yet. | P1 failed ONNX export preflight before optimizer creation because adaptive 8-to-3 pooling is unsupported. Report SHA-256 `9e3bee532c852ba13c3bdde9390ab9dcbf1a1fcf091072f802dddefe20b6d56c`; zero public evaluations. P2 changes only that pool to a fixed 4-by-4 stride-two average pool and remains selection-only. | Experimental, training authorized | P1 is consumed and cannot rerun. P2 must pass every selection scene and CPU ONNX parity before the single truth-hidden public evaluation can be authorized. No manifest, store, package, or approval exists. |
