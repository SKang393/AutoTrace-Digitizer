...
"""Fresh procedural graph scenes for OCR production-composition V2."""
...
ANNOTATION_WORDS = ("Probe", "Review", "Change", "Transfer", "Prompt", "Criterion", "O o l I")
...
def _rng(split: str, index: int) -> np.random.Generator:
    ...
    material = f"production-composition-v2:{registration.seed_offset}:{split}:{index}".encode()
...
def _render_scene(split: str, index: int) -> CompositionScene:
    ...
    _draw_structures(draw, rng, index + 211, ink)

    y_display, y_truth = NUMERIC_LABELS[(index * 7 + 3) % len(NUMERIC_LABELS)]
    x_display, x_truth = NUMERIC_LABELS[(index * 11 + 5) % len(NUMERIC_LABELS)]
    phase = PHASE_WORDS[(index * 3 + 2) % len(PHASE_WORDS)]
    annotation = ANNOTATION_WORDS[(index * 5 + 1) % len(ANNOTATION_WORDS)]
    legend = LEGEND_WORDS[(index * 4 + 3) % len(LEGEND_WORDS)]
    y_slot = 75 + (index * 31) % 99
    x_slot = 178 + (index * 47) % 215
    phase_slot = 142 + (index * 43) % 180
    annotation_x = 355 + (index * 37) % 98
    annotation_y = 101 + (index * 23) % 50
    if split == "sealed_public":
        y_slot = 83 + (index * 37) % 87
        x_slot = 190 + (index * 59) % 198
        phase_slot = 153 + (index * 61) % 160
        annotation_x = 365 + (index * 41) % 86
        annotation_y = 106 + (index * 29) % 42
    draw.rectangle((326, annotation_y - 5, 638, annotation_y + 34), fill=background)
    ...
    labels = (
        ...
        (legend, legend, "legend_text", "word", (582, 176 + (index * 7) % 9)),
    )
    ...
    if split == "validation":
        ...
        column = 61 + (index * 43) % 514
        pixels[:, max(0, column - 1) : min(SCENE_WIDTH, column + 2)] = np.maximum(
            pixels[:, max(0, column - 1) : min(SCENE_WIDTH, column + 2)], 174
        )
    else:
        ...
        foreground = np.argwhere(pixels < 178)
        if len(foreground):
            for selected in rng.choice(len(foreground), size=min(5, len(foreground)), replace=False):
                y, x = foreground[int(selected)]
                pixels[int(y), int(x)] = int(rng.integers(236, 251))
    raster = np.asarray(pixels, dtype=np.uint8).copy()
    ...
    return CompositionScene(
        f"ocr-production-composition-v2-{split}-{index:05d}", split,
...
def save_sealed_archive(scenes: tuple[CompositionScene, ...], path: Path) -> dict[str, object]:
    if path.exists():
        raise RuntimeError(f"OCR composition V2 archive exists: {path}")
    ...
    manifest = {
        "schema": "graphreader.ocr-production-composition-fixtures.v2", "revision": REVISION,
    ...
    return {
        "schema": "graphreader.ocr-production-composition-private-manifest.v2", "revision": REVISION,
        "split": scenes[0].split, "scene_count": len(scenes),
        "truth_region_count": sum(len(scene.truths) for scene in scenes),
