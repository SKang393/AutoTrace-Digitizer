...
"""Single-use exact-payload validation gate for composition V2."""
...
def _cpu_session(path: Path, input_name: str, output_name: str) -> ort.InferenceSession:
    ...
    if session.get_providers() != ["CPUExecutionProvider"]:
        raise RuntimeError("OCR composition V2 requires CPUExecutionProvider only")
    if [item.name for item in session.get_inputs()] != [input_name] or [item.name for item in session.get_outputs()] != [output_name]:
        raise RuntimeError(f"OCR composition V2 ONNX I/O changed: {path}")
    return session
...
def evaluate_validation(
    *, detector_path: Path, official_recognizer_path: Path, numeric_recognizer_path: Path,
    ...
    if output_path.exists():
        raise RuntimeError(f"OCR composition V2 validation output exists: {output_path}")
    ...
    if sha256_file(seal_path) != config["validation_seal_sha256"]:
        raise RuntimeError("OCR composition V2 validation seal changed")
    ...
    if sha256_file(archive_path) != seal["fixture_archive_sha256"] or sha256_file(manifest_path) != seal["private_manifest_sha256"]:
        raise RuntimeError("OCR composition V2 validation fixture bytes changed")
    ...
    if not hashes_match:
        raise RuntimeError("OCR composition V2 exact payload or spacing source changed")
    ...
    if any(summary[key] != seal[key] for key in summary) or split_fingerprint(scenes) != seal["split_fingerprint"]:
        raise RuntimeError("OCR composition V2 validation fixtures violate frozen contract")
    metrics = evaluate_scenes(scenes, detector, official, numeric, read_character_alphabet(inference_yaml_path))
    passed = _passes(metrics) and detector.calls == len(scenes) and official.calls == metrics["truth_region_count"] and numeric.calls > 0
    report: dict[str, object] = {
        "schema": "graphreader.ocr-production-composition-validation-gate.v2", "task": TASK,
        ...
        "private_manifest_sha256": seal["private_manifest_sha256"], "validation_seal_sha256": sha256_file(seal_path),
        "metrics": metrics,
