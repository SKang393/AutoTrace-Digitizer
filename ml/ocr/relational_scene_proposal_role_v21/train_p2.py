...
"""One-shot CPU continuation and visible-selection runner for OCR V21 P2."""
...
CONFIG_PATH = CANDIDATE_ROOT / "P2_CONFIG.json"
AUTHORIZATION_PATH = CANDIDATE_ROOT / "P2_TRAINING_AUTHORIZATION.json"
SEAL_PATH = CANDIDATE_ROOT / "SPLIT_SEAL.json"
P1_RESULT_PATH = CANDIDATE_ROOT / "P1_SELECTION_RESULT.json"
RESULT_PATH = CANDIDATE_ROOT / "P2_SELECTION_RESULT.json"
ATTEMPT_PATH = Path("artifacts/production-validation/ocr-v21-p2-attempt.json")
...
P1_CHECKPOINT_PATH = Path("artifacts/production-validation/ocr-v21-p1-checkpoint.pt")
P1_REPORT_PATH = Path("artifacts/production-validation/ocr-v21-p1-selection-report.json")
RUNNER_SOURCE_PATHS = (
    CONFIG_PATH,
...
def _proposal_class_weights(labels: torch.Tensor, positive_multiplier: float) -> torch.Tensor:
    positive = int((labels == 1).sum())
    negative = int((labels == 0).sum())
    if positive == 0 or negative == 0:
        raise RuntimeError("OCR V21 scene lacks a balanced proposal target")
    if positive_multiplier <= 1.0:
        raise RuntimeError("OCR V21 P2 positive proposal multiplier must exceed the P1 baseline")
    weights = torch.tensor((1.0 / negative, positive_multiplier / positive), dtype=torch.float32)
    weights *= 2.0 / weights.sum()
    return weights


def _asymmetric_proposal_loss(
    logits: torch.Tensor,
    labels: torch.Tensor,
    ...
    return nn.functional.cross_entropy(
        logits,
        labels,
        weight=_proposal_class_weights(labels, positive_multiplier),
    )
...
def preflight() -> dict[str, Any]:
    ...
    expected_config = {
        "schema": "graphreader.ocr-relational-scene-proposal-role-continuation-candidate.v1",
        "revision": "graph-text-relational-scene-proposal-role-v21",
        "candidate_id": "P2",
        "candidate_type": "bounded-checkpoint-continuation",
        "continuation_epochs": 1,
        "expected_candidate_optimizer_steps": 384,
        "expected_total_optimizer_steps": 1920,
        "positive_proposal_loss_multiplier": 2.0,
    ...
    for key, value in expected_config.items():
        if config.get(key) != value:
            raise RuntimeError(f"OCR V21 P2 candidate config field mismatch: {key}")
    if config.get("thresholds") != list(THRESHOLDS):
        raise RuntimeError("OCR V21 P2 candidate thresholds changed")
    if int(config["expected_candidate_optimizer_steps"]) > 1536:
        raise RuntimeError("OCR V21 P2 optimizer budget exceeds the preregistered per-candidate maximum")

    authorization_path = REPO_ROOT / AUTHORIZATION_PATH
    if not authorization_path.is_file():
        raise RuntimeError("OCR V21 P2 training is not authorized")
    ...
    for key, value in expected_authorization.items():
        if authorization.get(key) != value:
            raise RuntimeError(f"OCR V21 P2 authorization field mismatch: {key}")

    seal_path = REPO_ROOT / SEAL_PATH
    if sha256_file(seal_path) != config["split_seal_sha256"]:
        raise RuntimeError("OCR V21 split seal changed before P2")
    if authorization.get("split_seal_sha256") != config["split_seal_sha256"]:
        raise RuntimeError("OCR V21 P2 authorization does not bind the split seal")
    config_sha256 = sha256_file(REPO_ROOT / CONFIG_PATH)
    if authorization.get("candidate_config_sha256") != config_sha256:
        raise RuntimeError("OCR V21 P2 authorization does not bind the candidate config")
    hashes = source_hashes()
    if authorization.get("runner_source_sha256") != hashes:
        raise RuntimeError("OCR V21 P2 authorized runner sources changed")
    bundle = source_bundle_sha256(hashes)
    if authorization.get("runner_source_bundle_sha256") != bundle:
        raise RuntimeError("OCR V21 P2 authorized runner source bundle changed")
    ...
    for key, value in expected_seal.items():
        if seal.get(key) != value:
            raise RuntimeError(f"OCR V21 split seal state mismatch before P2: {key}")
    ...
    for relative_path, expected_hash in sorted(sealed_sources.items()):
        ...
        if not source_path.is_file() or sha256_file(source_path) != expected_hash:
            raise RuntimeError(f"OCR V21 sealed source changed before P2: {relative_path}")
    ...
    for key, path in (("train", TRAIN_ARCHIVE_PATH), ("selection", SELECTION_ARCHIVE_PATH)):
        ...
        if sha256_file(REPO_ROOT / path) != expected_hash or config[f"{key}_archive_sha256"] != expected_hash:
            raise RuntimeError(f"OCR V21 {key} archive changed before P2")
        if authorization.get(f"{key}_archive_sha256") != expected_hash:
            raise RuntimeError(f"OCR V21 P2 authorization does not bind the {key} archive")

    prerequisite_paths = (
        (P1_CHECKPOINT_PATH, "p1_checkpoint_sha256"),
        (P1_REPORT_PATH, "p1_report_sha256"),
        (P1_RESULT_PATH, "p1_selection_result_sha256"),
    )
    for path, key in prerequisite_paths:
        ...
        if sha256_file(REPO_ROOT / path) != expected_hash or authorization.get(key) != expected_hash:
            raise RuntimeError(f"OCR V21 P2 prerequisite changed: {path.as_posix()}")
    p1_result = _read_json(REPO_ROOT / P1_RESULT_PATH)
    if (
        p1_result.get("p1_consumed") is not True
        or p1_result.get("selection_gate_passed") is not False
        or p1_result.get("public_archive_opened") is not False
        or p1_result.get("public_evaluation_count") != 0
    ):
        raise RuntimeError("OCR V21 P2 does not have the exact failed-closed P1 prerequisite")
    ...
    if existing:
        raise RuntimeError("OCR V21 P2 is already executed or has output: " + ", ".join(existing))
    ...
    if not isinstance(authorized_source_commit, str) or not _is_commit_ancestor(
        ...
        raise RuntimeError("OCR V21 P2 authorization does not bind an ancestor source commit")
    return {
...
def execute() -> dict[str, Any]:
    ...
    torch.manual_seed(int(config["order_seed"]))
    np.random.seed(int(config["order_seed"]) % (2**32))
    model = RelationalSceneProposalRoleNet(seed=int(config["model_seed"]))
    inherited = torch.load(REPO_ROOT / P1_CHECKPOINT_PATH, map_location="cpu", weights_only=True)
    if (
        inherited.get("candidate_id") != "P1"
        or inherited.get("optimizer_steps") != 1536
        or inherited.get("config_sha256") != "e3fdbb0208a49b890ae4eebda0bf3db9b52417c31ecdbef5d9521fd327be5fca"
        or inherited.get("authorization_sha256") != "ba689f8ec13d62995413a9dcdf09ef509b50c3d3ecf92d11eb814da70e350134"
    ):
        raise RuntimeError("OCR V21 P2 inherited checkpoint metadata is invalid")
    ...
    if any(
        ...
        raise RuntimeError("OCR V21 P2 inherited optimizer hyperparameters changed")

    train_scenes = load_archive(REPO_ROOT / TRAIN_ARCHIVE_PATH)
    selection_scenes = load_archive(REPO_ROOT / SELECTION_ARCHIVE_PATH)
    order_generator = torch.Generator().manual_seed(int(config["order_seed"]))
    ...
    _write_canonical(REPO_ROOT / ATTEMPT_PATH, attempt)
    model.train()
    for epoch in range(int(config["continuation_epochs"])):
        total_loss = 0.0
        for scene_index in torch.randperm(len(train_scenes), generator=order_generator).tolist():
            ...
            proposal_loss = _asymmetric_proposal_loss(
                output[:, :2],
                proposal_target,
                float(config["positive_proposal_loss_multiplier"]),
            )
            ...
            loss.backward()
            optimizer.step()
            candidate_optimizer_steps += 1
            total_loss += float(loss.detach())
        epoch_losses.append(total_loss / len(train_scenes))
        print(
            f"OCR V21 P2 epoch {epoch + 1}/{config['continuation_epochs']} complete; "
            f"candidate_optimizer_steps={candidate_optimizer_steps}",
            flush=True,
        )
    if candidate_optimizer_steps != int(config["expected_candidate_optimizer_steps"]):
        raise RuntimeError(f"OCR V21 P2 candidate optimizer step mismatch: {candidate_optimizer_steps}")
    total_optimizer_steps = int(inherited["optimizer_steps"]) + candidate_optimizer_steps
    if total_optimizer_steps != int(config["expected_total_optimizer_steps"]):
        raise RuntimeError(f"OCR V21 P2 total optimizer step mismatch: {total_optimizer_steps}")
    ...
    torch.save({
        ...
        "candidate_id": "P2",
        "inherited_checkpoint_sha256": config["p1_checkpoint_sha256"],
    ...
    report = {
        ...
        "inherited_checkpoint_sha256": config["p1_checkpoint_sha256"],
        "positive_proposal_loss_multiplier": config["positive_proposal_loss_multiplier"],
...
def main() -> int:
    ...
    print(json.dumps({
        ...
        "candidate_optimizer_steps": report["candidate_optimizer_steps"],
