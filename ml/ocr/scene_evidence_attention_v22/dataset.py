...
"""Fresh stored procedural scenes for OCR V22."""
...
def _registration_scope(seed_shift: int = 0) -> Iterator[None]:
    with _RENDER_LOCK:
        original = v21.split_registration

        def v22_registration(split: str):
            base = original(split)
            registered = split_registration(split)
            return replace(
                base,
                scene_count=registered.scene_count,
                seed_offset=registered.seed_offset + seed_shift,
                renderer_families=registered.renderer_families,
                degradation_families=registered.degradation_families,
                font_paths=registered.font_paths,
            )

        v21.split_registration = v22_registration
        try:
            yield
        finally:
            v21.split_registration = original


def _apply_degradation(raster: np.ndarray, split: Split, index: int) -> np.ndarray:
    """Apply new split-specific photometric families without moving truth."""
    values = raster.astype(np.float32)
    source_ink = raster < 128
    height, width = raster.shape
    if split == "train":
        row = np.arange(height, dtype=np.float32)[:, None]
        values += np.sin((row + float(index % 17)) * 0.17) * float(1 + index % 3)
        values *= 0.91 + 0.025 * float(index % 7)
        step = float(2 + index % 4)
        ...
        shelf = 2.0 + float(index % 5)
        values[:, width // 3:2 * width // 3] += shelf
    else:
        column = np.linspace(-4.0, 4.0, width, dtype=np.float32)[None, :]
        wave = np.sin(np.linspace(0.0, np.pi * 3.0, height, dtype=np.float32))[:, None]
        values += column + wave * float(1 + index % 4)
        band = 23 + (index * 29) % max(24, height - 24)
        values[band:band + 1] -= 3.0
...
def render_scene(split: Split, index: int) -> SceneSample:
    ...
    if index < 0 or index >= registered.scene_count:
        raise IndexError(f"OCR V22 scene index outside frozen {split} split: {index}")
    for attempt in range(48):
        source_index = (index + attempt * 53) % registered.scene_count
        with _registration_scope(seed_shift=attempt * 20_000_000):
            source = v21.render_scene(split, source_index)
        candidate = SceneSample(
            f"scene-evidence-attention-v22-{split}-{index:05d}",
            split,
        ...
        try:
            ...
            continue
        return candidate
    raise RuntimeError(f"OCR V22 could not render one production proposal per truth: {split}/{index}")
...
def save_archive(scenes: tuple[SceneSample, ...], path: Path) -> dict[str, object]:
    if path.exists():
        raise FileExistsError(f"OCR V22 fixture identity already exists: {path}")
    if not scenes or len({scene.split for scene in scenes}) != 1:
        raise RuntimeError("OCR V22 archive requires one nonempty split")
    ...
    manifest: dict[str, object] = {
        "schema": "graphreader.ocr-scene-evidence-attention-fixtures.v1",
        "revision": REVISION,
        "split": split,
...
def load_archive(path: Path) -> tuple[SceneSample, ...]:
    with ZipFile(path, "r") as archive:
        ...
        if (
            manifest.get("schema") != "graphreader.ocr-scene-evidence-attention-fixtures.v1"
            ...
            raise RuntimeError("OCR V22 stored fixture contract changed")
        scenes: list[SceneSample] = []
        for case in manifest["cases"]:
            payload = archive.read(case["source_path"])
            if sha256(payload).hexdigest() != case["source_sha256"]:
                raise RuntimeError(f"OCR V22 fixture hash mismatch: {case['scene_id']}")
    ...
    if len(result) != registered.scene_count:
        raise RuntimeError("OCR V22 stored fixture scene count changed")
