...
"""Single-use CPU training and visible selection for OCR V22 P1."""
...
def _cpu_session(path: Path) -> ort.InferenceSession:
    ...
    if session.get_providers() != ["CPUExecutionProvider"]:
        raise RuntimeError("OCR V22 requires CPUExecutionProvider only")
    return session
...
def preflight() -> dict[str, Any]:
    ...
    for key, value in expected.items():
        if config.get(key) != value:
            raise RuntimeError(f"OCR V22 P1 config field mismatch: {key}")
    if config.get("selection_thresholds") != list(THRESHOLDS):
        raise RuntimeError("OCR V22 P1 thresholds changed")
    if int(config["expected_optimizer_steps"]) > 1280:
        raise RuntimeError("OCR V22 P1 optimizer budget exceeds the preregistration")
    if config.get("expected_runner_source_bundle_sha256") != source_bundle_sha256(
        REPO_ROOT, RUNNER_SOURCE_PATHS,
    ):
        raise RuntimeError("OCR V22 P1 runner source bundle changed")
    if sha256_file(REPO_ROOT / SEAL_PATH) != config.get("split_seal_sha256"):
        raise RuntimeError("OCR V22 split seal changed before P1")
    ...
    for split, (path, config_key) in archive_bindings.items():
        ...
        if actual != seal["splits"][split]["archive_sha256"] or actual != config.get(config_key):
            raise RuntimeError(f"OCR V22 {split} archive changed before P1")
    exact_inputs = {
    ...
    if (REPO_ROOT / CANONICAL_OUTPUT).exists():
        raise RuntimeError("OCR V22 P1 output already exists")
...
def _feature_groups(records: tuple[ProposalRecord, ...], scene_count: int) -> tuple[np.ndarray, ...]:
    ...
    if any(len(indices) == 0 for indices in groups):
        raise RuntimeError("OCR V22 production proposal stream omitted an entire scene")
    flattened = np.concatenate(groups)
    if not np.array_equal(flattened, np.arange(len(records), dtype=np.int64)):
        raise RuntimeError("OCR V22 production proposal stream is not scene-contiguous")
    return groups
...
def _calibrated_records(
    ...
    if outputs.shape != (len(records), 2 + len(ROLE_ORDER)):
        raise RuntimeError("OCR V22 calibrator output stream shape changed")
    return tuple(
...
def train_candidate(output_dir: Path) -> dict[str, object]:
    if output_dir.exists():
        raise RuntimeError(f"OCR V22 P1 output exists: {output_dir}")
    evidence = preflight()
    ...
    try:
        ...
        if train_values.shape[1:] != (FEATURE_COUNT,):
            raise RuntimeError("OCR V22 P1 training feature width changed")
        ...
        for key in ("proposal_count", "positive_proposal_count", "negative_proposal_count"):
            if training_evidence[key] != registered_training[key]:
                raise RuntimeError(f"OCR V22 P1 incomplete training proposal stream: {key}")
        ...
        if int(train_labels.sum()) != expected_truths or np.any(train_roles[train_labels == 1] < 0):
            raise RuntimeError("OCR V22 P1 production stream omitted a training truth")
        ...
        if positives == 0 or negatives == 0:
            raise RuntimeError("OCR V22 P1 requires positive and negative production proposals")
        ...
        for epoch in range(int(config["epochs"])):
            ...
            for scene_index in torch.randperm(len(train_groups), generator=generator).tolist():
                ...
                proposal_loss = nn.functional.cross_entropy(
                    output[:, :2], proposal_targets, weight=proposal_weights,
                )
            ...
            losses.append({"epoch": epoch + 1, "loss": epoch_loss / len(train_groups)})
            print(
                f"OCR V22 P1 epoch {epoch + 1}/{config['epochs']} complete; "
                f"optimizer_steps={optimizer_steps}",
                flush=True,
            )
        if optimizer_steps != int(config["expected_optimizer_steps"]):
            raise RuntimeError("OCR V22 P1 optimizer-step count changed")

        phase = "checkpoint_and_onnx_export"
        checkpoint_path = output_dir / "graph-text-scene-evidence-attention-v22-p1.pt"
        torch.save({"state_dict": model.state_dict()}, checkpoint_path)
        onnx_path = output_dir / "graph-text-scene-evidence-attention-v22-p1.onnx"
        ...
        for key in ("proposal_count", "positive_proposal_count", "negative_proposal_count"):
            if selection_evidence[key] != registered_selection[key]:
                raise RuntimeError(f"OCR V22 P1 incomplete selection proposal stream: {key}")
        ...
        if sum(record.truth_index >= 0 for record in selection_records) != expected_truths:
            raise RuntimeError("OCR V22 P1 production stream omitted a validation truth")
