...
"""One-time fixture freeze for OCR scene-evidence attention V22."""
...
ARCHIVE_PATHS = {
    "train": Path("artifacts/production-validation/ocr-v22-train.zip"),
    "validation": Path("artifacts/production-validation/ocr-v22-selection.zip"),
    "sealed_public": Path("artifacts/production-validation/ocr-v22-public.zip"),
}
SOURCE_PATHS = (
    ROOT / "PROTOCOL.json",
    ROOT / "dataset.py",
    ...
    ROOT / "protocol.py",
    Path(V20_RESULT_PATH),
    Path(V21_RESULT_PATH),
...
def _require_sources_at_head() -> None:
    for path in SOURCE_PATHS:
        ...
        if not local.is_file():
            raise RuntimeError(f"OCR V22 required source is missing: {relative}")
        completed = subprocess.run(
            ("git", "show", f"HEAD:{relative}"), cwd=REPO_ROOT,
            check=False, capture_output=True,
        )
        if completed.returncode != 0 or completed.stdout != local.read_bytes():
            raise RuntimeError(f"OCR V22 source must be committed unchanged before freeze: {relative}")
    expected = {
        V20_RESULT_PATH: V20_RESULT_SHA256,
        V21_RESULT_PATH: V21_RESULT_SHA256,
    ...
    for relative, digest in expected.items():
        ...
        if not path.is_file() or sha256_file(path) != digest:
            raise RuntimeError(f"OCR V22 exact prerequisite changed: {relative}")
...
def freeze() -> dict[str, object]:
    if (REPO_ROOT / SEAL_PATH).exists():
        raise RuntimeError("OCR V22 split identity is already frozen")
    existing = [path.as_posix() for path in ARCHIVE_PATHS.values() if (REPO_ROOT / path).exists()]
    if existing:
        raise RuntimeError("OCR V22 archive already exists before freeze: " + ", ".join(existing))
    ...
    for split, relative in ARCHIVE_PATHS.items():
        ...
        if len(hashes) != len(scenes):
            raise RuntimeError(f"OCR V22 {split} contains duplicate fixture bytes")
    ...
    if any(overlaps.values()):
        raise RuntimeError("OCR V22 split fixture bytes overlap")
