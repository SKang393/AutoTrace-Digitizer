...
"""Truth-hidden archive writer for the stride-4 detector public split."""
...
def save_sealed_public_archive(samples: tuple[EvaluationFrame, ...], path: Path) -> dict[str, object]:
    if path.exists():
        raise RuntimeError(f"Refusing to overwrite stride-4 sealed fixtures: {path}")
    ...
    return {
        "schema": "graphreader.ocr-graph-text-stride4-private-manifest.v1",
