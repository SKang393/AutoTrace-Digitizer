...
"""One-time full-selection geometry diagnosis for failed candidate P2."""
...
from ml.ocr.graph_text_ignore_band_v3.train_p1 import normalize_bgr
...
REPORT_PATH = Path("ml/ocr/graph_text_stride4_v4/artifacts/P2-run/candidate-report.json")
ONNX_PATH = Path("ml/ocr/graph_text_stride4_v4/artifacts/P2-run/graph-text-stride4-v4-p2.onnx")
OUTPUT_PATH = Path("ml/ocr/graph_text_stride4_v4/artifacts/P2-run/selection-diagnosis.json")
EXPECTED_REPORT_SHA256 = "04a4c6aa6f541dcba1f95c1aac2e02ad07dde3d90e2b742a5f842f11322359f4"
EXPECTED_ONNX_SHA256 = "4fea761e5d905b9c081cbe1a2b4d2f9a535657342a34c85b0ed572a47d6bc8f1"
...
def main() -> int:
    ...
    if output_path.exists():
        raise RuntimeError(f"P2 diagnosis already exists and cannot be rerun: {output_path}")
    if sha256_file(report_path) != EXPECTED_REPORT_SHA256:
        raise RuntimeError("P2 report changed before diagnosis")
    if sha256_file(onnx_path) != EXPECTED_ONNX_SHA256:
        raise RuntimeError("P2 ONNX changed before diagnosis")

    report = json.loads(report_path.read_text(encoding="utf-8"))
    expected_records = report["selection_metrics"]["records"]
    ...
    for frame in frames:
        ...
        if len(regions) != int(expected["prediction_count"]):
            raise RuntimeError(f"P2 diagnosis prediction count drifted for {frame.case_id}")
        ...
        ranked = sorted(((_iou(region.bounds, frame.truth_bbox), region) for region in regions), reverse=True, key=lambda item: item[0])
        if not ranked:
            group["no_region_count"] += 1
            no_region_maximums.append(float(probabilities.max()))
            no_region_pixels_above_threshold.append(float(np.count_nonzero(probabilities > np.float32(0.3))))
            continue
    ...
    diagnosis = {
        "schema": "graphreader.ocr-graph-text-stride4-selection-diagnosis.v1",
        "task": "ocr-detection",
