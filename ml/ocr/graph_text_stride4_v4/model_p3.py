...
"""Full-resolution input refinement for stride-4 detector candidate P3."""
...
class Stride4FullResolutionRefinementNet(nn.Module):
    """Fuse input-resolution detail with P2's stable resize-convolution decoder."""
    ...
    def __init__(self, seed: int = 20260911) -> None:
        ...
        self.input_detail = nn.Conv2d(3, 8, kernel_size=3, padding=1)
        self.fusion = nn.Conv2d(20, 12, kernel_size=3, padding=1)
    ...
    def forward(self, value: torch.Tensor) -> torch.Tensor:
        if not torch.onnx.is_in_onnx_export():
            if value.ndim != 4 or value.shape[1] != 3:
                raise ValueError("Stride-4 P3 detector input must be [batch,3,H,W]")
            if value.shape[2] % 4 or value.shape[3] % 4:
                raise ValueError("Stride-4 P3 detector dimensions must be divisible by four")
        ...
        input_detail = self.activation(self.input_detail(value))
        fused = self.activation(self.fusion(torch.cat((full_resolution, input_detail), dim=1)))
        return torch.clamp(torch.sigmoid(self.output(fused)), min=0.0, max=1.0)


__all__ = ["Stride4FullResolutionRefinementNet"]
