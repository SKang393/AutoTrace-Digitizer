...
"""Fixed scene and role metrics for OCR V13."""
...
def evaluate_thresholds(
    ...
    for scene in scenes:
        ...
        if not candidates:
            raise RuntimeError("OCR V13 scene produced no proposals")
        ...
        if logits.shape != (len(candidates), 2 + len(ROLE_ORDER)) or not np.isfinite(logits).all():
            raise RuntimeError("OCR V13 runner returned invalid logits")
