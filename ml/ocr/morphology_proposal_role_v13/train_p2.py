...
"""Single-use zero-optimizer P2 calibration candidate for OCR V13."""
...
P1_CHECKPOINT_PATH = ROOT / "artifacts/P1-run/graph-text-morphology-proposal-role-v13-p1.pt"
...
class ScaledCandidate(nn.Module):
    def __init__(self, candidate: MorphologyProposalRoleNet, scale: float) -> None:
        super().__init__()
...
def run_candidate(output_dir: Path) -> dict[str, object]:
    if output_dir.exists():
        raise RuntimeError(f"OCR V13 P2 output exists: {output_dir}")
    ...
    try:
        ...
        if (
            ...
            raise RuntimeError("OCR V13 P1 is not the exact unopened failed-selection trigger")
        checkpoint_path = REPO_ROOT / P1_CHECKPOINT_PATH
        if (
            p1["checkpoint_sha256"] != config["p1_checkpoint_sha256"]
            or sha256_file(checkpoint_path) != config["p1_checkpoint_sha256"]
            or p1["report_sha256"] != config["p1_report_sha256"]
            or p1["onnx_sha256"] != config["p1_onnx_sha256"]
        ):
            raise RuntimeError("OCR V13 P1 evidence binding changed")
        ...
        if split_fingerprint(validation) != selection["validation"]["split_fingerprint"]:
            raise RuntimeError("OCR V13 P2 validation split changed")
        if proposal_summary(validation) != {key: selection["validation"][key] for key in proposal_summary(validation)}:
            raise RuntimeError("OCR V13 P2 validation proposals changed")

        base = MorphologyProposalRoleNet(seed=int(config["seed"]))
        ...
        phase = "export"
        onnx_path = output_dir / "graph-text-morphology-proposal-role-v13-p2.onnx"
        _export(model, parity_values, onnx_path)
        session = ort.InferenceSession(str(onnx_path), providers=["CPUExecutionProvider"])
        if session.get_providers() != ["CPUExecutionProvider"]:
            raise RuntimeError("OCR V13 P2 requires CPU selection execution")
        ...
        report: dict[str, object] = {
            ...
            "effective_output_logit_scale": config["effective_output_logit_scale"],
