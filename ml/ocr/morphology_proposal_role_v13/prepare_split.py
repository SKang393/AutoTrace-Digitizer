...
"""Freeze V13 train, selection, and hidden-public evidence."""
...
def freeze_split() -> dict[str, str]:
    ...
    if existing:
        raise RuntimeError("OCR V13 split freeze refuses overwrite: " + ", ".join(existing))
    protocol_path = REPO_ROOT / PROTOCOL
    if protocol_path.read_bytes() != canonical_json_bytes(protocol_configuration()):
        raise RuntimeError("OCR V13 committed preregistration changed before split freeze")
    ...
    if any(training_evidence[key] is not False for key in (
        ...
        raise RuntimeError("OCR V13 training examples violated the frozen scope")
    selection = {
        "schema": "graphreader.ocr-morphology-proposal-role-selection.v1", "task": TASK, "revision": REVISION,
    ...
    seal = {
        "schema": "graphreader.ocr-morphology-proposal-role-sealed-test-seal.v1", "task": TASK,
    ...
    gate = {
        "schema": "graphreader.ocr-morphology-proposal-role-gate-config.v1", "task": TASK,
    ...
    config = {
        "schema": "graphreader.ocr-morphology-proposal-role-training.v1", "task": TASK, "revision": REVISION,
