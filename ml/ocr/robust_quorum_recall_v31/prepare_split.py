...
"""One-time fresh fixture freeze for OCR robust quorum recall V31."""
...
ARCHIVE_PATHS = {
    "train": Path("artifacts/production-validation/ocr-v31-train.zip"),
    "validation": Path("artifacts/production-validation/ocr-v31-selection.zip"),
    "sealed_public": Path("artifacts/production-validation/ocr-v31-public.zip"),
}
SOURCE_PATHS = (
    ROOT / "PROTOCOL.json",
    ROOT / "dataset.py",
    ...
    Path("ml/ocr/unanimous_structure_veto_v30/dataset.py"),
    Path("ml/ocr/unanimous_structure_veto_v30/model.py"),
...
def _require_sources_at_head() -> None:
    for path in SOURCE_PATHS:
        ...
        if not local.is_file():
            raise RuntimeError(f"OCR V31 required source is missing: {relative}")
        completed = subprocess.run(
        ...
        if completed.returncode != 0 or completed.stdout != local.read_bytes():
            raise RuntimeError(
                f"OCR V31 source must be committed unchanged before freeze: {relative}"
            )
    for relative, digest in {
        ...
        if not path.is_file() or sha256_file(path) != digest:
            raise RuntimeError(f"OCR V31 exact prerequisite changed: {relative}")
...
def freeze() -> dict[str, object]:
    ...
    if existing:
        raise RuntimeError(
            "OCR V31 archive already exists before freeze: " + ", ".join(existing)
        )
