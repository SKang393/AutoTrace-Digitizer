...
"""Run the final preregistered project numeric OCR Candidate 3."""
...
from .candidate3_model import SpatialQuerySlotRecognizer
from .candidate3_protocol import (
    CANDIDATE_ID,
    CANONICAL_OUTPUT_PATH,
    assert_candidate_execution_allowed,
    validate_frozen_protocol,
)
from .dataset import split_fingerprint, split_metadata
...
from .verify_candidate3 import verify_committed_preregistration
...
def run(output: Path, candidate_id: str) -> dict[str, object]:
    ...
    for split_name, samples in (("train", train_samples), ("validation", validation_samples)):
        if split_fingerprint(samples) != frozen["split_fingerprints"][split_name]:
            raise RuntimeError(f"Frozen Candidate 3 {split_name} fingerprint mismatch.")

    output.mkdir(parents=True, exist_ok=False)
    model = SpatialQuerySlotRecognizer()
    ...
    checkpoint = output / "graph-numeric-project-v1-candidate3.pt"
    torch.save(
        {
    ...
    validation = _evaluate(model, validation_samples)  # type: ignore[arg-type]
    ...
    if validation_passed:
        ...
        if split_fingerprint(sealed_samples) != frozen["split_fingerprints"]["sealed_test"]:
            raise RuntimeError("Frozen Candidate 3 sealed-test fingerprint mismatch.")
        sealed = _evaluate(model, sealed_samples)  # type: ignore[arg-type]
        quality_passed = _quality_passes(sealed, sealed=True)
        if quality_passed:
            onnx_result = _export_and_measure_parity(  # type: ignore[arg-type]
    ...
    report = {
        ...
        "split_metadata": {
            ...
            "sealed_test": split_metadata(sealed_samples) if validation_passed else sealed,
        },
