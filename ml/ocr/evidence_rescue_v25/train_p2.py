...
"""Single-use pooled-evidence residual training for OCR V25 P2."""
...
from .model_p2 import FrozenParentPooledEvidenceResidualNet
...
def _proposal_residual_objective(
    ...
    if not torch.any(positive) or not torch.any(negative):
        raise RuntimeError("OCR V25 P2 requires positive and negative proposals per scene")
...
def preflight() -> dict[str, Any]:
    ...
    expected = {
        ...
        "candidate_limit": 3,
        "architecture": "frozen-v24-parent-plus-pooled-evidence-residual-v1",
        "objective": "teacher-preserving-pooled-residual-separation-v1",
    ...
    for key, value in expected.items():
        if config.get(key) != value:
            raise RuntimeError(f"OCR V25 P2 config field mismatch: {key}")
    if config.get("selection_thresholds") != list(THRESHOLDS):
        raise RuntimeError("OCR V25 P2 thresholds changed")
    if config.get("expected_runner_source_bundle_sha256") != source_bundle_sha256(
        REPO_ROOT, RUNNER_SOURCE_PATHS,
    ):
        raise RuntimeError("OCR V25 P2 runner source bundle changed")
    exact_inputs = {
    ...
    for relative, expected_hash in exact_inputs.items():
        ...
        if not path.is_file() or sha256_file(path) != expected_hash:
            raise RuntimeError(f"OCR V25 P2 frozen input changed: {relative.as_posix()}")
    p1 = _read_json(REPO_ROOT / P1_RESULT_PATH)
    metrics = p1.get("selection_metrics", {})
    if (
        ...
        raise RuntimeError("OCR V25 P2 aggregate-only P1 trigger changed")
    ...
    if (REPO_ROOT / CANONICAL_OUTPUT).exists():
        raise RuntimeError("OCR V25 P2 output already exists")
    return {"config": config, "head": head, "seal": seal}


def train_candidate(output_dir: Path) -> dict[str, object]:
    if output_dir.exists():
        raise RuntimeError(f"OCR V25 P2 output exists: {output_dir}")
    evidence = preflight()
    ...
    try:
        ...
        if train_values.shape[1:] != (FEATURE_COUNT,):
            raise RuntimeError("OCR V25 P2 training evidence width changed")
        if train_crops.shape[1:] != (CROP_CHANNELS, CROP_HEIGHT, CROP_WIDTH):
            raise RuntimeError("OCR V25 P2 training crop shape changed")
        ...
        for key in ("proposal_count", "positive_proposal_count", "negative_proposal_count"):
            if training_evidence[key] != registered[key]:
                raise RuntimeError(f"OCR V25 P2 incomplete training stream: {key}")

        phase = "pooled_evidence_residual_training"
        generator = _configure(int(config["seed"]))
        model = FrozenParentPooledEvidenceResidualNet(
            seed=int(config["seed"]), residual_scale=float(config["residual_scale"]),
        )
        ...
        if not trainable or any(name.startswith("parent.") for name in trainable_names):
            raise RuntimeError("OCR V25 P2 frozen-parent boundary changed")
        ...
        for epoch in range(int(config["epochs"])):
            ...
            print(
                f"OCR V25 P2 epoch {epoch + 1}/{config['epochs']} complete; "
                f"optimizer_steps={optimizer_steps}",
                flush=True,
            )
        if optimizer_steps != int(config["expected_optimizer_steps"]):
            raise RuntimeError("OCR V25 P2 optimizer-step count changed")
        frozen_after = _parameter_stream_sha256(model.parent)
        if frozen_after != frozen_before:
            raise RuntimeError("OCR V25 P2 modified the frozen V24 parent")

        phase = "checkpoint_and_onnx_export"
        checkpoint_path = output_dir / "graph-text-evidence-rescue-v25-p2.pt"
        torch.save({"state_dict": model.state_dict()}, checkpoint_path)
        onnx_path = output_dir / "graph-text-evidence-rescue-v25-p2.onnx"
        ...
        if {item.name for item in candidate_session.get_inputs()} != {
            ...
            raise RuntimeError("OCR V25 P2 ONNX input identity changed")
        ...
        for key in ("proposal_count", "positive_proposal_count", "negative_proposal_count"):
            if selection_evidence[key] != registered[key]:
                raise RuntimeError(f"OCR V25 P2 incomplete selection stream: {key}")
