...
"""Single-use truth-hidden public gate for a selected OCR V25 candidate."""
...
PUBLIC_REVISION = "graph-text-evidence-rescue-v25-public-v1"
...
def _cpu_session(path: Path) -> ort.InferenceSession:
    ...
    if session.get_providers() != ["CPUExecutionProvider"]:
        raise RuntimeError("OCR V25 public gate requires CPUExecutionProvider only")
    return session
...
def _public_window(selection: dict[str, object]) -> tuple[float, ...]:
    ...
    if (
        ...
        raise RuntimeError("OCR V25 selection has no preregistered robust threshold window")
...
def evaluate_public(*, selection_report_path: Path, output_path: Path) -> dict[str, object]:
    ...
    if output_path.exists():
        raise RuntimeError(f"OCR V25 public output exists: {output_path}")
    ...
    ordinal = (
        int(candidate_id[1:])
        if isinstance(candidate_id, str) and candidate_id.startswith("P") and candidate_id[1:].isdigit()
        else 0
    )
    ...
    if (
        ...
        raise RuntimeError("OCR V25 public gate is not authorized by the canonical ledger")
    if (
        ...
        raise RuntimeError("Only the exact robustly selected OCR V25 candidate may open the public gate")
    ...
    if (
        ...
        raise RuntimeError("OCR V25 public inputs changed")
    ...
    if (
        ...
        raise RuntimeError("OCR V25 public fixtures violate the frozen contract")
    ...
    if (
        ...
        raise RuntimeError("OCR V25 public production proposal stream is invalid")
    ...
    for indices in groups:
        ...
        if parent_output.shape != candidate_output.shape or candidate_output.shape[2] != 10:
            raise RuntimeError("OCR V25 public candidate output contract changed")
    ...
    if len(groups) != len(scenes) or parent_role_error != 0.0:
        raise RuntimeError("OCR V25 public candidate did not preserve exact parent roles")
    ...
    report: dict[str, object] = {
        "schema": "graphreader.ocr-evidence-rescue-public-gate.v1",
        "task": TASK,
        "revision": PUBLIC_REVISION,
        ...
        "parent_sha256": PARENT_ONNX_SHA256,
