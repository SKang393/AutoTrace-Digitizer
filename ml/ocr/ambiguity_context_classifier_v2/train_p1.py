...
"""Single-use P1 training for the line-context ambiguity classifier."""
...
def train_candidate(output_dir: Path) -> dict[str, object]:
    if output_dir.exists():
        raise RuntimeError(f"Line-context ambiguity output exists: {output_dir}")
    ...
    try:
        ...
        for key, label in (
            ...
            if sha256_file(path) != config[f"{key}_sha256"]:
                raise RuntimeError(f"Line-context ambiguity {label} changed")
        ...
        if sha256_file(REPO_ROOT / str(public_seal["fixture_archive_path"])) != public_seal["fixture_archive_sha256"]:
            raise RuntimeError("Line-context ambiguity public archive changed before training")
        ...
        if split_fingerprint("train") != selection["train_split_fingerprint"]:
            raise RuntimeError("Line-context ambiguity training split changed")
        if split_fingerprint("validation") != selection["validation_split_fingerprint"]:
            raise RuntimeError("Line-context ambiguity validation split changed")

        model = LineContextAmbiguityNet(seed=seed)
        ...
        phase = "export"
        checkpoint_path = output_dir / "graph-ambiguity-line-context-v2-p1.pt"
        torch.save({"state_dict": model.state_dict(), "class_order": list(GLYPHS)}, checkpoint_path)
        onnx_path = output_dir / "graph-ambiguity-line-context-v2-p1.onnx"
