...
"""Freeze V16 train, selection, and truth-hidden public evidence."""
...
def freeze_split() -> dict[str, str]:
    ...
    if existing:
        raise RuntimeError("OCR V16 split freeze refuses overwrite: " + ", ".join(existing))
    protocol_path = REPO_ROOT / PROTOCOL
    if protocol_path.read_bytes() != canonical_json_bytes(protocol_configuration()):
        raise RuntimeError("OCR V16 committed preregistration changed before split freeze")
    ...
    if any(training_evidence[key] is not False for key in (
        ...
        raise RuntimeError("OCR V16 training examples violated the frozen scope")
    selection = {
        "schema": "graphreader.ocr-margin-robust-layout-proposal-role-selection.v1",
    ...
    seal = {
        "schema": "graphreader.ocr-margin-robust-layout-proposal-role-sealed-test-seal.v1",
    ...
    gate = {
        "schema": "graphreader.ocr-margin-robust-layout-proposal-role-gate-config.v1",
    ...
    config = {
        "schema": "graphreader.ocr-margin-robust-layout-proposal-role-training.v1",
        ...
        "proposal_margin": training["proposal_margin"],
        "proposal_margin_loss_weight": training["proposal_margin_loss_weight"],
        "role_loss_weight": training["role_loss_weight"],
        **training_evidence,
