...
"""Fixed scene, proposal, role, and threshold-margin metrics for OCR V16."""
...
def evaluate_thresholds(
    ...
    for scene in scenes:
        ...
        if not candidates:
            raise RuntimeError("OCR V16 scene produced no proposals")
        values = np.stack([
            encode_proposal(scene.raster, candidate, scene.plot) for candidate in candidates
        ]).astype(np.float32)
        input_stream.update(np.ascontiguousarray(values).tobytes(order="C"))
        ...
        if logits.shape != (len(candidates), 2 + len(ROLE_ORDER)) or not np.isfinite(logits).all():
            raise RuntimeError("OCR V16 runner returned invalid logits")
        output_stream.update(np.ascontiguousarray(logits).tobytes(order="C"))
        shifted = logits[:, :2] - logits[:, :2].max(axis=1, keepdims=True)
        exponent = np.exp(shifted)
        probability = exponent[:, 1] / exponent.sum(axis=1)
        cached.append((scene, candidates, probability, np.argmax(logits[:, 2:], axis=1)))
