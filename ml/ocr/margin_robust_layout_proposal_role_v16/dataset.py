...
"""Fresh margin-robust procedural graph scenes for OCR V16."""
...
@contextmanager
def _v16_registration_scope() -> Iterator[None]:
    """Route the reviewed procedural renderer through frozen V16 registrations."""
...
def render_scene(split: Split, index: int) -> SceneSample:
    ...
    if index < 0 or index >= registration.scene_count:
        raise IndexError(f"OCR V16 scene index outside frozen {split} split: {index}")
    with _v16_registration_scope():
        source = v15.render_scene(split, index)
    return SceneSample(
        f"margin-robust-layout-proposal-role-v16-{split}-{index:05d}",
        split,
...
def save_sealed_public_archive(scenes: tuple[SceneSample, ...], path: Path) -> dict[str, object]:
    if any(scene.split != "sealed_public" for scene in scenes):
        raise RuntimeError("OCR V16 sealed archive accepts only sealed-public scenes")
    summary = proposal_summary(scenes)
    manifest: dict[str, object] = {
        "schema": "graphreader.ocr-margin-robust-layout-proposal-role-fixtures.v1",
        "split": "sealed_public",
        **summary,
        "cases": [],
    }
    ...
    return {
        "schema": "graphreader.ocr-margin-robust-layout-proposal-role-private-manifest.v1",
        **summary,
...
def load_sealed_public_archive(path: Path, private_manifest_path: Path) -> tuple[SceneSample, ...]:
    ...
    if private.get("schema") != "graphreader.ocr-margin-robust-layout-proposal-role-private-manifest.v1":
        raise RuntimeError("OCR V16 private manifest schema mismatch")
    if private.get("fixture_archive_sha256") != sha256_file(path):
        raise RuntimeError("OCR V16 sealed archive checksum mismatch")
    truth_by_id = {case["scene_id"]: case["items"] for case in private["truths"]}
    scenes: list[SceneSample] = []
    with ZipFile(path, "r") as archive:
        manifest = json.loads(archive.read("manifest.json"))
        if manifest.get("schema") != "graphreader.ocr-margin-robust-layout-proposal-role-fixtures.v1":
            raise RuntimeError("OCR V16 sealed fixture schema mismatch")
        for case in manifest["cases"]:
            payload = archive.read(case["image_path"])
            if sha256(payload).hexdigest() != case["source_sha256"]:
                raise RuntimeError("OCR V16 sealed fixture checksum mismatch")
    ...
    if proposal_summary(result)["role_truth_counts"] != {role: len(result) for role in ROLE_ORDER}:
        raise RuntimeError("OCR V16 sealed role balance changed")
    return result
