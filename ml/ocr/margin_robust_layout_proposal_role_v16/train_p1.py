...
"""Single-use P1 training for margin-robust OCR V16."""
...
def _balanced_order(labels: torch.Tensor, generator: torch.Generator) -> torch.Tensor:
    ...
    if len(negative) == 0 or len(positive) == 0:
        raise RuntimeError("OCR V16 balanced sampler requires both proposal classes")
...
def _select_robust_window(
    ...
    if not eligible:
        return None
    run = max(eligible, key=lambda item: (
        len(item),
        -abs(float(comparisons[item[len(item) // 2]]["threshold"]) - 0.68),
    ))
    selected = comparisons[run[len(run) // 2]]
    window = tuple(float(comparisons[index]["threshold"]) for index in run)
    return selected, window


def train_candidate(output_dir: Path) -> dict[str, object]:
    if output_dir.exists():
        raise RuntimeError(f"OCR V16 output exists: {output_dir}")
    ...
    try:
        if config["expected_runner_source_bundle_sha256"] != source_bundle_sha256(REPO_ROOT, RUNNER_SOURCE_PATHS):
            raise RuntimeError("OCR V16 runner sources changed")
        if config["selection_manifest_sha256"] != sha256_file(REPO_ROOT / SELECTION_PATH):
            raise RuntimeError("OCR V16 selection manifest changed")
        if config["sealed_public_test_seal_sha256"] != sha256_file(REPO_ROOT / SEAL_PATH):
            raise RuntimeError("OCR V16 public seal changed")
        ...
        for key in (
            ...
            if training_evidence[key] != config[key]:
                raise RuntimeError(f"OCR V16 training evidence changed: {key}")
        if any(training_evidence[key] is not False for key in (
            ...
            raise RuntimeError("OCR V16 training data violated preregistered scope")

        generator = _configure(int(config["seed"]))
        model = MarginRobustLayoutProposalRoleNet(seed=int(config["seed"]))
        ...
        for epoch in range(int(config["epochs"])):
            ...
            for start in range(0, len(order), int(config["batch_size"])):
                ...
                batch_targets = proposal_targets.index_select(0, indices)
                output = model(tensors.index_select(0, indices))
                proposal_loss = proposal_criterion(output[:, :2], batch_targets)
                signed_target = batch_targets.to(torch.float32).mul(2.0).sub(1.0)
                signed_logit = output[:, 1] - output[:, 0]
                margin_loss = torch.relu(float(config["proposal_margin"]) - signed_target * signed_logit).mean()
                ...
                loss = (
                    proposal_loss
                    + float(config["proposal_margin_loss_weight"]) * margin_loss
                    + float(config["role_loss_weight"]) * role_loss
                )
            ...
            if epoch in {0, int(config["epochs"]) // 2, int(config["epochs"]) - 1}:
                checkpoints.append({
                    "epoch": epoch + 1,
                    "multitask_loss": sum(losses) / len(losses),
                    "proposal_margin_loss": sum(margins) / len(margins),
                })

        phase = "export"
        model.eval()
        checkpoint_path = output_dir / "graph-text-margin-robust-layout-proposal-role-v16-p1.pt"
        torch.save({"state_dict": model.state_dict()}, checkpoint_path)
        onnx_path = output_dir / "graph-text-margin-robust-layout-proposal-role-v16-p1.onnx"
        parity_values = torch.from_numpy(values[:256])
        _export(model, parity_values, onnx_path)
        session = ort.InferenceSession(str(onnx_path), providers=["CPUExecutionProvider"])
        if session.get_providers() != ["CPUExecutionProvider"]:
            raise RuntimeError("OCR V16 selection requires CPU execution only")
        ...
        passed = robust is not None and parity_passed and calls == len(validation)
