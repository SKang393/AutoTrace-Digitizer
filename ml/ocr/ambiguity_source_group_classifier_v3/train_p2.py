...
from .model_p2 import ParityScaledSourceGroupNet
...
RUNNER_SOURCE_PATHS = (
    ROOT / "crop.py", ROOT / "dataset.py", ROOT / "model.py", ROOT / "model_p2.py",
...
def train_candidate(output_dir: Path) -> dict[str, object]:
    if output_dir.exists():
        raise RuntimeError(f"Source-group ambiguity P2 output exists: {output_dir}")
    ...
    try:
        ...
        for key, label in (
            ...
            if sha256_file(path) != config[f"{key}_sha256"]:
                raise RuntimeError(f"Source-group ambiguity P2 {label} changed")
        ...
        if sha256_file(REPO_ROOT / str(public_seal["fixture_archive_path"])) != public_seal["fixture_archive_sha256"]:
            raise RuntimeError("Source-group ambiguity public archive changed before P2")
        ...
        if split_fingerprint("validation") != selection["validation_split_fingerprint"]:
            raise RuntimeError("Source-group ambiguity P2 validation split changed")
        ...
        if checkpoint.get("class_order") != list(GLYPHS):
            raise RuntimeError("Source-group ambiguity P1 checkpoint class order changed")
        base = SourceGroupAmbiguityNet(seed=seed)
        base.load_state_dict(checkpoint["state_dict"])
        model = ParityScaledSourceGroupNet(base).eval()
        phase = "selection"
        ...
        phase = "export"
        onnx_path = output_dir / "graph-ambiguity-source-group-v3-p2.onnx"
        ...
        report: dict[str, object] = {
            ...
            "isolated_change": "multiply exact P1 output logits by 0.125 during export",
