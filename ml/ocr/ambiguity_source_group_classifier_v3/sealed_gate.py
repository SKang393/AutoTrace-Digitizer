...
"""Single-use direct ONNX gate over unopened source-group fixtures."""
...
EVALUATOR_SOURCE_PATHS = (
    ROOT / "crop.py", ROOT / "dataset.py", ROOT / "protocol.py", ROOT / "sealed_gate.py",
...
def _load_public(seal: dict[str, object]) -> tuple[np.ndarray, np.ndarray, str]:
    ...
    if sha256_file(archive_path) != seal["fixture_archive_sha256"] or sha256_file(manifest_path) != seal["private_manifest_sha256"]:
        raise RuntimeError("Source-group ambiguity public split bytes changed")
    ...
    with zipfile.ZipFile(BytesIO(archive_path.read_bytes())) as fixtures:
        for sample in manifest["samples"]:
            ...
            if hash_bytes(source) != sample["source_sha256"]:
                raise RuntimeError(f"Source-group public fixture changed: {sample['sample_id']}")
            with Image.open(BytesIO(source)) as image:
                gray = image.convert("L")
                groups = active_groups(gray)
                if len(groups) != int(sample["group_count"]):
                    raise RuntimeError(f"Source-group count changed: {sample['sample_id']}")
                tensor = group_tensor(gray, groups, int(sample["target_group_index"]))
            if tensor.shape != (1, IMAGE_SIZE, IMAGE_SIZE):
                raise RuntimeError("Source-group public tensor shape changed")
            values.append(tensor)
            labels.append(int(sample["label"]))
    stacked = np.stack(values).astype(np.float32)
    encoded_labels = np.asarray(labels, dtype=np.int64)
    tensor_label_digest = hash_bytes(
        np.ascontiguousarray(stacked).tobytes() + np.ascontiguousarray(encoded_labels).tobytes()
    )
    if tensor_label_digest != seal["tensor_label_stream_sha256"]:
        raise RuntimeError("Source-group public adapter output changed")
    return stacked, encoded_labels, str(seal["fixture_archive_sha256"])


def evaluate_public(*, onnx_path: Path, selection_report_path: Path, output_path: Path) -> dict[str, object]:
    if output_path.exists():
        raise RuntimeError(f"Source-group ambiguity public output exists: {output_path}")
    ...
    if (
        ...
        raise RuntimeError("Source-group ambiguity public gate lacks exact authorization")
    ...
    if sha256_file(seal_path) != split_config["sealed_public_test_seal_sha256"]:
        raise RuntimeError("Source-group ambiguity public seal changed")
    ...
    if session.get_providers() != ["CPUExecutionProvider"]:
        raise RuntimeError("Source-group ambiguity public gate requires CPUExecutionProvider only")
    input_digest = sha256()
    ...
    output: dict[str, object] = {
        "schema": "graphreader.ocr-ambiguity-source-group-public-gate.v1",
