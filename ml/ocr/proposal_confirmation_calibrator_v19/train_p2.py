...
"""Single-use P2 training and visible selection for OCR V19."""
...
RUNNER_SOURCE_PATHS = (
    ...
    ROOT / "train_p2.py", Path("ml/ocr/official_bakeoff/production_evaluate.py"),
...
def train_candidate(output_dir: Path) -> dict[str, object]:
    if output_dir.exists():
        raise RuntimeError(f"OCR V19 P2 output exists: {output_dir}")
    ...
    try:
        if config["expected_runner_source_bundle_sha256"] != source_bundle_sha256(REPO_ROOT, RUNNER_SOURCE_PATHS):
            raise RuntimeError("OCR V19 P2 runner sources changed")
        ...
        if optimizer_steps != int(config["expected_optimizer_steps"]):
            raise RuntimeError("OCR V19 P2 optimizer-step count changed")

        phase = "export"
        checkpoint_path = output_dir / "graph-text-proposal-confirmation-calibrator-v19-p2.pt"
        torch.save({"state_dict": model.state_dict()}, checkpoint_path)
        onnx_path = output_dir / "graph-text-proposal-confirmation-calibrator-v19-p2.onnx"
