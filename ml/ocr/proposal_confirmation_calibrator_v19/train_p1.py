...
"""Single-use P1 training and visible selection for OCR V19."""
...
from .model import ProposalConfirmationCalibrator
...
def train_candidate(output_dir: Path) -> dict[str, object]:
    if output_dir.exists():
        raise RuntimeError(f"OCR V19 P1 output exists: {output_dir}")
    ...
    try:
        if config["expected_runner_source_bundle_sha256"] != source_bundle_sha256(REPO_ROOT, RUNNER_SOURCE_PATHS):
            raise RuntimeError("OCR V19 P1 runner sources changed")
        ...
        model = ProposalConfirmationCalibrator(seed=int(config["seed"]))
        ...
        if optimizer_steps != int(config["expected_optimizer_steps"]):
            raise RuntimeError("OCR V19 P1 optimizer-step count changed")

        phase = "export"
        checkpoint_path = output_dir / "graph-text-proposal-confirmation-calibrator-v19-p1.pt"
        torch.save({"state_dict": model.state_dict()}, checkpoint_path)
        onnx_path = output_dir / "graph-text-proposal-confirmation-calibrator-v19-p1.onnx"
        ...
        actual = np.asarray(calibrator_session.run(None, {calibrator_input: parity_values.numpy()})[0], dtype=np.float32)
        ...
        calibration_input_stream = sha256_file(onnx_path)
        validation_logits = np.asarray(
            calibrator_session.run(None, {calibrator_input: np.ascontiguousarray(validation_values)})[0],
            dtype=np.float32,
        )
        selection_evidence["calibrator_input_tensor_stream_sha256"] = __import__("hashlib").sha256(
            np.ascontiguousarray(validation_values).tobytes(order="C")
        ).hexdigest()
        selection_evidence["calibrator_output_tensor_stream_sha256"] = __import__("hashlib").sha256(
            np.ascontiguousarray(validation_logits).tobytes(order="C")
        ).hexdigest()
        selection_evidence["calibrator_onnx_sha256"] = calibration_input_stream
