...
"""Freeze V19 train, visible validation, and truth-hidden public fixture bytes."""
...
from .protocol import (
    ...
    LICENSE_SHA256, NOTICE_PATH, NOTICE_SHA256, PUBLIC_REVISION, RECOGNIZER_PATH,
...
def freeze_split() -> dict[str, str]:
    ...
    if existing:
        raise RuntimeError("OCR V19 split freeze refuses overwrite: " + ", ".join(existing))
    protocol_path = REPO_ROOT / PROTOCOL_PATH
    if protocol_path.read_bytes() != canonical_json_bytes(protocol_configuration()):
        raise RuntimeError("OCR V19 committed protocol is not canonical")
    exact_files = {
    ...
    for relative, expected in exact_files.items():
        if sha256_file(REPO_ROOT / relative) != expected:
            raise RuntimeError(f"OCR V19 preregistered input changed: {relative}")
    ...
    selection = {
        "schema": "graphreader.ocr-proposal-confirmation-calibrator-selection.v1",
    ...
    seal = {
        "schema": "graphreader.ocr-proposal-confirmation-calibrator-sealed-test-seal.v1",
    ...
    gate = {
        "schema": "graphreader.ocr-proposal-confirmation-calibrator-gate-config.v1",
    ...
    expected_steps = ((train_count + 255) // 256) * 20
    config = {
        ...
        "seed": SEED, "epochs": 20, "batch_size": 256, "learning_rate": 0.001,
        "weight_decay": 0.0001, "negative_class_weight": 2.0,
        ...
        "v18_aggregate_metrics_only_used_for_design": True,
