...
"""Single-use truth-hidden public gate for OCR V19."""
...
from .pipeline import evaluate_thresholds, extract_features, metrics_pass
from .protocol import (
    DETECTOR_PATH, DETECTOR_SHA256, PUBLIC_REVISION, RECOGNIZER_PATH, RECOGNIZER_SHA256,
    RECOGNIZER_YAML_PATH, REVISION, TASK,
)
...
GATE_CONFIG = {
    ...
    "detector_recognizer_and_calibrator_tensor_stream_hashes_required": True,
...
def _cpu_session(path: Path) -> ort.InferenceSession:
    ...
    if session.get_providers() != ["CPUExecutionProvider"]:
        raise RuntimeError("OCR V19 public gate requires CPUExecutionProvider only")
    return session


def evaluate_public(*, selection_report_path: Path, output_path: Path) -> dict[str, object]:
    if output_path.exists():
        raise RuntimeError(f"OCR V19 public output exists: {output_path}")
    ...
    if (
        ...
        raise RuntimeError("OCR V19 public gate is not authorized by the canonical ledger")
    if (
        ...
        raise RuntimeError("Only the exact selected OCR V19 calibrator may open the public gate")
    calibrator_path = REPO_ROOT / selection["onnx_path"]
    if sha256_file(calibrator_path) != selection["onnx_sha256"]:
        raise RuntimeError("OCR V19 selected calibrator payload changed")
    ...
    if (
        ...
        raise RuntimeError("OCR V19 public inputs changed")
    gate = acquire_gate_seal(
        repo_root=REPO_ROOT, task=TASK, revision=PUBLIC_REVISION,
        candidate_hashes={
            "detector_onnx_sha256": DETECTOR_SHA256, "recognizer_onnx_sha256": RECOGNIZER_SHA256,
            "calibrator_onnx_sha256": selection["onnx_sha256"], "selection_report_sha256": selection_sha,
        },
    ...
    if split_fingerprint(scenes) != seal["split_fingerprint"] or any(summary[key] != seal[key] for key in summary):
        raise RuntimeError("OCR V19 public fixtures violate the frozen split")
    detector_session = _cpu_session(REPO_ROOT / DETECTOR_PATH)
    recognizer_session = _cpu_session(REPO_ROOT / RECOGNIZER_PATH)
    calibrator_session = _cpu_session(calibrator_path)
    ...
    def detector_runner(values: np.ndarray) -> np.ndarray:
        return np.asarray(detector_session.run(None, {detector_input: np.ascontiguousarray(values)})[0], dtype=np.float32)

    def recognizer_runner(values: np.ndarray) -> np.ndarray:
        return np.asarray(recognizer_session.run(None, {recognizer_input: np.ascontiguousarray(values)})[0], dtype=np.float32)

    features, _, records, evidence = extract_features(
        scenes, detector_runner, recognizer_runner, alphabet, mode="evaluate",
    )
    logits = np.asarray(calibrator_session.run(None, {calibrator_input: np.ascontiguousarray(features)})[0], dtype=np.float32)
    import hashlib
    evidence["calibrator_input_tensor_stream_sha256"] = hashlib.sha256(np.ascontiguousarray(features).tobytes(order="C")).hexdigest()
    evidence["calibrator_output_tensor_stream_sha256"] = hashlib.sha256(np.ascontiguousarray(logits).tobytes(order="C")).hexdigest()
    metrics = evaluate_thresholds(
        scenes, records, logits, (float(selection["selected_threshold"]),), evidence,
    )[0]["metrics"]
    passed = metrics_pass(metrics)
    report: dict[str, object] = {
        "schema": "graphreader.ocr-proposal-confirmation-calibrator-public-gate.v1",
        "task": TASK, "revision": PUBLIC_REVISION, "status": "pass" if passed else "fail",
        "evaluation_count": 1, "detector_sha256": DETECTOR_SHA256,
        "recognizer_sha256": RECOGNIZER_SHA256, "calibrator_sha256": selection["onnx_sha256"],
        "selection_report_sha256": selection_sha, "selected_threshold": selection["selected_threshold"],
        "fixture_archive_sha256": seal["fixture_archive_sha256"], "provider": "CPUExecutionProvider",
        "metrics": metrics, "marker_creation_evaluated": False,
        ...
        "case_level_details_emitted": False, "production_approval": False, "release_eligible": False,
