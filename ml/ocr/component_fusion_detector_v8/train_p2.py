...
"""Single-use P2 export repair and training for OCR component-fusion V8."""
...
from .model_p2 import ComponentFusionP2Net
from .pipeline import evaluate_scenes
from .protocol import REVISION, TASK
from .train_p1 import _balanced_order, _configure, _export, _runner


REPO_ROOT = Path(__file__).resolve().parents[3]
CANDIDATE_ID = "P2"
CONFIG_PATH = Path("ml/ocr/component_fusion_detector_v8/training/p2.json")
CANONICAL_OUTPUT = Path("ml/ocr/component_fusion_detector_v8/artifacts/P2-run")
RUNNER_SOURCE_PATHS = (
    Path("ml/ocr/component_fusion_detector_v8/dataset.py"),
    Path("ml/ocr/component_fusion_detector_v8/model_p2.py"),
    ...
    Path("ml/ocr/component_fusion_detector_v8/train_p2.py"),
...
def train_candidate(output_dir: Path) -> dict[str, object]:
    ...
    try:
        ...
        model = ComponentFusionP2Net(seed=seed).eval()
        ...
        if preflight_output.shape != (3, 2) or not np.isfinite(preflight_output).all():
            raise RuntimeError("OCR V8 P2 export preflight returned an invalid tensor")
        ...
        phase = "export"
        checkpoint_path = output_dir / "graph-text-component-fusion-v8-p2.pt"
        torch.save({"state_dict": model.state_dict(), "selected_threshold": selected["threshold"]}, checkpoint_path)
        onnx_path = output_dir / "graph-text-component-fusion-v8-p2.onnx"
...
def main() -> int:
    ...
    print(json.dumps({"candidate_id": report["candidate_id"], "status": report["status"], "selected_threshold": report["selected_threshold"], "selection_gate_passed": report["selection_gate_passed"], "onnx_parity_passed": report["onnx_parity_passed"]}, indent=2, sort_keys=True))
