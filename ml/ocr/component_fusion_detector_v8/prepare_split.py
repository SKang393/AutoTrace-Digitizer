...
"""Freeze OCR V8 selection metadata and a truth-hidden public archive."""
...
DEFAULT_PRIVATE_ROOT = Path("ml/ocr/component_fusion_detector_v8/artifacts/split-freeze")
DEFAULT_PROTOCOL = Path("ml/ocr/component_fusion_detector_v8/PROTOCOL.json")
DEFAULT_SELECTION = Path("ml/ocr/component_fusion_detector_v8/SELECTION_MANIFEST.json")
DEFAULT_SEAL = Path("ml/ocr/component_fusion_detector_v8/SEALED_PUBLIC_TEST_SEAL.json")
DEFAULT_GATE = Path("ml/ocr/component_fusion_detector_v8/gates/sealed-public-v1.json")
SPLIT_SOURCE_PATHS = (
    Path("ml/ocr/component_fusion_detector_v8/dataset.py"),
    Path("ml/ocr/component_fusion_detector_v8/prepare_split.py"),
...
def freeze_split(*, private_root: Path, protocol_path: Path, selection_path: Path, seal_path: Path, gate_path: Path) -> dict[str, str]:
    ...
    if existing:
        raise RuntimeError("OCR V8 split freeze refuses to overwrite evidence: " + ", ".join(existing))
    ...
    selection = {
        "schema": "graphreader.ocr-component-fusion-selection.v1",
        "task": TASK,
        "revision": REVISION,
    ...
    seal = {
        "schema": "graphreader.ocr-component-fusion-sealed-test-seal.v1",
        "task": TASK,
        "revision": REVISION,
    ...
    gate = {
        "schema": "graphreader.ocr-component-fusion-gate-config.v1",
        "task": TASK,
