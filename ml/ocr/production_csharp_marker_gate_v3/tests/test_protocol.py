...
from ml.ocr.production_csharp_marker_gate_v3.dataset import build_scene
...
from ml.ocr.composite_proposal_role_v11.protocol import SPLITS as V11_SPLITS
from ml.ocr.composite_proposal_role_v11.dataset import box_iou, proposals
...
def test_protocol_is_frozen_fail_closed_and_private_free() -> None:
    ...
    tracked = json.loads(
        (REPO_ROOT / "ml/ocr/production_csharp_marker_gate_v3/PROTOCOL.json").read_text(encoding="utf-8")
    )
...
def test_exact_v11_models_and_direct_evidence_requirements_are_frozen() -> None:
    protocol = protocol_configuration()
...
def test_composite_families_are_disjoint_from_v2_and_v11() -> None:
    protocol = protocol_configuration()
    v11_renderers = {item.renderer_family for item in V11_SPLITS}
    v11_degradations = {item.degradation_family for item in V11_SPLITS}
    assert len(RENDERER_FAMILIES) == 6 and len(set(RENDERER_FAMILIES)) == 6
    assert len(DEGRADATION_FAMILIES) == 4 and len(set(DEGRADATION_FAMILIES)) == 4
    assert set(RENDERER_FAMILIES).isdisjoint(set(V2_RENDERER_FAMILIES) | v11_renderers)
    assert set(DEGRADATION_FAMILIES).isdisjoint(set(V2_DEGRADATION_FAMILIES) | v11_degradations)
    assert protocol["split"]["v1_v2_or_v11_fixture_bytes_reused"] is False
...
def test_scene_has_all_eight_roles_and_real_marker_structure_truth() -> None:
    scene = build_scene(0x9BD5A0173C42EF11, 0)
    assert scene.scene_id.startswith("ocr-marker-csharp-v3-public-")
    ...
    assert len(scene.text_truths) == 8
    ...
    assert {item.family for item in scene.text_truths} >= {"numeric", "word", "ambiguity"}
...
def test_preregistration_has_no_split_or_execution_result() -> None:
    root = REPO_ROOT / "ml/ocr/production_csharp_marker_gate_v3"
    seal_exists = (root / "SEALED_PUBLIC_TEST_SEAL.json").exists()
    if not seal_exists:
        assert not (REPO_ROOT / "artifacts/production-validation/ocr-marker-csharp-v3-sealed.zip").exists()
...
def test_future_split_seal_is_hash_only_fail_closed_and_has_no_secret() -> None:
    ...
    assert seal["text_truth_count"] == SCENE_COUNT * 8
