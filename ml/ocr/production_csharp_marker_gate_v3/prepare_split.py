...
"""Generate and seal the disjoint V3 archive without executing any model."""
...
SOURCE_PATHS = (
    ...
    Path("ml/ocr/production_csharp_marker_gate_v3/protocol.py"),
    Path("ml/ocr/production_csharp_marker_gate_v3/dataset.py"),
    Path("ml/ocr/production_csharp_marker_gate_v3/prepare_split.py"),
    Path("ml/ocr/production_csharp_marker_gate_v3/PROTOCOL.json"),
    ...
    Path("src/GraphReader.Ocr/OcrV11CandidateCompositionFactory.cs"),
    Path("src/GraphReader.Ocr/LocalOnnxProposalTextRegionDetector.cs"),
    Path("src/GraphReader.Markers/Detection/NormalizedMarkerProposalDetector.cs"),
    Path("tests/GraphReader.Integration.Tests/IntegrationSmoke/OcrMarkerDirectPublicGateV3Tests.cs"),
)
...
def main() -> int:
    ...
    if protocol_configuration()["state"] != "frozen_before_sealed_public_identity_materialization_or_model_execution":
    ...
    source_hashes = {path.as_posix(): sha256_file(REPO_ROOT / path) for path in SOURCE_PATHS}
    seal = {
        **summary,
