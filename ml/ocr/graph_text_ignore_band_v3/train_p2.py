...
"""Single-use P2 training for whole-frame tile composition."""
...
from .protocol import BATCH_SIZE, ONNX_PARITY_MAXIMUM_ABSOLUTE_ERROR, REVISION, SEED, TASK, WEIGHT_DECAY
from .train_p1 import _configure, _export, _loss, _selection_passed, evaluate_frames, normalize_bgr


REPO_ROOT = Path(__file__).resolve().parents[3]
CANDIDATE_ID = "P2"
CONFIG_PATH = Path("ml/ocr/graph_text_ignore_band_v3/training/p2.json")
CANONICAL_OUTPUT = Path("ml/ocr/graph_text_ignore_band_v3/artifacts/P2-run")
RUNNER_SOURCE_PATHS = (
    ...
    Path("ml/ocr/graph_text_ignore_band_v3/train_p2.py"),
...
def train_candidate(output_dir: Path) -> dict[str, object]:
    ...
    try:
        if sha256_file(REPO_ROOT / config["trigger_result_path"]) != config["trigger_result_sha256"]:
            raise RuntimeError("P2 trigger result changed")
        if sha256_file(REPO_ROOT / config["p1_report_path"]) != config["p1_report_sha256"]:
            raise RuntimeError("P2 P1 report evidence changed")
        ...
        if not np.isfinite(preflight_output).all() or float(preflight_output.min()) < 0.0 or float(preflight_output.max()) > 1.0:
            raise RuntimeError("P2 ONNX preflight violated the probability contract")
        ...
        if optimizer_steps != int(config["expected_optimizer_steps"]):
            raise RuntimeError("P2 optimizer-step count differs from preregistration")
        phase = "export"
        model.eval()
        checkpoint_path = output_dir / "graph-text-ignore-band-v3-p2.pt"
        torch.save({"state_dict": model.state_dict(), "revision": REVISION, "candidate_id": CANDIDATE_ID}, checkpoint_path)
        onnx_path = output_dir / "graph-text-ignore-band-v3-p2.onnx"
