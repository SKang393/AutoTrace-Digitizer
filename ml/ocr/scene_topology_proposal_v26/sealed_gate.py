...
"""Single-use truth-hidden public gate for a selected OCR V26 candidate."""
...
PUBLIC_REVISION = "graph-text-scene-topology-proposal-v26-public-v1"
...
GATE_CONFIG = {
    ...
    "detector_recognizer_parent_and_candidate_tensor_stream_hashes_required": True,
...
def _cpu_session(path: Path) -> ort.InferenceSession:
    ...
    if session.get_providers() != ["CPUExecutionProvider"]:
        raise RuntimeError("OCR V26 public gate requires CPUExecutionProvider only")
    return session
...
def _public_window(selection: dict[str, object]) -> tuple[float, ...]:
    ...
    if (
        ...
        raise RuntimeError("OCR V26 selection has no preregistered robust threshold window")
...
def evaluate_public(*, selection_report_path: Path, output_path: Path) -> dict[str, object]:
    ...
    if output_path.exists():
        raise RuntimeError(f"OCR V26 public output exists: {output_path}")
    ...
    if (
        ...
        raise RuntimeError("OCR V26 public gate is not authorized by the canonical ledger")
    if (
        ...
        or selection.get("parent_roles_preserved") is not True
        ...
        raise RuntimeError("Only the exact robustly selected OCR V26 candidate may open the public gate")
    ...
    if sha256_file(split_seal_path) != config["split_seal_sha256"]:
        raise RuntimeError("OCR V26 split seal changed")
    ...
    if (
        ...
        or sha256_file(REPO_ROOT / ROLE_PARENT_ONNX_PATH) != ROLE_PARENT_ONNX_SHA256
    ):
        raise RuntimeError("OCR V26 public inputs changed")
    ...
    if (
        ...
        raise RuntimeError("OCR V26 public fixtures violate the frozen contract")
    detector_session = _cpu_session(REPO_ROOT / DETECTOR_PATH)
    recognizer_session = _cpu_session(REPO_ROOT / RECOGNIZER_PATH)
    parent_session = _cpu_session(REPO_ROOT / ROLE_PARENT_ONNX_PATH)
    candidate_session = _cpu_session(candidate_path)
    ...
    if (
        ...
        raise RuntimeError("OCR V26 public production proposal stream is invalid")
    ...
    for indices in groups:
        ...
        parent_output = np.asarray(parent_session.run(None, inputs)[0], dtype=np.float32)
        candidate_output = np.asarray(candidate_session.run(None, inputs)[0], dtype=np.float32)
        if parent_output.shape != candidate_output.shape or candidate_output.shape[2] != 10:
            raise RuntimeError("OCR V26 public candidate output contract changed")
        parent_role_error = max(
            parent_role_error,
            float(np.max(np.abs(candidate_output[:, :, 2:] - parent_output[:, :, 2:]))),
        )
        ...
        candidate_outputs.update(np.ascontiguousarray(candidate_output).tobytes(order="C"))
    if len(groups) != len(scenes) or parent_role_error != 0.0:
        raise RuntimeError("OCR V26 public candidate did not preserve exact parent roles")
    ...
    runtime_evidence.update({
        ...
        "parent_role_maximum_absolute_error": parent_role_error,
        "candidate_onnx_sha256": candidate_sha,
    })
    ...
    report: dict[str, object] = {
        "schema": "graphreader.ocr-scene-topology-public-gate.v1",
        "task": TASK,
        "revision": PUBLIC_REVISION,
        ...
        "role_parent_sha256": ROLE_PARENT_ONNX_SHA256,
