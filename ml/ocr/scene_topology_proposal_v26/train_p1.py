...
"""Single-use axial topology proposal training for OCR V26 P1."""
...
from ml.ocr.evidence_rescue_v25.train_p1 import _load_parent_state
...
def _validate_stored_split(
    ...
    if (
        ...
        raise RuntimeError(f"OCR V26 {name} stored fixtures violate the seal")
...
def _proposal_objective(
    ...
    if not torch.any(positive) or not torch.any(negative):
        raise RuntimeError("OCR V26 requires positive and negative proposals per scene")
    positive_floor = torch.relu(
        float(config["positive_logit_margin_floor"]) - margin[positive]
    ).max()
    negative_ceiling = torch.relu(
        margin[negative] - float(config["negative_logit_margin_ceiling"])
    ).max()
    separation = torch.relu(
        margin[negative].max() - margin[positive].min()
...
def preflight() -> dict[str, Any]:
    ...
    expected = {
        ...
        "candidate_limit": 3,
        "architecture": "frozen-role-axial-topology-proposal-v1",
        "objective": "complete-proposal-scene-extrema-margin-v1",
    ...
    for key, value in expected.items():
        if config.get(key) != value:
            raise RuntimeError(f"OCR V26 P1 config field mismatch: {key}")
    if config.get("expected_runner_source_bundle_sha256") != source_bundle_sha256(
        REPO_ROOT, RUNNER_SOURCE_PATHS,
    ):
        raise RuntimeError("OCR V26 P1 runner source bundle changed")
    if sha256_file(REPO_ROOT / SEAL_PATH) != config.get("split_seal_sha256"):
        raise RuntimeError("OCR V26 split seal changed before P1")
    ...
    for key, value in required_seal.items():
        if seal.get(key) != value:
            raise RuntimeError(f"OCR V26 split seal field changed: {key}")
    ...
    for split, (path, key) in archive_bindings.items():
        ...
        if actual != seal["splits"][split]["archive_sha256"] or actual != config.get(key):
            raise RuntimeError(f"OCR V26 {split} archive changed before P1")
    exact_inputs = {
    ...
    for relative, expected_hash in exact_inputs.items():
        if sha256_file(REPO_ROOT / relative) != expected_hash:
            raise RuntimeError(f"OCR V26 frozen input changed: {relative}")
    trigger = _read_json(REPO_ROOT / TRIGGER_RESULT_PATH)
    metrics = trigger.get("selection_metrics", {})
    if (
        trigger.get("status") != "failed_selection"
        or trigger.get("candidate_consumed") is not True
        or trigger.get("case_level_details_emitted") is not False
        or metrics.get("exact_scene_count") != 112
        or metrics.get("true_positives") != 1017
        or metrics.get("false_positives") != 1
        or metrics.get("false_negatives") != 7
        ...
        or trigger.get("public_gate_evaluations") != 0
        or "cases" in trigger
        or "predictions" in trigger
    ):
        raise RuntimeError("OCR V26 aggregate-only V25 trigger changed")
    if (REPO_ROOT / CANONICAL_OUTPUT).exists():
        raise RuntimeError("OCR V26 P1 output already exists")
    ...
    if (
        ...
        raise RuntimeError("OCR V26 P1 canonical authorization changed")
    return {"config": config, "head": head, "seal": seal}


def train_candidate(output_dir: Path) -> dict[str, object]:
    if output_dir.exists():
        raise RuntimeError(f"OCR V26 P1 output exists: {output_dir}")
    evidence = preflight()
    ...
    try:
        ...
        if train_values.shape[1:] != (FEATURE_COUNT,):
            raise RuntimeError("OCR V26 training evidence width changed")
        if train_crops.shape[1:] != (CROP_CHANNELS, CROP_HEIGHT, CROP_WIDTH):
            raise RuntimeError("OCR V26 training crop shape changed")
        ...
        for key in ("proposal_count", "positive_proposal_count", "negative_proposal_count"):
            if training_evidence[key] != registered[key]:
                raise RuntimeError(f"OCR V26 incomplete training stream: {key}")

        phase = "axial_topology_proposal_training"
        generator = _configure(int(config["seed"]))
        model = FrozenRoleAxialTopologyProposalNet(
            seed=int(config["seed"]),
            dropout_probability=float(config["dropout_probability"]),
        )
        model.load_role_parent_state_dict(_load_parent_state())
        ...
        if not trainable or any(name.startswith("role_parent.") for name in trainable_names):
            raise RuntimeError("OCR V26 frozen role-parent boundary changed")
        ...
        for epoch in range(int(config["epochs"])):
            ...
            print(
                f"OCR V26 P1 epoch {epoch + 1}/{config['epochs']} complete; "
                f"optimizer_steps={optimizer_steps}",
                flush=True,
            )
        if optimizer_steps != int(config["expected_optimizer_steps"]):
            raise RuntimeError("OCR V26 optimizer-step count changed")
        frozen_after = _parameter_stream_sha256(model.role_parent)
        if frozen_after != frozen_before:
            raise RuntimeError("OCR V26 modified the frozen role parent")

        phase = "checkpoint_and_onnx_export"
        checkpoint_path = output_dir / "graph-text-scene-topology-proposal-v26-p1.pt"
        torch.save({"state_dict": model.state_dict()}, checkpoint_path)
        onnx_path = output_dir / "graph-text-scene-topology-proposal-v26-p1.onnx"
        ...
        if {item.name for item in candidate_session.get_inputs()} != {
            ...
            raise RuntimeError("OCR V26 ONNX input identity changed")
        ...
        for key in ("proposal_count", "positive_proposal_count", "negative_proposal_count"):
            if selection_evidence[key] != registered[key]:
                raise RuntimeError(f"OCR V26 incomplete selection stream: {key}")
        ...
        report: dict[str, object] = {
            ...
            "frozen_role_parent_preserved": frozen_preserved,
            "role_parent_checkpoint_sha256": ROLE_PARENT_CHECKPOINT_SHA256,
            "parent_role_maximum_absolute_error": role_error,
