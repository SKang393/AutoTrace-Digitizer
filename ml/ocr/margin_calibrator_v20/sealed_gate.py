...
"""Single-use truth-hidden public gate for OCR V20."""
...
def _cpu_session(path: Path) -> ort.InferenceSession:
    ...
    if session.get_providers() != ["CPUExecutionProvider"]:
        raise RuntimeError("OCR V20 public gate requires CPUExecutionProvider only")
    return session


def evaluate_public(*, selection_report_path: Path, output_path: Path) -> dict[str, object]:
    if output_path.exists():
        raise RuntimeError(f"OCR V20 public output exists: {output_path}")
    ...
    if (
        ...
        raise RuntimeError("OCR V20 public gate is not authorized by the canonical ledger")
    if (
        ...
        raise RuntimeError("Only the exact selected OCR V20 calibrator may open the public gate")
    calibrator_path = REPO_ROOT / selection["onnx_path"]
    if sha256_file(calibrator_path) != selection["onnx_sha256"]:
        raise RuntimeError("OCR V20 selected calibrator payload changed")
    ...
    if (
        ...
        raise RuntimeError("OCR V20 public inputs changed")
    ...
    if split_fingerprint(scenes) != seal["split_fingerprint"] or any(summary[key] != seal[key] for key in summary):
        raise RuntimeError("OCR V20 public fixtures violate the frozen split")
    ...
    report: dict[str, object] = {
        "schema": "graphreader.ocr-margin-calibrator-public-gate.v1",
