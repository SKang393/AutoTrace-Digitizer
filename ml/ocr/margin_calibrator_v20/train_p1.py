...
"""Single-use P1 training and visible selection for OCR V20."""
...
from .protocol import (
    DETECTOR_PATH, DETECTOR_SHA256, MARGIN_LOSS_WEIGHT, NEGATIVE_LOGIT_MARGIN,
    ONNX_PARITY_MAXIMUM_ABSOLUTE_ERROR, POSITIVE_LOGIT_MARGIN,
    RECOGNIZER_PATH, RECOGNIZER_SHA256, RECOGNIZER_YAML_PATH, RECOGNIZER_YAML_SHA256,
    REVISION, TASK, TRIGGER_RESULT_PATH, TRIGGER_RESULT_SHA256,
)
...
RUNNER_SOURCE_PATHS = (
    ...
    ROOT / "train_p1.py", Path("ml/ocr/official_bakeoff/production_evaluate.py"),
...
def _cpu_session(path: Path) -> ort.InferenceSession:
    ...
    if session.get_providers() != ["CPUExecutionProvider"]:
        raise RuntimeError("OCR V20 requires CPUExecutionProvider only")
    return session
...
def train_candidate(output_dir: Path) -> dict[str, object]:
    if output_dir.exists():
        raise RuntimeError(f"OCR V20 P1 output exists: {output_dir}")
    ...
    try:
        if config["expected_runner_source_bundle_sha256"] != source_bundle_sha256(REPO_ROOT, RUNNER_SOURCE_PATHS):
            raise RuntimeError("OCR V20 P1 runner sources changed")
        ...
        if (
            config["trigger_result_path"] != TRIGGER_RESULT_PATH
            or config["trigger_result_sha256"] != TRIGGER_RESULT_SHA256
            or sha256_file(REPO_ROOT / TRIGGER_RESULT_PATH) != TRIGGER_RESULT_SHA256
        ):
            raise RuntimeError("OCR V20 aggregate V19 trigger result changed")
        for relative, expected in {
            DETECTOR_PATH: DETECTOR_SHA256, RECOGNIZER_PATH: RECOGNIZER_SHA256,
            RECOGNIZER_YAML_PATH: RECOGNIZER_YAML_SHA256,
        }.items():
            if sha256_file(REPO_ROOT / relative) != expected:
                raise RuntimeError(f"OCR V20 exact frozen input changed: {relative}")
        ...
        if any(training_evidence[key] != config[key] for key in (
            ...
            raise RuntimeError("OCR V20 training sample counts changed")
        generator = _configure(int(config["seed"]))
        model = MarginSeparatedProposalCalibrator(seed=int(config["seed"]))
        ...
        for epoch in range(int(config["epochs"])):
            ...
            for start in range(0, len(order), int(config["batch_size"])):
                ...
                batch_logits = model(tensors.index_select(0, indices))
                differences = batch_logits[:, 1] - batch_logits[:, 0]
                positive = differences[batch_targets == 1]
                negative = differences[batch_targets == 0]
                positive_loss = (
                    torch.relu(positive.new_tensor(POSITIVE_LOGIT_MARGIN) - positive).square().mean()
                    if positive.numel() else differences.new_zeros(())
                )
                negative_loss = (
                    torch.relu(negative - negative.new_tensor(NEGATIVE_LOGIT_MARGIN)).square().mean()
                    if negative.numel() else differences.new_zeros(())
                )
                loss = criterion(batch_logits, batch_targets) + MARGIN_LOSS_WEIGHT * (
        ...
        if optimizer_steps != int(config["expected_optimizer_steps"]):
            raise RuntimeError("OCR V20 P1 optimizer-step count changed")

        phase = "export"
        checkpoint_path = output_dir / "graph-text-margin-calibrator-v20-p1.pt"
        torch.save({"state_dict": model.state_dict()}, checkpoint_path)
        onnx_path = output_dir / "graph-text-margin-calibrator-v20-p1.onnx"
