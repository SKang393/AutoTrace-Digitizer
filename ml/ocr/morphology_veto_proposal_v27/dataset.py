...
"""Fresh stored procedural scenes for OCR morphology-veto V27."""
...
def _apply_degradation(raster: np.ndarray, split: Split, index: int) -> np.ndarray:
    """Apply a V27-only photometric transform without moving truth geometry."""
    ...
    if split == "train":
        local = np.sin((x + 1.0) * np.pi * float(2 + index % 5))
        values += local * float(1 + index % 3)
        values[:, (index * 5) % 9::9] += 1.0
        if index % 2 == 0:
            padded = np.pad(values, ((0, 0), (1, 1)), mode="edge")
            values = (padded[:, :-2] + 6.0 * padded[:, 1:-1] + padded[:, 2:]) / 8.0
        quantum = float(2 + index % 4)
        values = np.rint(values / quantum) * quantum
    elif split == "validation":
        values += (x - y) * float(1 + index % 4)
        values[index % 4::4, :] += 1.0
        halo = np.maximum(0.0, 1.0 - np.abs(x) * 1.75)
        values -= halo * float(index % 3)
        values[:, 1::2] += 0.5
    else:
        gamma = 0.975 + 0.005 * float(index % 8)
        values = 255.0 * np.power(np.clip(values / 255.0, 0.0, 1.0), gamma)
        values += (x * 2.0 + y) * float(1 + index % 3)
        band = 13 + (index * 31) % max(14, height - 14)
        values[max(0, band - 1):band + 1] += 1.25
        values[(index * 7) % 11::11, :] -= 0.75
...
def render_scene(split: Split, index: int) -> SceneSample:
    ...
    if index < 0 or index >= registered.scene_count:
        raise IndexError(f"OCR V27 scene index outside frozen {split} split: {index}")
    for attempt in range(64):
        source_index = (index + attempt * 97) % registered.scene_count
        with _registration_scope():
            source = v24.render_scene(split, source_index)
        candidate = SceneSample(
            f"morphology-veto-v27-{split}-{index:05d}",
            split,
        ...
        return candidate
    raise RuntimeError(
        f"OCR V27 could not render one production proposal per truth: {split}/{index}"
    )
...
def save_archive(scenes: tuple[SceneSample, ...], path: Path) -> dict[str, object]:
    if path.exists():
        raise FileExistsError(f"OCR V27 fixture identity already exists: {path}")
    if not scenes or len({scene.split for scene in scenes}) != 1:
        raise RuntimeError("OCR V27 archive requires one nonempty split")
    ...
    manifest: dict[str, object] = {
        "schema": "graphreader.ocr-morphology-veto-fixtures.v1",
        "revision": REVISION,
...
def load_archive(path: Path) -> tuple[SceneSample, ...]:
    with ZipFile(path, "r") as archive:
        ...
        if (
            manifest.get("schema") != "graphreader.ocr-morphology-veto-fixtures.v1"
            ...
            raise RuntimeError("OCR V27 stored fixture contract changed")
        scenes: list[SceneSample] = []
        for case in manifest["cases"]:
            payload = archive.read(case["source_path"])
            if sha256(payload).hexdigest() != case["source_sha256"]:
                raise RuntimeError(f"OCR V27 fixture hash mismatch: {case['scene_id']}")
            raster = np.asarray(
    ...
    if len(result) != registered.scene_count:
        raise RuntimeError("OCR V27 stored fixture scene count changed")
