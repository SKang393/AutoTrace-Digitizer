...
"""One-time fixture freeze for OCR morphology-veto proposal V27."""
...
ARCHIVE_PATHS = {
    "train": Path("artifacts/production-validation/ocr-v27-train.zip"),
    "validation": Path("artifacts/production-validation/ocr-v27-selection.zip"),
    "sealed_public": Path("artifacts/production-validation/ocr-v27-public.zip"),
}
SOURCE_PATHS = (
    ROOT / "PROTOCOL.json",
    ROOT / "dataset.py",
...
def _require_sources_at_head() -> None:
    for path in SOURCE_PATHS:
        ...
        if not local.is_file():
            raise RuntimeError(f"OCR V27 required source is missing: {relative}")
        completed = subprocess.run(
        ...
        if completed.returncode != 0 or completed.stdout != local.read_bytes():
            raise RuntimeError(
                f"OCR V27 source must be committed unchanged before freeze: {relative}"
            )
    expected = {
    ...
    for relative, digest in expected.items():
        ...
        if not path.is_file() or sha256_file(path) != digest:
            raise RuntimeError(f"OCR V27 exact prerequisite changed: {relative}")
...
def freeze() -> dict[str, object]:
    if (REPO_ROOT / SEAL_PATH).exists():
        raise RuntimeError("OCR V27 split identity is already frozen")
    existing = [
    ...
    if existing:
        raise RuntimeError(
            "OCR V27 archive already exists before freeze: " + ", ".join(existing)
        )
    ...
    for split, relative in ARCHIVE_PATHS.items():
        ...
        if len(hashes) != len(scenes):
            raise RuntimeError(f"OCR V27 {split} contains duplicate fixture bytes")
    ...
    if any(overlaps.values()):
        raise RuntimeError("OCR V27 split fixture bytes overlap")
