...
"""Materialize the P2 visible-selection identity once without model execution."""
...
SOURCE_PATHS = (
    ...
    Path("ml/ocr/selected_confidence_acceptance_v9_p2/protocol.py"),
    Path("ml/ocr/selected_confidence_acceptance_v9_p2/dataset.py"),
    Path("ml/ocr/selected_confidence_acceptance_v9_p2/prepare_split.py"),
    Path("ml/ocr/selected_confidence_acceptance_v9_p2/PROTOCOL.json"),
    Path("src/GraphReader.Ocr/OcrV9P2CandidateCompositionPipeline.cs"),
    Path("tests/GraphReader.Ocr.Tests/OcrV9P2CandidateSelectionTests.cs"),
)
...
def main() -> int:
    ...
    if output.exists() or seal_output.exists():
        raise RuntimeError("V9 P2 selection archive or seal already exists")
    if configuration()["state"] != (
        "p2_preregistered_before_selection_identity_materialization_or_model_execution"
    ):
        raise RuntimeError("V9 P2 protocol state changed")
    secret_seed = secrets.randbits(128)
    summary = save_selection_archive(build_selection_split(secret_seed), output)
    seal = {
        **summary,
        "schema": "graphreader.ocr-selected-confidence-selection-seal.v1",
        "candidate_id": "P2",
