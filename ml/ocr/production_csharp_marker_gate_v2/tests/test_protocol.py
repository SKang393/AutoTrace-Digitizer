...
from ml.ocr.production_csharp_marker_gate_v2.dataset import build_scene
from ml.ocr.production_csharp_marker_gate_v1.protocol import (
    DEGRADATION_FAMILIES as V1_DEGRADATION_FAMILIES,
    RENDERER_FAMILIES as V1_RENDERER_FAMILIES,
)
...
def test_protocol_is_frozen_fail_closed_and_private_free() -> None:
    ...
    tracked = json.loads(
        (REPO_ROOT / "ml/ocr/production_csharp_marker_gate_v2/PROTOCOL.json").read_text(encoding="utf-8")
    )
...
def test_new_composite_families_and_fixture_mask_limit_are_explicit() -> None:
    ...
    assert set(RENDERER_FAMILIES).isdisjoint(V1_RENDERER_FAMILIES)
    assert set(DEGRADATION_FAMILIES).isdisjoint(V1_DEGRADATION_FAMILIES)
    ...
    assert protocol["composition"]["marker_runtime"].endswith(
        "frozen_1e-5_activation_boundary_tolerance"
    )
...
def test_tracked_split_seal_is_hash_only_fail_closed_and_has_no_secret() -> None:
    seal_path = REPO_ROOT / "ml/ocr/production_csharp_marker_gate_v2/SEALED_PUBLIC_TEST_SEAL.json"
