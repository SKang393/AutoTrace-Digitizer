...
"""Exact V10 plus two-band tick consensus and explicit ambiguity segmentation."""
...
from .protocol import (
    CONSENSUS_RESCUE_SCORE_MINIMUM, DETECTOR_THRESHOLD, NUMERIC_THRESHOLD,
    OFFICIAL_RESCUE_SCORE_MINIMUM, TRUTH_MATCH_IOU_MINIMUM,
)
...
def _force_expected_ambiguity_groups(crop: Any, expected_count: int) -> tuple[tuple[int, int, int, int], ...]:
    groups = active_groups(crop)
    if len(groups) == expected_count:
        return groups
    gray = np.asarray(crop.convert("L"), dtype=np.float32)
    if gray.ndim != 2 or expected_count < 2:
        return groups
    edge = np.concatenate((gray[0], gray[-1], gray[:, 0], gray[:, -1]))
    background = float(np.median(edge))
    contrast = max(0.0, background - float(np.percentile(gray, 1)))
    ...
    gaps = sorted(
        ((int(current - prior - 1), int(prior), int(current))
         for prior, current in zip(active[:-1], active[1:], strict=True) if current - prior > 1),
        reverse=True,
    )
    minimum_gap = max(1, int(np.ceil((bottom - top + 1) * 0.25)))
    selected = sorted((prior, current) for gap, prior, current in gaps[:expected_count - 1] if gap >= minimum_gap)
    if len(selected) != expected_count - 1:
        return groups
    starts = [int(left), *(current for _, current in selected)]
    ends = [*(prior + 1 for prior, _ in selected), int(right) + 1]
    result = []
    for group_left, group_right in zip(starts, ends, strict=True):
        columns = foreground[:, group_left:group_right]
        rows = np.where(columns.any(axis=1))[0]
        if not len(rows):
            return groups
        result.append((group_left, int(rows[0]), group_right, int(rows[-1]) + 1))
    return tuple(result)
...
def _official_recognize(gray: np.ndarray, box: Any, official: DirectRunner,
    ...
    groups = active_groups(crop)
    if nonspace and all(character in AMBIGUITY_GLYPHS for character in nonspace):
    ...
    for scene in scenes:
        ...
        for candidate_index, (candidate, probability) in enumerate(zip(candidates, probabilities, strict=True)):
            if probability >= DETECTOR_THRESHOLD:
                ...
                continue
            if probability < CONSENSUS_RESCUE_SCORE_MINIMUM:
                continue
