...
from ml.ocr.production_composition_v6.dataset import build_split, proposal_summary, split_fingerprint
from ml.ocr.production_composition_v6.protocol import (
    AMBIGUITY_PUBLIC_REPORT_SHA256, AMBIGUITY_RECOGNIZER_ONNX_SHA256,
    CONSENSUS_RESCUE_SCORE_MINIMUM, OFFICIAL_RESCUE_SCORE_MINIMUM,
    REVISION, SPLITS, protocol_configuration,
)
from ml.ocr.production_composition_v6.sealed_gate import EVALUATOR_SOURCE_PATHS


REPO_ROOT = Path(__file__).resolve().parents[4]
ROOT = REPO_ROOT / "ml/ocr/production_composition_v6"
...
def test_protocol_is_fresh_fail_closed_four_model_composition() -> None:
    ...
    assert CONSENSUS_RESCUE_SCORE_MINIMUM == 0.85
...
def test_fresh_splits_are_disjoint_complete_and_unopened() -> None:
    ...
    predecessor_fingerprints = {
        _load(REPO_ROOT / "ml/ocr/production_composition_v5/VALIDATION_SEAL.json")["split_fingerprint"],
        _load(REPO_ROOT / "ml/ocr/production_composition_v5/SEALED_PUBLIC_TEST_SEAL.json")["split_fingerprint"],
    }
...
def test_consumed_predecessor_and_component_evidence_are_unchanged() -> None:
    assert sha256_file(REPO_ROOT / "ml/ocr/production_composition_v5/VALIDATION_REPORT.json") == (
