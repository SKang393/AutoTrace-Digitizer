...
"""Single-use final P3 context-topology veto training for OCR V17."""
...
from .model_p3 import ContextTopologyVetoProposalRoleNet
...
from .train_p2 import class_balanced_focal_loss, hard_tail_separation_loss
...
RUNNER_SOURCE_PATHS = (
    ...
    ROOT / "train_p2.py", ROOT / "train_p3.py",
...
def train_candidate(output_dir: Path) -> dict[str, object]:
    if output_dir.exists():
        raise RuntimeError(f"OCR V17 P3 output exists: {output_dir}")
    ...
    try:
        if config["expected_runner_source_bundle_sha256"] != source_bundle_sha256(REPO_ROOT, RUNNER_SOURCE_PATHS):
            raise RuntimeError("OCR V17 P3 runner sources changed")
        ...
        if config["p2_result_sha256"] != sha256_file(REPO_ROOT / P2_RESULT_PATH):
            raise RuntimeError("OCR V17 P2 result changed")
        ...
        p2_checkpoint_path = REPO_ROOT / config["p2_checkpoint_path"]
        p2_report_path = REPO_ROOT / p2["candidate_report_path"]
        if (
            p1["status"] != "failed_selection" or p2["status"] != "failed_selection"
            or p1["consumed"] is not True or p2["consumed"] is not True
            or p1["passing_threshold_window"] != [] or p2["passing_threshold_window"] != []
            or p1["public_gate_evaluations"] != 0 or p2["public_gate_evaluations"] != 0
            or p1["public_gate_archive_opened"] is not False or p2["public_gate_archive_opened"] is not False
            or p2["checkpoint_sha256"] != config["p2_checkpoint_sha256"]
            or p2["candidate_report_sha256"] != config["p2_candidate_report_sha256"]
            or sha256_file(p2_checkpoint_path) != config["p2_checkpoint_sha256"]
            or sha256_file(p2_report_path) != config["p2_candidate_report_sha256"]
        ):
            raise RuntimeError("OCR V17 consumed P1/P2 evidence does not authorize P3")
        ...
        for key in (
            ...
            if training_evidence[key] != config[key]:
                raise RuntimeError(f"OCR V17 P3 training evidence changed: {key}")
        if any(training_evidence[key] is not False for key in (
            ...
            raise RuntimeError("OCR V17 P3 training data violated preregistered scope")

        generator = _configure(int(config["seed"]))
        model = ContextTopologyVetoProposalRoleNet(
            seed=int(config["seed"]), maximum_context_veto=float(config["maximum_context_veto"]),
        )
        checkpoint = torch.load(p2_checkpoint_path, map_location="cpu", weights_only=True)
        ...
        with torch.inference_mode():
            frozen_before = model.base(parity_values).clone()
        ...
        if optimizer_steps != int(config["expected_optimizer_steps"]):
            raise RuntimeError("OCR V17 P3 optimizer-step count changed")
        model.eval()
        with torch.inference_mode():
            frozen_after = model.base(parity_values)
        base_invariance_error = float(torch.max(torch.abs(frozen_before - frozen_after)))
        if base_invariance_error != 0.0:
            raise RuntimeError("OCR V17 P3 changed frozen P2 outputs")

        phase = "export"
        checkpoint_path = output_dir / "graph-text-structural-veto-proposal-role-v17-p3.pt"
        torch.save({"state_dict": model.state_dict()}, checkpoint_path)
        onnx_path = output_dir / "graph-text-structural-veto-proposal-role-v17-p3.onnx"
        _export(model, parity_values, onnx_path)
        session = ort.InferenceSession(str(onnx_path), providers=["CPUExecutionProvider"])
        if session.get_providers() != ["CPUExecutionProvider"]:
            raise RuntimeError("OCR V17 P3 selection requires CPU execution only")
        with torch.inference_mode():
        ...
        report: dict[str, object] = {
            ...
            "p2_result_sha256": sha256_file(REPO_ROOT / P2_RESULT_PATH),
            ...
            "p2_base_parameters_frozen": True,
            "p2_base_invariance_maximum_absolute_error": base_invariance_error,
            "role_invariance_maximum_absolute_error": base_invariance_error,
...
__all__ = ["RUNNER_SOURCE_PATHS", "train_candidate"]
