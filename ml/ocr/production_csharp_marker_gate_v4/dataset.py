...
"""Disjoint five-role procedural scenes for the V8 C# composition gate."""
...
SOURCE_GRAPH_FAMILIES = (
    "ascending-terrace-v4-composed",
    "descending-terrace-v4-composed",
    "paired-switchback-v4-composed",
    "sparse-maintenance-v4-composed",
    "dense-alternation-v4-composed",
    "offset-stability-v4-composed",
)
NUMERIC_LABELS = (
    ("0", "0"), ("5", "5"), ("10", "10"), ("20", "20"),
...
def _derived_seed(secret_seed: int, index: int) -> int:
    material = f"{REVISION}:{secret_seed}:{index}:sealed-public-v4".encode()
...
def _apply_degradation(
    ...
    if family == "sloped-paper-quantization-v4-public":
        value += (0.52 * xx) - (0.31 * yy)
        value = np.rint(value / 3.0) * 3.0
    elif family == "offset-ellipse-contrast-v4-public":
        radius = np.square((xx - 0.17) / 1.09) + np.square((yy + 0.21) / 0.91)
        value = ((value - 128.0) * float(rng.uniform(0.985, 1.018))) + 128.0
        value += np.where(radius < 0.67, 0.72, -0.46)
    elif family == "alternating-row-shelf-v4-public":
        value += ((np.floor((yy + 1.0) * 9.0) % 2.0) * 0.74) - 0.37
        value += 0.31 * np.sin((1.11 * xx + 0.83 * yy) * np.pi)
    elif family == "diagonal-cosine-fade-v4-public":
        value -= np.clip((np.abs(xx + 0.28) + np.abs(yy - 0.19)) * 0.63, 0.0, 1.54)
        value += 0.38 * np.cos((1.43 * xx - 0.91 * yy) * np.pi)
    else:
        raise ValueError(f"Unsupported degradation family {family!r}")
    noise = rng.normal(0.0, 0.16, size=value.shape)
...
def build_scene(secret_seed: int, index: int) -> CompositeScene:
    ...
    annotation_clear = (317, annotation_y - 6, 638, annotation_y + 38)
    legend_clear = (562, 158, 639, 229)
    draw.rectangle(annotation_clear, fill=background)
    draw.rectangle(legend_clear, fill=background)
    artifact_mask[annotation_clear[1]:annotation_clear[3] + 1, annotation_clear[0]:annotation_clear[2] + 1] = 0
    artifact_mask[legend_clear[1]:legend_clear[3] + 1, legend_clear[0]:legend_clear[2] + 1] = 0
    labels = (
    ...
    for label_index, (display, truth_override, role, family, position) in enumerate(labels):
        ...
        truths.append(TextTruth(display, truth_override, role, family, bounds))
    ...
    return CompositeScene(
        f"ocr-marker-csharp-v4-public-{index:05d}",
