...
"""Single-use P1 training and fixed validation for graph text detection."""
...
CONFIG_PATH = Path("ml/ocr/graph_text_detector_v1/training/p1.json")
RUNNER_SOURCE_PATHS = (
...
def _loss(probabilities: torch.Tensor, target: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    binary = nn.functional.binary_cross_entropy(probabilities, target)
    intersection = (probabilities * target).sum(dim=(1, 2, 3))
    denominator = probabilities.sum(dim=(1, 2, 3)) + target.sum(dim=(1, 2, 3))
    dice = 1.0 - ((2.0 * intersection + 1.0) / (denominator + 1.0)).mean()
    return binary + dice, binary, dice
...
def evaluate_frames(
    ...
    for frame in frames:
        tensor = detector_contract.detector_tensor(
            frame.detector_bgr,
            FRAME_WIDTH,
            FRAME_HEIGHT,
        )
        ...
        if output.shape != expected_shape or not np.isfinite(output).all():
            raise RuntimeError(f"Detector runner returned an invalid output for {frame.case_id}: {output.shape}")
        if float(output.min()) < 0.0 or float(output.max()) > 1.0:
            raise RuntimeError(f"Detector runner returned non-probability output for {frame.case_id}")
...
def train_candidate(output_dir: Path) -> dict[str, object]:
    ...
    try:
        ...
        if training_split_fingerprint() != selection["train_split_fingerprint"]:
            raise RuntimeError("Graph text detector training split changed")
        ...
        model = GraphTextRegionNet(seed=int(config["seed"]))
        phase = "onnx_preflight"
        preflight = output_dir / "export-preflight.onnx"
        _export(model.eval(), preflight)
        _ = ort.InferenceSession(str(preflight), providers=["CPUExecutionProvider"])
        ...
        phase = "export"
        model.eval()
        checkpoint_path = output_dir / "graph-text-region-detector-v1-p1.pt"
        torch.save({"state_dict": model.state_dict(), "revision": REVISION, "candidate_id": CANDIDATE_ID}, checkpoint_path)
        onnx_path = output_dir / "graph-text-region-detector-v1-p1.onnx"
        _export(model, onnx_path)
        session = ort.InferenceSession(str(onnx_path), providers=["CPUExecutionProvider"])
        input_name = session.get_inputs()[0].name
        output_name = session.get_outputs()[0].name
        parity_input = normalize_bgr(bgr_tensor[:8]).numpy()
