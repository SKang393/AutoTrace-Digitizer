...
"""Single-use P3 training with DB shrink targets and targeted exclusions."""
...
from .dataset import build_validation_split, split_fingerprint
from .dataset_p3 import (
    P3_PATCH_HEIGHT,
    P3_PATCH_WIDTH,
    build_p3_training_arrays,
    p3_training_split_fingerprint,
)
from .model_p2 import GraphTextRegionNetP2
from .protocol import REVISION, TASK
from .train_p1 import _configure, _loss, _selection_passed, evaluate_frames, normalize_bgr


REPO_ROOT = Path(__file__).resolve().parents[3]
CANDIDATE_ID = "P3"
CONFIG_PATH = Path("ml/ocr/graph_text_detector_v1/training/p3.json")
CANONICAL_OUTPUT = Path("ml/ocr/graph_text_detector_v1/artifacts/P3-run")
RUNNER_SOURCE_PATHS = (
    Path("ml/ocr/graph_text_detector_v1/dataset.py"),
    Path("ml/ocr/graph_text_detector_v1/dataset_p2.py"),
    Path("ml/ocr/graph_text_detector_v1/dataset_p3.py"),
    Path("ml/ocr/graph_text_detector_v1/model.py"),
    Path("ml/ocr/graph_text_detector_v1/model_p2.py"),
    Path("ml/ocr/graph_text_detector_v1/protocol.py"),
    Path("ml/ocr/graph_text_detector_v1/train_p1.py"),
    Path("ml/ocr/graph_text_detector_v1/train_p3.py"),
...
def _export(model: torch.nn.Module, path: Path) -> None:
    example = torch.zeros((1, 3, P3_PATCH_HEIGHT, P3_PATCH_WIDTH), dtype=torch.float32)
    torch.onnx.export(
        model,
        example,
        path,
...
def train_candidate(output_dir: Path) -> dict[str, object]:
    ...
    try:
        if sha256_file(REPO_ROOT / config["p2_result_path"]) != config["p2_result_sha256"]:
            raise RuntimeError("P2 failure evidence changed")
        ...
        if sha256_file(selection_path) != config["selection_manifest_sha256"]:
            raise RuntimeError("Graph text detector selection manifest changed")
        ...
        if split_fingerprint(validation_frames) != selection["validation_split_fingerprint"]:
            raise RuntimeError("Graph text detector validation split changed")
        if p3_training_split_fingerprint() != config["training_split_fingerprint"]:
            raise RuntimeError("Graph text detector P3 training split changed")
        ...
        if sha256_file(seal_path) != config["sealed_public_test_seal_sha256"]:
            raise RuntimeError("Graph text detector sealed-public seal changed")
        seal = json.loads(seal_path.read_text(encoding="utf-8"))
        if sha256_file(REPO_ROOT / str(seal["fixture_archive_path"])) != seal["fixture_archive_sha256"]:
            raise RuntimeError("Graph text detector sealed-public archive changed")

        generator = _configure(int(config["seed"]))
        bgr, targets = build_p3_training_arrays()
        bgr_tensor = torch.from_numpy(bgr)
        target_tensor = torch.from_numpy(targets).float() / 255.0
        model = GraphTextRegionNetP2(seed=int(config["seed"]))
        ...
        if not np.isfinite(preflight_output).all() or float(preflight_output.min()) < 0.0 or float(preflight_output.max()) > 1.0:
            raise RuntimeError("P3 ONNX preflight violated the strict probability output contract")
        ...
        for epoch in range(int(config["epochs"])):
            ...
            for start in range(0, len(order), int(config["batch_size"])):
                ...
                total, binary, dice = _loss(model(inputs), target)
                ...
                checkpoints.append(
                    {
                        ...
                        "binary_cross_entropy": sum(item[1] for item in losses) / len(losses),
        ...
        phase = "export"
        model.eval()
        checkpoint_path = output_dir / "graph-text-region-detector-v1-p3.pt"
        torch.save({"state_dict": model.state_dict(), "revision": REVISION, "candidate_id": CANDIDATE_ID}, checkpoint_path)
        onnx_path = output_dir / "graph-text-region-detector-v1-p3.onnx"
        ...
        report: dict[str, object] = {
            "schema": "graphreader.ocr-graph-text-detector-training-report.v1",
            "task": TASK,
            "revision": REVISION,
            "candidate_id": CANDIDATE_ID,
        ...
        failure = {
            "schema": "graphreader.ocr-graph-text-detector-training-failure.v1",
            "task": TASK,
            "revision": REVISION,
            "candidate_id": CANDIDATE_ID,
