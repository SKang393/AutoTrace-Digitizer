...
"""Single-use P2 training on exact production-scale synthetic crops."""
...
CANDIDATE_ID = "P2"
CONFIG_PATH = Path("ml/ocr/graph_text_detector_v1/training/p2.json")
CANONICAL_OUTPUT = Path("ml/ocr/graph_text_detector_v1/artifacts/P2-run")
RUNNER_SOURCE_PATHS = (
    ...
    Path("ml/ocr/graph_text_detector_v1/train_p2.py"),
...
def _export(model: torch.nn.Module, path: Path) -> None:
    example = torch.zeros((1, 3, P2_PATCH_HEIGHT, P2_PATCH_WIDTH), dtype=torch.float32)
    torch.onnx.export(
        model,
        example,
        path,
...
def train_candidate(output_dir: Path) -> dict[str, object]:
    ...
    try:
        if sha256_file(REPO_ROOT / config["p1_result_path"]) != config["p1_result_sha256"]:
            raise RuntimeError("P1 failure evidence changed")
        ...
        if p2_training_split_fingerprint() != config["training_split_fingerprint"]:
            raise RuntimeError("Graph text detector P2 training split changed")
        ...
        bgr, targets = build_p2_training_arrays()
        ...
        if not np.isfinite(preflight_output).all() or float(preflight_output.min()) < 0.0 or float(preflight_output.max()) > 1.0:
            raise RuntimeError("P2 ONNX preflight violated the strict probability output contract")
        ...
        phase = "export"
        model.eval()
        checkpoint_path = output_dir / "graph-text-region-detector-v1-p2.pt"
        torch.save({"state_dict": model.state_dict(), "revision": REVISION, "candidate_id": CANDIDATE_ID}, checkpoint_path)
        onnx_path = output_dir / "graph-text-region-detector-v1-p2.onnx"
