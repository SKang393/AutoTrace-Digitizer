...
"""Canonical-budget synthetic train/dev runner for OCR detector V37."""
...
ROOT = Path("ml/ocr/degradation_coverage_detector_v37")
...
def _verify_inputs(config: dict[str, object]) -> None:
    for path, expected in (
        ...
        (V36_RESULT_PATH, V36_RESULT_SHA256),
        (V36_DIAGNOSTIC_PATH, V36_DIAGNOSTIC_SHA256),
    ):
        if sha256_file(REPO_ROOT / path) != expected:
            raise RuntimeError(f"V37 trigger evidence changed: {path.as_posix()}")
    if split_fingerprint("dev") != EXPECTED_V32_DEV_FINGERPRINT:
        raise RuntimeError("V37 dev split changed")
    ...
    if v32_split_fingerprint("train") != EXPECTED_V32_TRAIN_FINGERPRINT:
        raise RuntimeError("V37 base train split changed")
    if len(build_split("train")) != TRAIN_SCENE_COUNT or len(build_split("dev")) != 5:
        raise RuntimeError("V37 scene counts changed")
    if config.get("seed") != SEED or config.get("train_scene_count") != TRAIN_SCENE_COUNT:
        raise RuntimeError("V37 frozen training configuration changed")
    if config.get("public_or_sealed_reads") != 0 or config.get("real_reads") != 0:
        raise RuntimeError("V37 cannot read protected evidence")


def train_candidate(output_dir: Path) -> dict[str, object]:
    if output_dir.exists():
        raise RuntimeError(f"V37 output already exists: {output_dir}")
    ...
    try:
        ...
        if best_state is None:
            raise RuntimeError("V37 training produced no selected train-loss state")
        model.load_state_dict(best_state)
        model.eval()
        phase = "export"
        checkpoint_path = output_dir / "graph-text-degradation-coverage-detector-v37-p1.pt"
        torch.save({"state_dict": model.state_dict(), "seed": SEED, "best_train_epoch": best_epoch}, checkpoint_path)
        onnx_path = output_dir / "graph-text-degradation-coverage-detector-v37-p1.onnx"
        ...
        if session.get_providers() != [ONNX_PROVIDER]:
            raise RuntimeError(f"V37 parity requires {ONNX_PROVIDER}")
        ...
        with torch.inference_mode():
            for count in PARITY_BATCH_SIZES:
                ...
                actual = np.asarray(
                    session.run(["text_logits"], {"source_tiles": sample.numpy()})[0],
                    dtype=np.float32,
                )
        ...
        report = {
            "schema": "graphreader.ocr-degradation-coverage-detector-v37-result.v1",
            "task": TASK,
            "revision": REVISION,
            "candidate_id": "P1",
        ...
        failure = {
            "schema": "graphreader.ocr-degradation-coverage-detector-v37-failure.v1",
            "task": TASK,
            "revision": REVISION,
            "candidate_id": "P1",
...
def main() -> int:
    ...
    print(json.dumps({
        ...
        "dev_metrics": report.get("dev_metrics"),
        "onnx_parity_maximum_absolute_error": report.get("onnx_parity_maximum_absolute_error"),
