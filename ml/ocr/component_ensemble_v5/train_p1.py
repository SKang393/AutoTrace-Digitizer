...
"""Single-use P1 training for the preregistered OCR V5 revision."""
...
from .model import ComponentEnsembleGlyphNet
...
def train_candidate(output_dir: Path) -> dict[str, object]:
    ...
    try:
        ...
        if sha256_file(selection_path) != config["selection_manifest_sha256"]:
            raise RuntimeError("OCR V5 selection manifest checksum changed")
        ...
        if sha256_file(sealed_path) != config["sealed_public_test_seal_sha256"]:
            raise RuntimeError("OCR V5 public seal checksum changed")
        sealed = json.loads(sealed_path.read_text(encoding="utf-8"))
        if sha256_file(REPO_ROOT / sealed["fixture_archive_path"]) != sealed["fixture_archive_sha256"]:
            raise RuntimeError("OCR V5 sealed-public archive changed before training")
        ...
        if split_fingerprint(training_samples) != selection["train_split_fingerprint"]:
            raise RuntimeError("OCR V5 training renderer changed after freeze")
        if split_fingerprint(validation_samples) != selection["validation_split_fingerprint"]:
            raise RuntimeError("OCR V5 validation renderer changed after freeze")
        ...
        model = ComponentEnsembleGlyphNet(seed=seed)
        ...
        model.eval()
        comparisons = [
            {"threshold": float(value), "metrics": evaluate_samples(validation_samples, _runner(model), float(value))}
            for value in config["selection_thresholds"]
        ]
        selected = max(
            comparisons,
            key=lambda item: (
        ...
        phase = "export"
        checkpoint_path = output_dir / "graph-numeric-component-ensemble-v5-p1.pt"
        torch.save({"state_dict": model.state_dict(), "selected_threshold": selected["threshold"]}, checkpoint_path)
        onnx_path = output_dir / "graph-numeric-component-ensemble-v5-p1.onnx"
        ...
        report: dict[str, object] = {
            "schema": "graphreader.ocr-component-ensemble-training-report.v1",
            "task": TASK,
            "revision": REVISION,
            "candidate_id": CANDIDATE_ID,
        ...
        failure = {
            "schema": "graphreader.ocr-component-ensemble-training-failure.v1",
            "task": TASK,
            "revision": REVISION,
            "candidate_id": CANDIDATE_ID,
