...
"""Single-use public gate for the selected OCR component-region V6 candidate."""
...
RESULT_PATH = Path("ml/ocr/component_region_detector_v6/P1_RESULT.json")
SPLIT_CONFIG_PATH = Path("ml/ocr/component_region_detector_v6/gates/sealed-public-v1.json")
...
def _entry(ledger: dict[str, object]) -> dict[str, object]:
    ...
    if entry is None:
        raise RuntimeError("OCR component-region V6 budget entry is missing")
    return entry
...
def evaluate_candidate(*, onnx_path: Path, selection_report_path: Path, output_path: Path) -> dict[str, object]:
    ...
    if output_path.exists():
        raise RuntimeError(f"OCR V6 public output already exists: {output_path}")
    ...
    if (
        ...
        raise RuntimeError("OCR component-region V6 public gate is not authorized by the canonical ledger")
    ...
    if (
        ...
        raise RuntimeError("Only the exact validation-selected OCR V6 candidate may open the public gate")
    ...
    if sha256_file(seal_path) != split_config["sealed_public_test_seal_sha256"]:
        raise RuntimeError("OCR V6 public seal differs from the frozen gate configuration")
    ...
    if sha256_file(archive_path) != seal_data["fixture_archive_sha256"]:
        raise RuntimeError("OCR V6 sealed fixture archive checksum mismatch")
    if sha256_file(private_manifest_path) != seal_data["private_manifest_sha256"]:
        raise RuntimeError("OCR V6 sealed private manifest checksum mismatch")
    ...
    if (
        ...
        raise RuntimeError("OCR V6 sealed private manifest violates the frozen data contract")

    session = ort.InferenceSession(str(onnx_path), providers=["CPUExecutionProvider"])
    if session.get_providers() != ["CPUExecutionProvider"]:
        raise RuntimeError("OCR V6 public gate requires CPUExecutionProvider only")
    if [item.name for item in session.get_inputs()] != ["region_proposals"]:
        raise RuntimeError("OCR V6 public gate found an unexpected ONNX input contract")
    ...
    if (
        ...
        raise RuntimeError("OCR V6 sealed fixture bytes do not reproduce the frozen scene contract")
    ...
    report: dict[str, object] = {
        "schema": "graphreader.ocr-component-region-public-gate.v1",
        "task": TASK,
        "revision": PUBLIC_REVISION,
