...
"""Single-use P1 training for the preregistered component-region detector V6."""
...
from .model import ComponentRegionNet
...
CANDIDATE_ID = "P1"
CONFIG_PATH = Path("ml/ocr/component_region_detector_v6/training/p1.json")
CANONICAL_OUTPUT = Path("ml/ocr/component_region_detector_v6/artifacts/P1-run")
RUNNER_SOURCE_PATHS = (
    Path("ml/ocr/component_region_detector_v6/dataset.py"),
    Path("ml/ocr/component_region_detector_v6/model.py"),
    Path("ml/ocr/component_region_detector_v6/pipeline.py"),
    Path("ml/ocr/component_region_detector_v6/protocol.py"),
    Path("ml/ocr/component_region_detector_v6/train_p1.py"),
...
def train_candidate(output_dir: Path) -> dict[str, object]:
    ...
    try:
        ...
        if sha256_file(selection_path) != config["selection_manifest_sha256"]:
            raise RuntimeError("OCR V6 selection manifest checksum changed")
        ...
        if sha256_file(sealed_path) != config["sealed_public_test_seal_sha256"]:
            raise RuntimeError("OCR V6 public seal checksum changed")
        sealed = json.loads(sealed_path.read_text(encoding="utf-8"))
        if sha256_file(REPO_ROOT / sealed["fixture_archive_path"]) != sealed["fixture_archive_sha256"]:
            raise RuntimeError("OCR V6 sealed-public archive changed before training")
        ...
        phase = "export"
        checkpoint_path = output_dir / "graph-text-component-region-v6-p1.pt"
        torch.save({"state_dict": model.state_dict(), "selected_threshold": selected["threshold"]}, checkpoint_path)
        onnx_path = output_dir / "graph-text-component-region-v6-p1.onnx"
        ...
        report: dict[str, object] = {
            "schema": "graphreader.ocr-component-region-training-report.v1",
            "task": TASK,
            "revision": REVISION,
            "candidate_id": CANDIDATE_ID,
        ...
        failure = {
            "schema": "graphreader.ocr-component-region-training-failure.v1",
            "task": TASK,
            "revision": REVISION,
            "candidate_id": CANDIDATE_ID,
