...
"""Single-use truth-hidden public gate for a selected OCR V15 candidate."""
...
def evaluate_candidate(*, onnx_path: Path, selection_report_path: Path, output_path: Path) -> dict[str, object]:
    ...
    if output_path.exists():
        raise RuntimeError(f"OCR V15 public output exists: {output_path}")
    ...
    if (
        ...
        raise RuntimeError("OCR V15 public gate is not authorized by the canonical ledger")
    if (
        ...
        raise RuntimeError("Only the exact selected V15 candidate may open the public gate")
    ...
    if sha256_file(archive_path) != seal["fixture_archive_sha256"] or sha256_file(manifest_path) != seal["private_manifest_sha256"]:
        raise RuntimeError("OCR V15 public fixture bytes changed")
    session = ort.InferenceSession(str(onnx_path), providers=["CPUExecutionProvider"])
    if session.get_providers() != ["CPUExecutionProvider"]:
        raise RuntimeError("OCR V15 public gate requires CPU execution only")
    ...
    if any(summary[key] != seal[key] for key in summary) or split_fingerprint(scenes) != seal["split_fingerprint"]:
        raise RuntimeError("OCR V15 public fixtures violate the frozen contract")
    ...
    report: dict[str, object] = {
        "schema": "graphreader.ocr-layout-conditioned-proposal-role-public-gate.v1",
