...
"""Freeze V15 train, selection, and truth-hidden public evidence."""
...
def freeze_split() -> dict[str, str]:
    ...
    if existing:
        raise RuntimeError("OCR V15 split freeze refuses overwrite: " + ", ".join(existing))
    protocol_path = REPO_ROOT / PROTOCOL
    if protocol_path.read_bytes() != canonical_json_bytes(protocol_configuration()):
        raise RuntimeError("OCR V15 committed preregistration changed before split freeze")
    ...
    if any(training_evidence[key] is not False for key in (
        ...
        raise RuntimeError("OCR V15 training examples violated the frozen scope")
    selection = {
        "schema": "graphreader.ocr-layout-conditioned-proposal-role-selection.v1",
    ...
    seal = {
        "schema": "graphreader.ocr-layout-conditioned-proposal-role-sealed-test-seal.v1",
    ...
    gate = {
        "schema": "graphreader.ocr-layout-conditioned-proposal-role-gate-config.v1",
    ...
    config = {
        "schema": "graphreader.ocr-layout-conditioned-proposal-role-training.v1",
