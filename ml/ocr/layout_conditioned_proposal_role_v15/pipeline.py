...
"""Fixed scene, proposal, and role metrics for OCR V15."""
...
def evaluate_thresholds(
    ...
    for scene in scenes:
        ...
        if not candidates:
            raise RuntimeError("OCR V15 scene produced no proposals")
        values = np.stack([
        ...
        if logits.shape != (len(candidates), 2 + len(ROLE_ORDER)) or not np.isfinite(logits).all():
            raise RuntimeError("OCR V15 runner returned invalid logits")
