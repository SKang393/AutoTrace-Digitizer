...
"""Single-use P2 training with absolute-scale glyph encoding."""
...
from .p2_pipeline import evaluate_samples, glyph_training_examples
...
CANDIDATE_ID = "P2"
CONFIG_PATH = Path("ml/ocr/component_geometric_v4/training/p2.json")
CANONICAL_OUTPUT = Path("ml/ocr/component_geometric_v4/artifacts/P2-run")
RUNNER_SOURCE_PATHS = (
    Path("ml/ocr/component_geometric_v4/dataset.py"),
    Path("ml/ocr/component_geometric_v4/model.py"),
    Path("ml/ocr/component_geometric_v4/p2_dataset.py"),
    Path("ml/ocr/component_geometric_v4/p2_pipeline.py"),
    Path("ml/ocr/component_geometric_v4/protocol.py"),
    Path("ml/ocr/component_geometric_v4/train_p1.py"),
    Path("ml/ocr/component_geometric_v4/train_p2.py"),
...
def train_candidate(output_dir: Path) -> dict[str, object]:
    ...
    try:
        ...
        if sha256_file(selection_path) != config["selection_manifest_sha256"]:
            raise RuntimeError("Selection manifest checksum does not match P2 preregistration")
        ...
        if sha256_file(sealed_path) != config["sealed_public_test_seal_sha256"]:
            raise RuntimeError("Sealed-public seal checksum does not match P2 preregistration")
        sealed = json.loads(sealed_path.read_text(encoding="utf-8"))
        if sha256_file(REPO_ROOT / sealed["fixture_archive_path"]) != sealed["fixture_archive_sha256"]:
            raise RuntimeError("Sealed-public archive changed before P2 training")
        ...
        if split_fingerprint(training_samples) != selection["train_split_fingerprint"]:
            raise RuntimeError("P2 training renderer no longer reproduces the frozen split")
        if split_fingerprint(validation_samples) != selection["validation_split_fingerprint"]:
            raise RuntimeError("P2 validation renderer no longer reproduces the frozen split")
        ...
        phase = "export"
        checkpoint_path = output_dir / "graph-numeric-component-geometric-v4-p2.pt"
        torch.save({"state_dict": model.state_dict(), "selected_threshold": selected["threshold"]}, checkpoint_path)
        onnx_path = output_dir / "graph-numeric-component-geometric-v4-p2.onnx"
