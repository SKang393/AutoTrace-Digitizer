...
"""P3 training examples, decoding, and unchanged OCR V4 metrics."""
...
from .p3_dataset import isolate_glyphs_shape_and_geometry
...
def glyph_training_examples(samples: Iterable[LabelSample]) -> tuple[np.ndarray, np.ndarray]:
    ...
    for sample in samples:
        isolated = isolate_glyphs_shape_and_geometry(sample.raster)
        if sample.exclusion_kind is None:
            if len(isolated) != len(sample.target_text):
                raise RuntimeError(
                    f"P3 deterministic isolation changed text length for {sample.sample_id}: "
    ...
    if not glyphs:
        raise RuntimeError("OCR V4 P3 training split produced no glyphs")
...
def decode_raster(raster: np.ndarray, runner: Runner, threshold: float) -> str:
    ...
    if logits.shape != (len(glyphs), len(ALPHABET) + 1) or not np.isfinite(logits).all():
        raise RuntimeError("OCR V4 P3 runner returned an invalid glyph-logit tensor")
