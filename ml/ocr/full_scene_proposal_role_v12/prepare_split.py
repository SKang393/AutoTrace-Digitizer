...
"""Freeze V12 train, selection, and hidden-public evidence."""
...
def freeze_split() -> dict[str, str]:
    ...
    if existing:
        raise RuntimeError("OCR V12 split freeze refuses overwrite: " + ", ".join(existing))
    protocol_path = REPO_ROOT / PROTOCOL
    if protocol_path.read_bytes() != canonical_json_bytes(protocol_configuration()):
        raise RuntimeError("OCR V12 committed preregistration changed before split freeze")
    ...
    values, labels, role_labels, training_evidence = training_examples(
        train, negative_cap_per_scene=int(protocol_configuration()["training"]["negative_cap_per_scene"]),
    )
    if any(training_evidence[key] is not False for key in (
        "validation_or_public_pixels_used", "predecessor_fixture_bytes_used", "v3_fixture_bytes_or_scene_truth_used",
    )):
        raise RuntimeError("OCR V12 training examples violated frozen scope")
    selection = {
        "schema": "graphreader.ocr-full-scene-proposal-role-selection.v1", "task": TASK, "revision": REVISION,
        ...
        "training_proposal_label_count": int(len(labels)), "training_role_label_count": int((role_labels >= 0).sum()),
        "synthetic_only": True, "private_or_article_images": False, "chandler_included": False,
        "generalization_label_included": False, "v3_fixture_bytes_or_scene_truth_used": False,
        "predecessor_fixture_bytes_reused": False, "validation_or_public_pixels_used_for_training": False,
        "sealed_public_truth_available_to_candidate": False, "production_approval": False, "release_eligible": False,
    }
    ...
    seal = {
        "schema": "graphreader.ocr-full-scene-proposal-role-sealed-test-seal.v1", "task": TASK,
        ...
        **proposal_summary(public), "split_fingerprint": split_fingerprint(public), "synthetic_only": True,
        ...
        "v3_fixture_bytes_or_scene_truth_used": False, "predecessor_fixture_bytes_reused": False,
    ...
    gate = {
        "schema": "graphreader.ocr-full-scene-proposal-role-gate-config.v1", "task": TASK,
    ...
    config = {
        "schema": "graphreader.ocr-full-scene-proposal-role-training.v1", "task": TASK, "revision": REVISION,
        "candidate_id": "P1", "experiment_ordinal": 1, "experiment_budget": 3,
        "architecture": protocol_configuration()["architecture"], "isolated_change": protocol_configuration()["isolated_change"],
        ...
        "negative_cap_per_scene": training["negative_cap_per_scene"], "negative_sampling": training["negative_sampling"],
        "output_logit_scale": training["output_logit_scale"], "role_loss_weight": training["role_loss_weight"],
        **training_evidence, "selection_thresholds": protocol_configuration()["selection_thresholds"],
        "selection_manifest_path": SELECTION.as_posix(), "selection_manifest_sha256": sha256_file(REPO_ROOT / SELECTION),
        "sealed_public_test_seal_path": SEAL.as_posix(), "sealed_public_test_seal_sha256": sha256_file(REPO_ROOT / SEAL),
        ...
        "predecessor_fixture_bytes_reused": False, "v3_fixture_bytes_or_scene_truth_used": False,
        "validation_or_public_pixels_used_for_training": False, "private_or_article_images": False,
    ...
    return {
        "protocol_sha256": sha256_file(protocol_path), "selection_manifest_sha256": sha256_file(REPO_ROOT / SELECTION),
        ...
        "gate_config_sha256": sha256_file(REPO_ROOT / GATE), "candidate_config_sha256": sha256_file(REPO_ROOT / CONFIG),
    }
