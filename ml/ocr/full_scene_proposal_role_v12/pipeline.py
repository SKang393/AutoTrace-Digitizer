...
"""Fixed selection metrics for OCR V12."""
...
def evaluate_thresholds(
    ...
    for scene in scenes:
        ...
        if not candidates:
            raise RuntimeError("OCR V12 scene produced no proposals")
        ...
        if logits.shape != (len(candidates), 2 + len(ROLE_ORDER)) or not np.isfinite(logits).all():
            raise RuntimeError("OCR V12 runner returned invalid logits")
        shifted = logits[:, :2] - logits[:, :2].max(axis=1, keepdims=True)
        probabilities = np.exp(shifted)[:, 1] / np.exp(shifted).sum(axis=1)
