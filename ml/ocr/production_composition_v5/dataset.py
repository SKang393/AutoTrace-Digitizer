...
"""Fresh procedural graph scenes for OCR composition V5."""
...
def _rng(split: str, index: int) -> np.random.Generator:
    ...
    material = f"production-composition-v5:{registration.seed_offset}:{split}:{index}".encode()
...
def _render_scene(split: str, index: int) -> CompositionScene:
    ...
    base -= np.linspace(float(rng.integers(1, 5)), float(rng.integers(0, 4)), SCENE_HEIGHT)[:, None]
    image = Image.fromarray(np.rint(base).clip(0, 255).astype(np.uint8), mode="L")
    draw = ImageDraw.Draw(image)
    _draw_structures(draw, rng, index + 1903, ink)
    y_display, y_truth = NUMERIC_LABELS[(index * 5 + 4) % len(NUMERIC_LABELS)]
    x_display, x_truth = NUMERIC_LABELS[(index * 8 + 2) % len(NUMERIC_LABELS)]
    phase = PHASE_WORDS[(index * 4 + 2) % len(PHASE_WORDS)]
    annotation = ANNOTATION_WORDS[(index * 7 + 3) % len(ANNOTATION_WORDS)]
    legend = LEGEND_WORDS[(index * 5 + 4) % len(LEGEND_WORDS)]
    annotation_y = 96 + (index * 31) % 55
    draw.rectangle((318, annotation_y - 6, 638, annotation_y + 38), fill=background)
    draw.rectangle((563, 158, 639, 229), fill=background)
    labels = (
        (y_display, y_truth, "y_tick", "numeric", (14, 69 + (index * 41) % 102)),
        (x_display, x_truth, "x_tick", "numeric", (171 + (index * 67) % 220, 282)),
        (phase, phase, "phase_heading", "word", (139 + (index * 53) % 181, 7)),
        (annotation, annotation, "annotation", "ambiguity" if annotation == "O o l I" else "word", (345 + (index * 43) % 97, annotation_y)),
        (legend, legend, "legend_text", "word", (577, 174 + (index * 7) % 12)),
    )
    ...
    if split == "validation":
        image = image.resize((636, 316), Image.Resampling.BILINEAR).resize((SCENE_WIDTH, SCENE_HEIGHT), Image.Resampling.BICUBIC)
        pixels = np.asarray(image, dtype=np.float32)
        diagonal = (np.arange(SCENE_WIDTH) * 3 + index * 17) % SCENE_HEIGHT
        for x, y in enumerate(diagonal):
            pixels[max(0, y - 1):min(SCENE_HEIGHT, y + 2), x] = np.maximum(
                pixels[max(0, y - 1):min(SCENE_HEIGHT, y + 2), x], 181
            )
    else:
        pixels = np.rint(np.clip((pixels - 128) * float(rng.uniform(0.965, 1.035)) + 128, 0, 255))
        pixels = pixels.astype(np.uint16) // 2 * 2
        edge = 48 + (index * 73) % 238
        pixels[edge:edge + 2, :] = np.maximum(pixels[edge:edge + 2, :], 179)
    raster = np.rint(pixels).clip(0, 255).astype(np.uint8)
    return CompositionScene(
        f"ocr-production-composition-v5-{split}-{index:05d}", split,
...
def save_sealed_archive(scenes: tuple[CompositionScene, ...], path: Path) -> dict[str, object]:
    ...
    manifest = {
        "schema": "graphreader.ocr-production-composition-fixtures.v5",
        "revision": REVISION,
...
def load_sealed_archive(path: Path) -> tuple[CompositionScene, ...]:
    ...
    if split_fingerprint(result) != manifest["split_fingerprint"]:
        raise RuntimeError("OCR composition V5 fingerprint changed")
    return result
