...
"""Exact V10 plus bounded official tick rescue and crop-matched ambiguity execution."""
...
def evaluate_scenes(scenes: tuple[CompositionScene, ...], detector: DirectRunner, official: DirectRunner,
    ...
    for scene in scenes:
        ...
        accepted: list[tuple[int, Any, str, tuple[str, str, str, int] | None]] = []
        for candidate_index, (candidate, probability) in enumerate(zip(candidates, probabilities, strict=True)):
            if probability >= DETECTOR_THRESHOLD:
                accepted.append((candidate_index, candidate, "detector", None))
                continue
            ...
            official_text = official_result[2]
            official_role = _role(candidate.box, official_text)
            if _GRAPH_NUMBER.fullmatch(official_text.strip()) and official_role in {"x_tick", "y_tick"}:
                accepted.append((candidate_index, candidate, "official_tick_rescue", official_result))
        matched_truths = set()
        predictions = []
        scene_fp = scene_dup = 0
        for _, candidate, acceptance_route, precomputed_official in accepted:
            matches = [
            ...
            raw, conservative_text, official_text, ambiguity_changes = (
                precomputed_official
                if precomputed_official is not None
            ...
            totals["forbidden_official_rescue"] += int(
                acceptance_route == "official_tick_rescue" and truth.role not in {"x_tick", "y_tick"}
            )
        ...
        totals["tp"] += len(matched_truths)
        totals["fp"] += scene_fp
        totals["fn"] += scene_fn
        totals["dup"] += scene_dup
        cases.append({
            "scene_id": scene.scene_id,
            "source_raster_sha256": sha256(scene.raster.tobytes()).hexdigest(),
            ...
            "duplicate_region_count": scene_dup,
            "prohibited_structure_hits": scene_fp,
            "exact_detection": scene_fp == scene_fn == scene_dup == 0,
            "predictions": predictions,
        })
    total = sum(len(scene.truths) for scene in scenes)
    return {
        "scene_count": len(scenes),
        "truth_region_count": total,
        ...
        "duplicate_region_count": totals["dup"],
        "prohibited_structure_hits": totals["fp"],
        ...
        "official_tick_rescue_count": totals["rescues"],
        "ambiguity_changed_count": totals["ambiguity_changes"],
        "spacing_changed_count": totals["spacing_changes"],
