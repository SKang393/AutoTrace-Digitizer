...
"""Single-use exact-payload validation for composition V5."""
...
def _cpu_session(path: Path, input_name: str, output_name: str) -> ort.InferenceSession:
    ...
    if session.get_providers() != ["CPUExecutionProvider"]:
        raise RuntimeError("OCR composition V5 requires CPU only")
    if [item.name for item in session.get_inputs()] != [input_name] or [item.name for item in session.get_outputs()] != [output_name]:
        raise RuntimeError(f"OCR composition V5 ONNX I/O changed: {path}")
    return session
...
def evaluate_validation(*, detector_path: Path, official_path: Path, numeric_path: Path,
    ...
    if output_path.exists():
        raise RuntimeError(f"OCR composition V5 output exists: {output_path}")
    ...
    if sha256_file(seal_path) != config["validation_seal_sha256"]:
        raise RuntimeError("V5 validation seal changed")
    ...
    if sha256_file(archive) != seal["fixture_archive_sha256"] or sha256_file(manifest) != seal["private_manifest_sha256"]:
        raise RuntimeError("V5 validation fixtures changed")
    ...
    if hashes != expected or sha256_file(inference_yaml_path) != OFFICIAL_INFERENCE_YAML_SHA256:
        raise RuntimeError("V5 payload changed")
    ...
    if any(summary[key] != seal[key] for key in summary) or split_fingerprint(scenes) != seal["split_fingerprint"]:
        raise RuntimeError("V5 split changed")
    metrics = evaluate_scenes(scenes, detector, official, numeric, ambiguity, read_character_alphabet(inference_yaml_path))
    passed = (
        _passes(metrics)
        and detector.calls == len(scenes)
        and official.calls >= metrics["truth_region_count"]
        and numeric.calls == metrics["truth_region_count"]
        and ambiguity.calls > 0
    )
    report = {
        "schema": "graphreader.ocr-production-composition-validation-gate.v5",
        "task": TASK,
        ...
        "metrics": metrics,
        "direct_execution": {
            "detector": asdict(detector.evidence()),
            "official_recognizer": asdict(official.evidence()),
            "numeric_recognizer": asdict(numeric.evidence()),
            "ambiguity_recognizer": asdict(ambiguity.evidence()),
        },
