...
"""Freeze V11 train, selection, and hidden-public evidence."""
...
def freeze_split() -> dict[str, str]:
    ...
    if existing:
        raise RuntimeError("OCR V11 split freeze refuses overwrite: " + ", ".join(existing))
    protocol_path = REPO_ROOT / PROTOCOL
    if protocol_path.read_bytes() != canonical_json_bytes(protocol_configuration()):
        raise RuntimeError("OCR V11 committed preregistration changed before split freeze")
    private.mkdir(parents=True, exist_ok=False)
    for target in generated:
        target.parent.mkdir(parents=True, exist_ok=True)
    train, validation, public = build_split("train"), build_split("validation"), build_split("sealed_public")
    training_values, training_labels, role_labels, training_evidence = training_examples(
        train, negative_cap_per_scene=int(protocol_configuration()["training"]["negative_cap_per_scene"])
    )
    if training_evidence["validation_or_public_pixels_used"] is not False or training_evidence["v2_bytes_used"] is not False:
        raise RuntimeError("OCR V11 training examples violated frozen scope")
    selection = {
        "schema": "graphreader.ocr-composite-proposal-role-selection.v1", "task": TASK, "revision": REVISION,
        ...
        "training_tensor_shape": list(training_values.shape), "training_proposal_label_count": int(len(training_labels)),
        "training_role_label_count": int((role_labels >= 0).sum()), "synthetic_only": True,
        "private_or_article_images": False, "chandler_included": False, "generalization_label_included": False,
        "v2_bytes_used": False, "predecessor_fixture_bytes_reused": False,
        "validation_or_public_pixels_used_for_training": False, "sealed_public_truth_available_to_candidate": False,
    ...
    seal = {
        "schema": "graphreader.ocr-composite-proposal-role-sealed-test-seal.v1", "task": TASK,
        ...
        **proposal_summary(public), "split_fingerprint": split_fingerprint(public), "synthetic_only": True,
        "private_or_article_images": False, "chandler_included": False, "generalization_label_included": False,
        "v2_bytes_used": False, "predecessor_fixture_bytes_reused": False, "truth_hidden_from_candidate_runner": True,
    ...
    gate = {
        "schema": "graphreader.ocr-composite-proposal-role-gate-config.v1", "task": TASK,
    ...
    config = {
        "schema": "graphreader.ocr-composite-proposal-role-training.v1", "task": TASK, "revision": REVISION,
        ...
        "negative_cap_per_scene": training["negative_cap_per_scene"], "role_loss_weight": training["role_loss_weight"],
        **training_evidence, "selection_thresholds": protocol_configuration()["selection_thresholds"],
        ...
        "predecessor_fixture_bytes_reused": False, "v2_bytes_used": False,
