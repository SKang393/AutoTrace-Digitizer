...
"""Single-use P1 training for OCR V11."""
...
def _balanced_order(labels: torch.Tensor, generator: torch.Generator) -> torch.Tensor:
    ...
    if len(negative) == 0 or len(positive) == 0:
        raise RuntimeError("OCR V11 balanced sampler requires both classes")
...
def train_candidate(output_dir: Path) -> dict[str, object]:
    if output_dir.exists():
        raise RuntimeError(f"OCR V11 output exists: {output_dir}")
    ...
    try:
        if config["expected_runner_source_bundle_sha256"] != source_bundle_sha256(REPO_ROOT, RUNNER_SOURCE_PATHS):
            raise RuntimeError("OCR V11 runner sources changed")
        if config["selection_manifest_sha256"] != sha256_file(REPO_ROOT / SELECTION_PATH):
            raise RuntimeError("OCR V11 selection manifest changed")
        ...
        for key in (
            "scene_count", "negative_cap_per_scene", "proposal_count", "positive_proposal_count",
            "negative_proposal_count", "tensor_label_stream_sha256",
        ):
            if training_evidence[key] != config[key]:
                raise RuntimeError(f"OCR V11 training evidence changed: {key}")
        if training_evidence["validation_or_public_pixels_used"] is not False or training_evidence["v2_bytes_used"] is not False:
            raise RuntimeError("OCR V11 training data violated scope")
        seed, generator = int(config["seed"]), _configure(int(config["seed"]))
        model = CompositeProposalRoleNet(seed=seed)
        phase = "training"
        ...
        for epoch in range(int(config["epochs"])):
            ...
            for start in range(0, len(order), int(config["batch_size"])):
                ...
                role_loss = role_criterion(output[positive, 2:], batch_roles[positive]) if positive.any() else output[:, 2:].sum() * 0.0
        ...
        phase = "export"
        model.eval()
        checkpoint_path = output_dir / "graph-text-composite-proposal-role-v11-p1.pt"
        torch.save({"state_dict": model.state_dict()}, checkpoint_path)
        onnx_path = output_dir / "graph-text-composite-proposal-role-v11-p1.onnx"
        parity_values = torch.from_numpy(values[:256])
        _export(model, parity_values, onnx_path)
        session = ort.InferenceSession(str(onnx_path), providers=["CPUExecutionProvider"])
        if session.get_providers() != ["CPUExecutionProvider"]:
            raise RuntimeError("OCR V11 requires CPU selection execution")
        ...
        input_digest, output_digest, calls = __import__("hashlib").sha256(), __import__("hashlib").sha256(), 0
        ...
        report: dict[str, object] = {
            ...
            "loss_checkpoints": checkpoints, "checkpoint_path": checkpoint_path.relative_to(REPO_ROOT).as_posix(),
            "checkpoint_sha256": sha256_file(checkpoint_path), "onnx_path": onnx_path.relative_to(REPO_ROOT).as_posix(),
