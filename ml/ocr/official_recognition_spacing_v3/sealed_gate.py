...
"""Single-use truth-hidden public gate for conservative spacing V3."""
...
from .evaluate import (
    INFERENCE_YAML_PATH, MODEL_PATH, PUBLIC_SEAL_PATH, _load, _load_partition, evaluate_partition,
)
...
AUTHORIZED_PUBLIC_GATE_CONFIG_PATH = ROOT / "gates/sealed-public-p1-authorized.json"
...
def evaluate_public(*, output_path: Path) -> dict[str, object]:
    if output_path.exists():
        raise RuntimeError(f"Conservative spacing public output exists: {output_path}")
    ...
    if (
        ...
        raise RuntimeError("Conservative spacing public gate is not authorized by exact selection evidence")
    ...
    report: dict[str, object] = {
        "schema": "graphreader.ocr-conservative-spacing-public-gate.v1",
        "task": TASK, "revision": f"{REVISION}-public-v1", "candidate_id": CANDIDATE_ID,
        "status": "pass" if passed else "fail", "evaluation_count": 1,
        "model_onnx_sha256": MODEL_SHA256, "spacing_source_sha256": spacing_source_sha256,
        "selection_report_sha256": selection_report_sha256,
        "fixture_archive_sha256": sha256(archive).hexdigest(),
        "private_manifest_sha256": seal["private_manifest_sha256"],
        "provider": "CPUExecutionProvider", "gate_requirements": GATES,
        **evaluated, "seal_binding": gate.binding, "canonical_seal_key": gate.key,
        "marker_creation_evaluated": False, "production_approval": False, "release_eligible": False,
    }
