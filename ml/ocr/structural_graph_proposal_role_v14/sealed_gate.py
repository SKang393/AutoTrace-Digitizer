...
"""Single-use hidden-public gate for a selected OCR V14 candidate."""
...
from ml.markers.gate_seal import (
    acquire_gate_seal, canonical_json_bytes, complete_gate_seal,
    require_committed_sources, sha256_file,
)
...
from .protocol import (
    PUBLIC_REVISION, REVISION, ROLE_ACCURACY_MINIMUM,
    ROLE_CLASS_ACCURACY_MINIMUM, TASK, THRESHOLDS,
)
...
def evaluate_candidate(*, onnx_path: Path, selection_report_path: Path, output_path: Path) -> dict[str, object]:
    ...
    if output_path.exists():
        raise RuntimeError(f"OCR V14 public output exists: {output_path}")
    ...
    authorized_candidate = entry.get("public_gate_authorized_candidate_id") if entry else None
    ordinal = int(authorized_candidate[1:]) if isinstance(authorized_candidate, str) and authorized_candidate[1:].isdigit() else 0
    ...
    if (
        ...
        raise RuntimeError("OCR V14 public gate is not authorized by the canonical ledger")
    if (
        ...
        raise RuntimeError("Only the exact selected V14 candidate may open the public gate")
    ...
    if sha256_file(seal_path) != config["sealed_public_test_seal_sha256"]:
        raise RuntimeError("OCR V14 public seal changed")
    ...
    if sha256_file(archive_path) != seal["fixture_archive_sha256"] or sha256_file(manifest_path) != seal["private_manifest_sha256"]:
        raise RuntimeError("OCR V14 public fixture bytes changed")
    session = ort.InferenceSession(str(onnx_path), providers=["CPUExecutionProvider"])
    if session.get_providers() != ["CPUExecutionProvider"]:
        raise RuntimeError("OCR V14 public gate requires CPU execution only")
    ...
    def runner(values: np.ndarray) -> np.ndarray:
        nonlocal calls
        calls += 1
        return np.asarray(session.run(None, {"region_proposals": np.ascontiguousarray(values)})[0], dtype=np.float32)
    ...
    if any(summary[key] != seal[key] for key in summary) or split_fingerprint(scenes) != seal["split_fingerprint"]:
        raise RuntimeError("OCR V14 public fixtures violate the frozen contract")
    ...
    report: dict[str, object] = {
        "schema": "graphreader.ocr-structural-graph-proposal-role-public-gate.v1", "task": TASK,
        ...
        "selected_threshold": threshold, "metrics": metrics, "direct_execution_inference_calls": calls,
