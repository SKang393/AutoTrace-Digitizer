...
"""Freeze V14 train, selection, and hidden-public evidence."""
...
from .dataset import (
    build_split, proposal_summary, save_sealed_public_archive,
...
def freeze_split() -> dict[str, str]:
    ...
    if existing:
        raise RuntimeError("OCR V14 split freeze refuses overwrite: " + ", ".join(existing))
    protocol_path = REPO_ROOT / PROTOCOL
    if protocol_path.read_bytes() != canonical_json_bytes(protocol_configuration()):
        raise RuntimeError("OCR V14 committed preregistration changed before split freeze")
    ...
    if any(training_evidence[key] is not False for key in (
        ...
        raise RuntimeError("OCR V14 training examples violated the frozen scope")
    selection = {
        "schema": "graphreader.ocr-structural-graph-proposal-role-selection.v1", "task": TASK, "revision": REVISION,
    ...
    seal = {
        "schema": "graphreader.ocr-structural-graph-proposal-role-sealed-test-seal.v1", "task": TASK,
    ...
    gate = {
        "schema": "graphreader.ocr-structural-graph-proposal-role-gate-config.v1", "task": TASK,
    ...
    config = {
        "schema": "graphreader.ocr-structural-graph-proposal-role-training.v1", "task": TASK, "revision": REVISION,
