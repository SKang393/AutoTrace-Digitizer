...
def test_ledger_records_consumed_p3_and_public_gate() -> None:
    ledger = _load(LEDGER)
...
def test_public_gate_is_hidden_bound_and_unapproved() -> None:
    ...
    assert selected["status"] == "public_gate_passed_unapproved"
    assert selected["public_gate_authorized"] is False
    assert selected["public_gate_evaluations"] == 1
    assert selected["public_gate_archive_opened"] is True
    assert selected["public_exact_scene_count"] == selected["public_scene_count"] == 112
    assert selected["public_true_positives"] == 560
    assert selected["public_false_positives"] == selected["public_false_negatives"] == 0
    assert selected["production_approval"] is False
