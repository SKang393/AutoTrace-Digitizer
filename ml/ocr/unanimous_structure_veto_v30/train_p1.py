...
"""Single-use unanimous structure-veto training for OCR V30 P1."""
...
from .model import UnanimousStructureVetoProposalNet
...
TRAIN_ARCHIVE_PATH = Path("artifacts/production-validation/ocr-v30-train.zip")
SELECTION_ARCHIVE_PATH = Path("artifacts/production-validation/ocr-v30-selection.zip")
...
def _validate_stored_split(
    ...
    if (
        ...
        raise RuntimeError(f"OCR V30 {name} stored fixtures violate the seal")
...
def _load_role_parent_state() -> dict[str, torch.Tensor]:
    ...
    if not isinstance(state, dict) or not state:
        raise RuntimeError("OCR V30 role-parent checkpoint state is missing")
    return state


def _unanimous_objective(
    ...
    unanimous_loss, unanimous_parts = _proposal_objective(
        consensus, targets, class_weights, config,
    )
    route_logits = (attention, summary, local_veto)
    route_losses = tuple(
        _proposal_objective(route, targets, class_weights, config)[0]
        for route in route_logits
    )
    route_probabilities = tuple(torch.softmax(route, dim=1)[:, 1] for route in route_logits)
    pairwise_diversity = sum(
        nn.functional.mse_loss(route_probabilities[left], route_probabilities[right])
        for left, right in ((0, 1), (0, 2), (1, 2))
    ) / 3.0
    worst_route = torch.stack(route_losses).max()
    total = (
        float(config["unanimous_objective_weight"]) * unanimous_loss
        + float(config["per_route_objective_weight"]) * sum(route_losses)
        + float(config["worst_route_objective_weight"]) * worst_route
        - float(config["route_diversity_weight"]) * pairwise_diversity
    )
    return total, {
        "unanimous": unanimous_loss,
        "attention_route": route_losses[0],
        "summary_route": route_losses[1],
        "local_veto_route": route_losses[2],
        "worst_route": worst_route,
        "pairwise_route_diversity_reward": pairwise_diversity,
        **{f"unanimous_{key}": value for key, value in unanimous_parts.items()},
    }
...
def _trigger_is_terminal(trigger: dict[str, Any]) -> bool:
    ...
    return bool(
        trigger.get("schema") == "graphreader.ocr-dual-route-consensus-public-result.v1"
        and trigger.get("revision") == "graph-text-dual-route-consensus-proposal-v29"
        ...
        and metrics.get("exact_scene_count") == 222
        and metrics.get("true_positives") == 1792
        and metrics.get("false_positives") == 2
        and metrics.get("false_negatives") == 0
        and metrics.get("duplicate_region_count") == 0
        and metrics.get("prohibited_structure_hits") == 2
...
def preflight() -> dict[str, Any]:
    ...
    expected = {
        "schema": "graphreader.ocr-unanimous-structure-veto-candidate.v1",
        "task": TASK,
        "revision": REVISION,
        "candidate_id": CANDIDATE_ID,
        "candidate_limit": 3,
        "architecture": "unanimous-three-route-structure-veto-v1",
    ...
    for key, value in expected.items():
        if config.get(key) != value:
            raise RuntimeError(f"OCR V30 P1 config field mismatch: {key}")
    if config.get("expected_runner_source_bundle_sha256") != source_bundle_sha256(
        REPO_ROOT, RUNNER_SOURCE_PATHS,
    ):
        raise RuntimeError("OCR V30 P1 runner source bundle changed")
    if sha256_file(REPO_ROOT / SEAL_PATH) != config.get("split_seal_sha256"):
        raise RuntimeError("OCR V30 split seal changed before P1")
    seal = _read_json(REPO_ROOT / SEAL_PATH)
    required_seal = {
        "schema": "graphreader.ocr-unanimous-structure-veto-split-seal.v1",
        "revision": REVISION,
    ...
    for split, (path, key) in archive_bindings.items():
        ...
        if actual != seal["splits"][split]["archive_sha256"] or actual != config.get(key):
            raise RuntimeError(f"OCR V30 {split} archive changed before P1")
    exact_inputs = {
    ...
    for relative, expected_hash in exact_inputs.items():
        if sha256_file(REPO_ROOT / relative) != expected_hash:
            raise RuntimeError(f"OCR V30 frozen input changed: {relative}")
    if not _trigger_is_terminal(_read_json(REPO_ROOT / TRIGGER_RESULT_PATH)):
        raise RuntimeError("OCR V30 aggregate-only V29 trigger changed")
    if (REPO_ROOT / CANONICAL_OUTPUT).exists():
        raise RuntimeError("OCR V30 P1 output already exists")
    ...
    if (
        ...
        raise RuntimeError("OCR V30 P1 canonical authorization changed")
    return {"config": config, "head": head, "seal": seal}


def train_candidate(output_dir: Path) -> dict[str, object]:
    if output_dir.exists():
        raise RuntimeError(f"OCR V30 P1 output exists: {output_dir}")
    evidence = preflight()
    ...
    try:
        ...
        if train_values.shape[1:] != (FEATURE_COUNT,):
            raise RuntimeError("OCR V30 training evidence width changed")
        if train_crops.shape[1:] != (CROP_CHANNELS, CROP_HEIGHT, CROP_WIDTH):
            raise RuntimeError("OCR V30 training crop shape changed")
        if len(train_relations) != len(train_scenes) or len(train_slices) != len(train_scenes):
            raise RuntimeError("OCR V30 training relation stream changed")
        ...
        for key in ("proposal_count", "positive_proposal_count", "negative_proposal_count"):
            if training_evidence[key] != registered[key]:
                raise RuntimeError(f"OCR V30 incomplete training stream: {key}")

        phase = "unanimous_structure_veto_training"
        ...
        if not trainable or any(
            ...
            raise RuntimeError("OCR V30 frozen role-parent boundary changed")
        ...
        for epoch in range(int(config["epochs"])):
            ...
            for scene_index in torch.randperm(
                ...
                loss, components = _unanimous_objective(
                    consensus[0], attention[0], summary[0], local_veto[0], targets,
            ...
            print(
                f"OCR V30 P1 epoch {epoch + 1}/{config['epochs']} complete; "
                f"optimizer_steps={optimizer_steps}",
                flush=True,
            )
        if optimizer_steps != int(config["expected_optimizer_steps"]):
            raise RuntimeError("OCR V30 optimizer-step count changed")
        frozen_after = _parameter_stream_sha256(model.attention_route.role_parent)
        if frozen_after != frozen_before:
            raise RuntimeError("OCR V30 modified the frozen V24 role parent")

        phase = "checkpoint_and_onnx_export"
        checkpoint_path = output_dir / "graph-text-unanimous-structure-veto-v30-p1.pt"
        torch.save({"state_dict": model.state_dict()}, checkpoint_path)
        onnx_path = output_dir / "graph-text-unanimous-structure-veto-v30-p1.onnx"
        ...
        if {item.name for item in candidate_session.get_inputs()} != {
            ...
            raise RuntimeError("OCR V30 ONNX input identity changed")
        ...
        for key in ("proposal_count", "positive_proposal_count", "negative_proposal_count"):
            if selection_evidence[key] != registered[key]:
                raise RuntimeError(f"OCR V30 incomplete selection stream: {key}")
        ...
        parity_error = 0.0
        role_mismatches = 0
        pairwise_probability_gap_sum = 0.0
        pairwise_probability_gap_count = 0
        with torch.inference_mode():
            for scene_index, scene_slice in enumerate(selection_slices):
                ...
                probabilities = tuple(
                    torch.softmax(route, dim=2)[:, :, 1]
                    for route in (attention, summary, local_veto)
                )
                for left, right in ((0, 1), (0, 2), (1, 2)):
                    pairwise_probability_gap_sum += float(
                        torch.abs(probabilities[left] - probabilities[right]).sum()
                    )
                    pairwise_probability_gap_count += int(probabilities[left].numel())
                ...
                local_outputs.update(local_veto.numpy().tobytes(order="C"))
        ...
        selection_evidence.update({
            ...
            "local_veto_route_output_tensor_stream_sha256": local_outputs.hexdigest(),
            "candidate_onnx_sha256": sha256_file(onnx_path),
            "candidate_onnx_graph_optimization_level": "ORT_DISABLE_ALL",
            "deterministic_role_mismatch_count": role_mismatches,
            "mean_pairwise_route_probability_gap": (
                pairwise_probability_gap_sum / max(1, pairwise_probability_gap_count)
            ),
        })
        ...
        report: dict[str, object] = {
            "schema": "graphreader.ocr-unanimous-structure-veto-candidate-report.v1",
            "task": TASK,
            "revision": REVISION,
            "candidate_id": CANDIDATE_ID,
        ...
        failure = {
            "schema": "graphreader.ocr-unanimous-structure-veto-failure.v1",
            "task": TASK,
            "revision": REVISION,
            "candidate_id": CANDIDATE_ID,
...
__all__ = [
    ...
    "_trigger_is_terminal", "_unanimous_objective", "preflight",
    "train_candidate",
]
