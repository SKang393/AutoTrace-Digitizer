...
"""One-time fresh fixture freeze for OCR unanimous structure veto V30."""
...
ARCHIVE_PATHS = {
    "train": Path("artifacts/production-validation/ocr-v30-train.zip"),
    "validation": Path("artifacts/production-validation/ocr-v30-selection.zip"),
    "sealed_public": Path("artifacts/production-validation/ocr-v30-public.zip"),
}
SOURCE_PATHS = (
    ROOT / "PROTOCOL.json",
    ROOT / "dataset.py",
    ...
    Path("ml/ocr/dual_route_consensus_proposal_v29/model.py"),
    Path("ml/ocr/relational_neighborhood_proposal_v28/model.py"),
    Path("ml/ocr/relational_neighborhood_proposal_v28/model_p3.py"),
    Path("ml/ocr/relational_neighborhood_proposal_v28/pipeline.py"),
    Path("ml/ocr/relational_neighborhood_proposal_v28/relations.py"),
    Path("ml/ocr/relational_neighborhood_proposal_v28/train_p1.py"),
...
def _require_sources_at_head() -> None:
    for path in SOURCE_PATHS:
        ...
        if not local.is_file():
            raise RuntimeError(f"OCR V30 required source is missing: {relative}")
        completed = subprocess.run(
        ...
        if completed.returncode != 0 or completed.stdout != local.read_bytes():
            raise RuntimeError(
                f"OCR V30 source must be committed unchanged before freeze: {relative}"
            )
    expected = {
    ...
    for relative, digest in expected.items():
        ...
        if not path.is_file() or sha256_file(path) != digest:
            raise RuntimeError(f"OCR V30 exact prerequisite changed: {relative}")
...
def freeze() -> dict[str, object]:
    if (REPO_ROOT / SEAL_PATH).exists():
        raise RuntimeError("OCR V30 split identity is already frozen")
    existing = [
    ...
    if existing:
        raise RuntimeError(
            "OCR V30 archive already exists before freeze: " + ", ".join(existing)
        )
    ...
    for split, relative in ARCHIVE_PATHS.items():
        ...
        if len(hashes) != len(scenes):
            raise RuntimeError(f"OCR V30 {split} contains duplicate fixture bytes")
    ...
    if any(overlaps.values()):
        raise RuntimeError("OCR V30 split fixture bytes overlap")
...
def main() -> int:
    ...
    print(json.dumps({
        ...
        "cross_split_source_overlap_counts": result[
            "cross_split_source_overlap_counts"
        ],
        "splits": {
            name: {
