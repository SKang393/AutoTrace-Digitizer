...
"""Freeze fresh validation and truth-hidden public composition V3 fixtures."""
...
DEFAULT_PRIVATE_ROOT = ROOT / "artifacts/composition-v3-exact-execution-freeze-final2"
...
def freeze_split(
    *, private_root: Path = DEFAULT_PRIVATE_ROOT, protocol_path: Path = DEFAULT_PROTOCOL,
    ...
    targets = tuple(
        REPO_ROOT / path
        for path in (protocol_path, validation_seal_path, public_seal_path, gate_path, validation_gate_path)
    )
    ...
    if existing:
        raise RuntimeError("OCR composition V3 freeze refuses to overwrite evidence: " + ", ".join(existing))
    ...
    validation_seal = {
        "schema": "graphreader.ocr-production-composition-validation-seal.v3", **common,
    ...
    validation_gate = {
        "schema": "graphreader.ocr-production-composition-validation-gate-config.v3",
    ...
    public_seal = {
        "schema": "graphreader.ocr-production-composition-sealed-test-seal.v3", **common,
    ...
    public_gate = {
        "schema": "graphreader.ocr-production-composition-gate-config.v3",
