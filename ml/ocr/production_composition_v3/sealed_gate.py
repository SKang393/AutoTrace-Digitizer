...
"""Single-use truth-hidden public gate for composition V3."""
...
def evaluate_public(
    *, detector_path: Path, official_recognizer_path: Path, numeric_recognizer_path: Path,
    ...
    if output_path.exists():
        raise RuntimeError(f"OCR composition V3 public output exists: {output_path}")
    ...
    if (
        ...
        raise RuntimeError("OCR composition V3 public gate requires exact committed passing validation")
    ...
    if sha256_file(seal_path) != config["sealed_public_test_seal_sha256"]:
        raise RuntimeError("OCR composition V3 public seal changed")
    ...
    if sha256_file(archive_path) != seal["fixture_archive_sha256"] or sha256_file(manifest_path) != seal["private_manifest_sha256"]:
        raise RuntimeError("OCR composition V3 public fixture bytes changed")
    ...
    if not (
        ...
        raise RuntimeError("OCR composition V3 public exact payload or spacing source changed")
    ...
    if any(summary[key] != seal[key] for key in summary) or split_fingerprint(scenes) != seal["split_fingerprint"]:
        raise RuntimeError("OCR composition V3 public fixture bytes violate frozen contract")
    metrics = evaluate_scenes(scenes, detector, official, numeric, read_character_alphabet(inference_yaml_path))
    passed = (
        _passes(metrics) and metrics["marker_creation_evaluated"] is False and detector.calls == len(scenes)
        and official.calls == metrics["truth_region_count"] and numeric.calls > 0
    )
    report: dict[str, object] = {
        "schema": "graphreader.ocr-production-composition-public-gate.v3", "task": TASK,
