...
"""Single-use exact-payload validation for composition V4."""
...
EVALUATOR_SOURCE_PATHS = (
    ...
    Path("ml/ocr/ambiguity_context_classifier_v2/protocol.py"),
    ...
    Path("ml/ocr/production_composition_v1/dataset.py"), Path("ml/ocr/production_composition_v2/pipeline.py"),
    Path("ml/ocr/production_composition_v2/protocol.py"), Path("ml/markers/gate_seal.py"),
)
GATE_CONFIG = {
...
def _cpu_session(path: Path, input_name: str, output_name: str) -> ort.InferenceSession:
    ...
    options.execution_mode = ort.ExecutionMode.ORT_SEQUENTIAL; options.use_deterministic_compute = True
    session = ort.InferenceSession(str(path), sess_options=options, providers=["CPUExecutionProvider"])
    if session.get_providers() != ["CPUExecutionProvider"]: raise RuntimeError("OCR composition V4 requires CPU only")
    if [item.name for item in session.get_inputs()] != [input_name] or [item.name for item in session.get_outputs()] != [output_name]:
        raise RuntimeError(f"OCR composition V4 ONNX I/O changed: {path}")
    return session


def _passes(metrics: dict[str, object]) -> bool:
    return bool(metrics["exact_detection_scene_count"] == metrics["scene_count"]
...
def evaluate_validation(*, detector_path: Path, official_path: Path, numeric_path: Path,
    ...
    if output_path.exists(): raise RuntimeError(f"OCR composition V4 output exists: {output_path}")
    config = json.loads((REPO_ROOT / SPLIT_CONFIG_PATH).read_text(encoding="utf-8"))
    seal_path = REPO_ROOT / config["validation_seal_path"]
    if sha256_file(seal_path) != config["validation_seal_sha256"]: raise RuntimeError("V4 validation seal changed")
    seal = json.loads(seal_path.read_text(encoding="utf-8")); archive = REPO_ROOT / seal["fixture_archive_path"]
    ...
    if sha256_file(archive) != seal["fixture_archive_sha256"] or sha256_file(manifest) != seal["private_manifest_sha256"]:
        raise RuntimeError("V4 validation fixtures changed")
    spacing_path = REPO_ROOT / "ml/ocr/official_recognition_spacing_v3/spacing.py"
    hashes = {"detector_onnx_sha256": sha256_file(detector_path), "official_recognizer_onnx_sha256": sha256_file(official_path),
              "numeric_recognizer_onnx_sha256": sha256_file(numeric_path), "ambiguity_recognizer_onnx_sha256": sha256_file(ambiguity_path),
              "spacing_source_sha256": sha256_file(spacing_path)}
    ...
    ambiguity = DirectRunner(_cpu_session(ambiguity_path, "glyphs", "logits"), "glyphs")
    started = time.perf_counter(); scenes = load_sealed_archive(archive); summary = proposal_summary(scenes)
    if any(summary[k] != seal[k] for k in summary) or split_fingerprint(scenes) != seal["split_fingerprint"]: raise RuntimeError("V4 split changed")
    metrics = evaluate_scenes(scenes, detector, official, numeric, ambiguity, read_character_alphabet(inference_yaml_path))
    passed = (_passes(metrics) and detector.calls == len(scenes)
              and official.calls == metrics["truth_region_count"] and numeric.calls > 0 and ambiguity.calls > 0)
    report = {"schema": "graphreader.ocr-production-composition-validation-gate.v4", "task": TASK,
              ...
              "direct_execution": {"detector": asdict(detector.evidence()), "official_recognizer": asdict(official.evidence()),
                                   "numeric_recognizer": asdict(numeric.evidence()), "ambiguity_recognizer": asdict(ambiguity.evidence())},
              "gate_requirements": GATE_CONFIG, "seal_binding": gate.binding, "canonical_seal_key": gate.key,
              "elapsed_ms": round((time.perf_counter() - started) * 1000, 3)}
    output_path.parent.mkdir(parents=True, exist_ok=True); output_path.write_bytes(canonical_json_bytes(report))
    complete_gate_seal(gate, status=report["status"], report_sha256=sha256_file(output_path)); return report
...
def main() -> int:
    ...
    a = parser.parse_args(); report = evaluate_validation(detector_path=REPO_ROOT/a.detector, official_path=REPO_ROOT/a.official,
        numeric_path=REPO_ROOT/a.numeric, ambiguity_path=REPO_ROOT/a.ambiguity,
        inference_yaml_path=REPO_ROOT/a.inference_yaml, output_path=REPO_ROOT/a.output)
