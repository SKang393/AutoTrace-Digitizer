...
"""Fail-closed P1 runner for the DB-objective detector defect class."""
    ...
    *,
    ...
    try:
        ...
        if sha256_file(trigger_path) != config["trigger_result_sha256"]:
            raise RuntimeError("DB-objective trigger result changed")
        ...
        for epoch in range(int(config["epochs"])):
            ...
            for start in range(0, len(order), int(config["batch_size"])):
                ...
                total, shrink_loss, threshold_loss, binary_loss = db_objective_loss(
                ...
                optimizer.step()
                optimizer_steps += 1
                losses.append((float(total.detach()), float(shrink_loss.detach()), float(threshold_loss.detach()), float(binary_loss.detach())))
        ...
        report: dict[str, object] = {
            ...
            "trigger_result_sha256": config["trigger_result_sha256"],
