...
"""Fail-closed P3 runner for the DB-objective detector defect class."""
...
from .losses_p3 import db_objective_loss_with_boundary_negative
from .model import DbObjectiveTextRegionNet
...
def run_candidate(
    *,
    ...
    config_path: Path,
    output_dir: Path,
    runner_source_paths: tuple[Path, ...],
) -> dict[str, object]:
    ...
    try:
        for label, path_key, hash_key in (
            ("P2 result", "trigger_result_path", "trigger_result_sha256"),
            ("P2 report", "prior_report_path", "prior_report_sha256"),
            ("P2 diagnosis", "prior_diagnosis_path", "prior_diagnosis_sha256"),
        ):
            if sha256_file(REPO_ROOT / str(config[path_key])) != config[hash_key]:
                raise RuntimeError(f"DB-objective {label} changed")
        ...
        if sha256_file(seal_path) != config["sealed_public_test_seal_sha256"]:
            raise RuntimeError("DB-objective sealed-public seal changed")
        seal = json.loads(seal_path.read_text(encoding="utf-8"))
        if sha256_file(REPO_ROOT / str(seal["fixture_archive_path"])) != seal["fixture_archive_sha256"]:
            raise RuntimeError("DB-objective sealed-public archive changed")
        if config["training_sample_count"] != TRAIN_SAMPLE_COUNT:
            raise RuntimeError("DB-objective training sample count changed")
        expected_steps = int(config["epochs"]) * (TRAIN_SAMPLE_COUNT // BATCH_SIZE)
        if config["expected_optimizer_steps"] != expected_steps:
            raise RuntimeError("DB-objective optimizer-step budget changed")
        if config["maximum_downsampling_factor"] != 4:
            raise RuntimeError("DB-objective maximum downsampling changed")
        boundary_negative_weight = float(config["boundary_negative_loss_weight"])
        if boundary_negative_weight != 1.0:
            raise RuntimeError("DB-objective ignored-boundary negative weight changed")

        generator = _configure(int(config["seed"]))
        bgr, shrink_targets, shrink_masks, threshold_targets, threshold_masks = build_training_arrays()
        bgr_tensor = torch.from_numpy(bgr)
        shrink_target_tensor = torch.from_numpy(shrink_targets).float() / 255.0
        shrink_mask_tensor = torch.from_numpy(shrink_masks).float() / 255.0
        threshold_target_tensor = torch.from_numpy(threshold_targets).float() / 255.0
        threshold_mask_tensor = torch.from_numpy(threshold_masks).float() / 255.0
        ...
        if (
            ...
            raise RuntimeError("DB-objective ONNX preflight violated the probability contract")
        ...
        for epoch in range(int(config["epochs"])):
            ...
            for start in range(0, len(order), int(config["batch_size"])):
                ...
                total, shrink_loss, threshold_loss, binary_loss, boundary_negative = (
                    db_objective_loss_with_boundary_negative(
                        model.forward_training(inputs),
                        shrink_target_tensor.index_select(0, indices),
                        shrink_mask_tensor.index_select(0, indices),
                        threshold_target_tensor.index_select(0, indices),
                        threshold_mask_tensor.index_select(0, indices),
                        boundary_negative_loss_weight=boundary_negative_weight,
                    )
                )
                ...
                losses.append(
                    (
                        float(total.detach()),
                        float(shrink_loss.detach()),
                        float(threshold_loss.detach()),
                        float(binary_loss.detach()),
                        float(boundary_negative.detach()),
                    )
                )
                ...
                checkpoints.append(
                    {
                        ...
                        "balanced_shrink": sum(item[1] for item in losses) / len(losses),
                        "masked_threshold_l1": sum(item[2] for item in losses) / len(losses),
                        "differentiable_binary_dice": sum(item[3] for item in losses) / len(losses),
                        "ignored_boundary_negative_bce": sum(item[4] for item in losses) / len(losses),
                    }
                )
        if optimizer_steps != int(config["expected_optimizer_steps"]):
            raise RuntimeError("DB-objective optimizer-step count differs from preregistration")

        phase = "export"
        model.eval()
        checkpoint_path = output_dir / f"graph-text-db-objective-v5-{candidate_id.casefold()}.pt"
        torch.save(
            {"state_dict": model.state_dict(), "revision": REVISION, "candidate_id": candidate_id},
            checkpoint_path,
        )
        onnx_path = output_dir / f"graph-text-db-objective-v5-{candidate_id.casefold()}.onnx"
        ...
        report: dict[str, object] = {
            "schema": "graphreader.ocr-graph-text-db-objective-training-report.v1",
            "task": TASK,
            "revision": REVISION,
            "candidate_id": candidate_id,
            ...
            "prior_diagnosis_sha256": config["prior_diagnosis_sha256"],
        ...
        failure = {
            "schema": "graphreader.ocr-graph-text-db-objective-training-failure.v1",
            "task": TASK,
            "revision": REVISION,
            "candidate_id": candidate_id,
