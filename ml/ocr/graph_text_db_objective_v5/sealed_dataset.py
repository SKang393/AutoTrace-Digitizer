...
"""Truth-hidden archive writer for the DB-objective detector public split."""
...
def build_sealed_public_split() -> tuple[EvaluationFrame, ...]:
    return tuple(_evaluation_frame("sealed_public", index) for index in range(SEALED_TEXT_COUNT + SEALED_EXCLUSION_COUNT))


def save_sealed_public_archive(samples: tuple[EvaluationFrame, ...], path: Path) -> dict[str, object]:
    if path.exists():
        raise RuntimeError(f"Refusing to overwrite DB-objective sealed fixtures: {path}")
    ...
    return {
        "schema": "graphreader.ocr-graph-text-db-objective-private-manifest.v1",
