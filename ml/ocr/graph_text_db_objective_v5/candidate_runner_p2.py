...
"""Fail-closed P2 runner for the DB-objective detector defect class."""
...
from .losses_p2 import db_objective_loss_with_boundary_margin
    ...
    *,
    ...
    try:
        for label, path_key, hash_key in (
            ("P1 result", "trigger_result_path", "trigger_result_sha256"),
            ("P1 report", "prior_report_path", "prior_report_sha256"),
            ("P1 diagnosis", "prior_diagnosis_path", "prior_diagnosis_sha256"),
        ):
        ...
        if ceiling != 0.25 or margin_weight != 1.0:
            raise RuntimeError("DB-objective ignored-boundary margin changed")
        ...
        for epoch in range(int(config["epochs"])):
            ...
            for start in range(0, len(order), int(config["batch_size"])):
                ...
                total, shrink_loss, threshold_loss, binary_loss, margin = db_objective_loss_with_boundary_margin(
            ...
            if epoch in {0, int(config["epochs"]) // 2, int(config["epochs"]) - 1}:
                checkpoints.append({
                    ...
                    "ignored_boundary_margin": sum(item[4] for item in losses) / len(losses),
                })
