...
"""Single-use P1 training for the balanced-recall graph text detector."""
...
CONFIG_PATH = Path("ml/ocr/graph_text_balanced_v2/training/p1.json")
RUNNER_SOURCE_PATHS = (
...
def _loss(probabilities: torch.Tensor, target: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    return weighted_binary + (DICE_LOSS_WEIGHT * dice), weighted_binary, dice
...
def evaluate_frames(
    ...
    for frame in frames:
        ...
        if output.shape != expected_shape or not np.isfinite(output).all():
            raise RuntimeError(f"Balanced-recall runner returned invalid output for {frame.case_id}: {output.shape}")
        if float(output.min()) < 0.0 or float(output.max()) > 1.0:
            raise RuntimeError(f"Balanced-recall runner returned non-probability output for {frame.case_id}")
...
def train_candidate(output_dir: Path) -> dict[str, object]:
    ...
    try:
        if sha256_file(REPO_ROOT / config["trigger_result_path"]) != config["trigger_result_sha256"]:
            raise RuntimeError("Balanced-recall trigger evidence changed")
        ...
        if sha256_file(seal_path) != config["sealed_public_test_seal_sha256"]:
            raise RuntimeError("Balanced-recall sealed-public seal changed")
        seal = json.loads(seal_path.read_text(encoding="utf-8"))
        if sha256_file(REPO_ROOT / str(seal["fixture_archive_path"])) != seal["fixture_archive_sha256"]:
            raise RuntimeError("Balanced-recall sealed-public archive changed")
        ...
        if not np.isfinite(preflight_output).all() or float(preflight_output.min()) < 0.0 or float(preflight_output.max()) > 1.0:
            raise RuntimeError("Balanced-recall ONNX preflight violated the probability contract")
        ...
        phase = "export"
        model.eval()
        checkpoint_path = output_dir / "graph-text-balanced-recall-v2-p1.pt"
        torch.save({"state_dict": model.state_dict(), "revision": REVISION, "candidate_id": CANDIDATE_ID}, checkpoint_path)
        onnx_path = output_dir / "graph-text-balanced-recall-v2-p1.onnx"
