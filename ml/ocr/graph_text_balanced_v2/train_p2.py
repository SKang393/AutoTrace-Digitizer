...
"""Single-use hard-negative P2 training for the balanced-recall detector."""
...
CANDIDATE_ID = "P2"
CONFIG_PATH = Path("ml/ocr/graph_text_balanced_v2/training/p2.json")
CANONICAL_OUTPUT = Path("ml/ocr/graph_text_balanced_v2/artifacts/P2-run")
HARD_NEGATIVE_TOPK_FRACTION = 0.02
...
def _loss(
    ...
    hard_negative = torch.topk(flattened, k=topk_count, dim=1, largest=True, sorted=False).values.mean()
    total = (
        weighted_binary
...
def train_candidate(output_dir: Path) -> dict[str, object]:
    ...
    try:
        if sha256_file(REPO_ROOT / config["trigger_result_path"]) != config["trigger_result_sha256"]:
            raise RuntimeError("Balanced-recall P2 trigger evidence changed")
        ...
        if sha256_file(seal_path) != config["sealed_public_test_seal_sha256"]:
            raise RuntimeError("Balanced-recall P2 sealed-public seal changed")
        seal = json.loads(seal_path.read_text(encoding="utf-8"))
        if sha256_file(REPO_ROOT / str(seal["fixture_archive_path"])) != seal["fixture_archive_sha256"]:
            raise RuntimeError("Balanced-recall P2 sealed-public archive changed")
        if float(config["hard_negative_topk_fraction"]) != HARD_NEGATIVE_TOPK_FRACTION:
            raise RuntimeError("Balanced-recall P2 hard-negative fraction changed")
        if float(config["hard_negative_loss_weight"]) != HARD_NEGATIVE_LOSS_WEIGHT:
            raise RuntimeError("Balanced-recall P2 hard-negative weight changed")
        ...
        if (
            ...
            raise RuntimeError("Balanced-recall P2 ONNX preflight violated the probability contract")
        ...
        phase = "export"
        model.eval()
        checkpoint_path = output_dir / "graph-text-balanced-recall-v2-p2.pt"
        torch.save(
            {"state_dict": model.state_dict(), "revision": REVISION, "candidate_id": CANDIDATE_ID},
            checkpoint_path,
        )
        onnx_path = output_dir / "graph-text-balanced-recall-v2-p2.onnx"
