...
"""Truth-hidden archive writer for the balanced-recall detector public split."""
...
def save_sealed_public_archive(samples: tuple[EvaluationFrame, ...], path: Path) -> dict[str, object]:
    if path.exists():
        raise RuntimeError(f"Refusing to overwrite balanced-recall sealed fixtures: {path}")
    ...
    return {
        "schema": "graphreader.ocr-graph-text-balanced-recall-private-manifest.v1",
