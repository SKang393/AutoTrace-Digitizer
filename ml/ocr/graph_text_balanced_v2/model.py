...
"""Skip-connected export-safe graph text probability model."""
...
class BalancedRecallTextRegionNet(nn.Module):
    """Preserve thin glyph strokes while retaining a bounded receptive field."""

    def __init__(self, seed: int = 20260904) -> None:
        super().__init__()
    ...
    def forward(self, value: torch.Tensor) -> torch.Tensor:
        if not torch.onnx.is_in_onnx_export():
            if value.ndim != 4 or value.shape[1] != 3:
                raise ValueError("Balanced-recall detector input must be [batch,3,H,W]")
            if value.shape[2] % 8 or value.shape[3] % 8:
                raise ValueError("Balanced-recall detector dimensions must be divisible by eight")
...
__all__ = ["BalancedRecallTextRegionNet"]
