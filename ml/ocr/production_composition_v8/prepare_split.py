...
PRIVATE_ROOT = ROOT / "artifacts/composition-v8-freeze"
SPLIT_SOURCES = (
...
def freeze_split() -> dict[str, str]:
    ...
    if private.exists() or any((REPO_ROOT / path).exists() for path in targets):
        raise RuntimeError("V8 freeze refuses overwrite")
    ...
    for split, seal_name, archive_name, manifest_name in (
        ...
        seal = {
            "schema": f"graphreader.ocr-production-composition-{'validation' if split == 'validation' else 'sealed-test'}-seal.v8",
    ...
    validation_config = {
        "schema": "graphreader.ocr-production-composition-validation-gate-config.v8",
    ...
    public_config = {
        "schema": "graphreader.ocr-production-composition-gate-config.v8",
