...
"""Single-use exact-payload validation for composition V8."""
...
GATE_CONFIG = {
    ...
    "forbidden_zero_consensus_rescue_route_count": 0, "numeric_onnx_direct_execution_minimum": 1,
...
def _cpu_session(path: Path, input_name: str, output_name: str) -> ort.InferenceSession:
    ...
    if session.get_providers() != ["CPUExecutionProvider"]:
        raise RuntimeError("OCR composition V8 requires CPU only")
    if [item.name for item in session.get_inputs()] != [input_name] or [item.name for item in session.get_outputs()] != [output_name]:
        raise RuntimeError(f"OCR composition V8 ONNX I/O changed: {path}")
    return session
...
def _passes(metrics: dict[str, object]) -> bool:
    return bool(
        ...
        and metrics["forbidden_zero_consensus_rescue_route_count"] == 0
    )
...
def evaluate_validation(*, detector_path: Path, official_path: Path, numeric_path: Path,
    ...
    if output_path.exists():
        raise RuntimeError(f"OCR composition V8 output exists: {output_path}")
    ...
    if sha256_file(seal_path) != config["validation_seal_sha256"]:
        raise RuntimeError("V8 validation seal changed")
    ...
    if sha256_file(archive) != seal["fixture_archive_sha256"] or sha256_file(manifest) != seal["private_manifest_sha256"]:
        raise RuntimeError("V8 validation fixtures changed")
    hashes = {
    ...
    if hashes != expected or sha256_file(inference_yaml_path) != OFFICIAL_INFERENCE_YAML_SHA256:
        raise RuntimeError("V8 payload changed")
    ...
    if any(summary[key] != seal[key] for key in summary) or split_fingerprint(scenes) != seal["split_fingerprint"]:
        raise RuntimeError("V8 split changed")
    ...
    passed = (
        ...
        and runners[2].calls > 0 and runners[3].calls > 0
    )
    report = {
        "schema": "graphreader.ocr-production-composition-validation-gate.v8", "task": TASK,
