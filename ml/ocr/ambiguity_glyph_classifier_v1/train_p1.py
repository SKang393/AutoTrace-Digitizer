...
"""Single-use deterministic P1 training for the ambiguity glyph classifier."""
...
from .model import AmbiguityGlyphNet
...
RUNNER_SOURCE_PATHS = (
    ROOT / "dataset.py", ROOT / "model.py", ROOT / "protocol.py", ROOT / "train_p1.py",
...
def _metrics(logits: np.ndarray, labels: np.ndarray) -> dict[str, object]:
    ...
    return {
        "sample_count": len(labels), "correct_count": int(np.sum(predictions == labels)),
        "accuracy": accuracy, "macro_accuracy": float(np.mean(list(per_class.values()))),
...
def train_candidate(output_dir: Path) -> dict[str, object]:
    if output_dir.exists():
        raise RuntimeError(f"Ambiguity glyph candidate output exists: {output_dir}")
    ...
    try:
        ...
        public_seal_path = REPO_ROOT / str(config["sealed_public_test_seal_path"])
        trigger_path = REPO_ROOT / str(config["trigger_result_path"])
        if sha256_file(selection_path) != config["selection_manifest_sha256"]:
            raise RuntimeError("Ambiguity glyph selection manifest changed")
        if sha256_file(public_seal_path) != config["sealed_public_test_seal_sha256"]:
            raise RuntimeError("Ambiguity glyph public seal changed")
        if sha256_file(trigger_path) != config["trigger_result_sha256"]:
            raise RuntimeError("Ambiguity glyph trigger evidence changed")
        selection = json.loads(selection_path.read_text(encoding="utf-8"))
        public_seal = json.loads(public_seal_path.read_text(encoding="utf-8"))
        if sha256_file(REPO_ROOT / public_seal["fixture_archive_path"]) != public_seal["fixture_archive_sha256"]:
            raise RuntimeError("Ambiguity glyph sealed-public archive changed before training")
        ...
        if split_fingerprint("train") != selection["train_split_fingerprint"]:
            raise RuntimeError("Ambiguity glyph training split changed")
        if split_fingerprint("validation") != selection["validation_split_fingerprint"]:
            raise RuntimeError("Ambiguity glyph validation split changed")
        model = AmbiguityGlyphNet(seed=seed)
        ...
        phase = "export"
        checkpoint_path = output_dir / "graph-ambiguity-glyph-classifier-v1-p1.pt"
        torch.save({"state_dict": model.state_dict(), "class_order": list(GLYPHS)}, checkpoint_path)
        onnx_path = output_dir / "graph-ambiguity-glyph-classifier-v1-p1.onnx"
        ...
        report: dict[str, object] = {
            "schema": "graphreader.ocr-ambiguity-glyph-training-report.v1",
        ...
        failure = {
            "schema": "graphreader.ocr-ambiguity-glyph-training-failure.v1",
            "task": TASK, "revision": REVISION, "candidate_id": CANDIDATE_ID,
