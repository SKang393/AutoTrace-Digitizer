...
"""Single-use direct ONNX public gate for the ambiguity glyph classifier."""
...
from .dataset import build_partition, hash_bytes
...
def _load_public(seal: dict[str, object]) -> tuple[np.ndarray, np.ndarray, str]:
    ...
    if sha256_file(archive_path) != seal["fixture_archive_sha256"] or sha256_file(manifest_path) != seal["private_manifest_sha256"]:
        raise RuntimeError("Ambiguity glyph public split bytes changed")
    ...
    with zipfile.ZipFile(BytesIO(archive_path.read_bytes())) as fixtures:
        for sample in manifest["samples"]:
            ...
            if hash_bytes(source) != sample["source_sha256"]:
                raise RuntimeError(f"Ambiguity public fixture changed: {sample['sample_id']}")
            with Image.open(BytesIO(source)) as image:
                tensor = (1.0 - np.asarray(image.convert("L"), dtype=np.float32) / 255.0)[None, :, :]
            if tensor.shape != (1, IMAGE_SIZE, IMAGE_SIZE):
                raise RuntimeError("Ambiguity public tensor shape changed")
...
def evaluate_public(*, onnx_path: Path, selection_report_path: Path, output_path: Path) -> dict[str, object]:
    if output_path.exists():
        raise RuntimeError(f"Ambiguity glyph public output exists: {output_path}")
    ...
    if (
        ...
        raise RuntimeError("Ambiguity glyph public gate is not authorized by exact selection evidence")
    ...
    if session.get_providers() != ["CPUExecutionProvider"]:
        raise RuntimeError("Ambiguity glyph public gate requires CPUExecutionProvider only")
    input_digest = sha256()
    ...
    output: dict[str, object] = {
        "schema": "graphreader.ocr-ambiguity-glyph-public-gate.v1",
        ...
        "onnx_sha256": onnx_sha256, "selection_report_path": selection_report_path.relative_to(REPO_ROOT).as_posix(),
...
def main() -> int:
    ...
    report = evaluate_public(onnx_path=REPO_ROOT / args.onnx, selection_report_path=REPO_ROOT / args.selection_report,
