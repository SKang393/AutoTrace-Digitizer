...
"""Fresh stored procedural scenes for OCR V23."""
...
def _registration_scope(seed_shift: int = 0) -> Iterator[None]:
    with _RENDER_LOCK:
        ...
        def v23_registration(split: str):
            base = original(split)
        ...
        v21.split_registration = v23_registration
        try:
            yield
        finally:
...
def _apply_degradation(raster: np.ndarray, split: Split, index: int) -> np.ndarray:
    """Apply fresh split-specific photometric families without moving truth."""
    values = raster.astype(np.float32)
    source_ink = raster < 128
    height, width = raster.shape
    if split == "train":
        column = np.arange(width, dtype=np.float32)[None, :]
        values += np.cos((column + float(index % 19)) * 0.13) * float(1 + index % 4)
        values *= 0.90 + 0.02 * float(index % 8)
        step = float(2 + (index * 3) % 5)
        ...
        left = width // 4
        right = 3 * width // 4
        values[:, left:right] += 1.5 + float(index % 6)
    else:
        row = np.linspace(-3.5, 3.5, height, dtype=np.float32)[:, None]
        wave = np.cos(np.linspace(0.0, np.pi * 4.0, width, dtype=np.float32))[None, :]
        values += row + wave * float(1 + index % 3)
        band = 19 + (index * 31) % max(20, height - 20)
        values[band:band + 2] -= 2.5
...
def render_scene(split: Split, index: int) -> SceneSample:
    ...
    if index < 0 or index >= registered.scene_count:
        raise IndexError(f"OCR V23 scene index outside frozen {split} split: {index}")
    for attempt in range(48):
        source_index = (index + attempt * 59) % registered.scene_count
        with _registration_scope(seed_shift=attempt * 23_000_000):
            source = v21.render_scene(split, source_index)
        candidate = SceneSample(
            f"role-anchor-set-v23-{split}-{index:05d}",
            split,
        ...
        try:
            ...
            continue
        return candidate
    raise RuntimeError(f"OCR V23 could not render one production proposal per truth: {split}/{index}")
...
def save_archive(scenes: tuple[SceneSample, ...], path: Path) -> dict[str, object]:
    if path.exists():
        raise FileExistsError(f"OCR V23 fixture identity already exists: {path}")
    if not scenes or len({scene.split for scene in scenes}) != 1:
        raise RuntimeError("OCR V23 archive requires one nonempty split")
    ...
    manifest: dict[str, object] = {
        "schema": "graphreader.ocr-role-anchor-set-fixtures.v1",
        "revision": REVISION,
        "split": split,
...
def load_archive(path: Path) -> tuple[SceneSample, ...]:
    with ZipFile(path, "r") as archive:
        ...
        if (
            manifest.get("schema") != "graphreader.ocr-role-anchor-set-fixtures.v1"
            ...
            raise RuntimeError("OCR V23 stored fixture contract changed")
        scenes: list[SceneSample] = []
        for case in manifest["cases"]:
            payload = archive.read(case["source_path"])
            if sha256(payload).hexdigest() != case["source_sha256"]:
                raise RuntimeError(f"OCR V23 fixture hash mismatch: {case['scene_id']}")
    ...
    if len(result) != registered.scene_count:
        raise RuntimeError("OCR V23 stored fixture scene count changed")
