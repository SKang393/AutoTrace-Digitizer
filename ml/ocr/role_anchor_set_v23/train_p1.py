...
"""Single-use CPU training and visible selection for OCR V23 P1."""
...
def _proposal_objective(
    ...
    """Combine balanced cross entropy with preregistered scene-extrema margins."""
    ...
    if negative.numel() == 0 or positive.numel() == 0:
        raise RuntimeError("OCR V23 P1 requires positive and negative proposals in every scene")
...
def _balanced_class_weights(
    ...
    if counts.shape != (class_count,) or torch.any(counts == 0):
        raise RuntimeError(f"OCR V23 P1 requires every {evidence_name} class")
...
def _cpu_session(path: Path) -> ort.InferenceSession:
    ...
    if session.get_providers() != ["CPUExecutionProvider"]:
        raise RuntimeError("OCR V23 requires CPUExecutionProvider only")
    return session
...
def preflight() -> dict[str, Any]:
    ...
    expected = {
        ...
        "protocol_path": (ROOT / "PROTOCOL.json").as_posix(),
        "protocol_sha256": "12194c6b7761e8e1068bc8cc087a319dff1e6a482340945281816b3a7874d847",
        "trigger_result_path": "ml/ocr/scene_evidence_attention_v22/P3_RESULT.json",
        "trigger_result_sha256": "cd8903db6ec7c9e46e2e49938b6552bf09521e53aa277d01f514f460f566afe9",
        "split_source_commit": "ba3ff1737fa0bf7b63c8343539e036a8d493db97",
        "split_source_bundle_sha256": "84c6228af099649f6cdfae18423654a8f7f422cb74a5701fca11c764711e3a68",
    ...
    for key, value in expected.items():
        if config.get(key) != value:
            raise RuntimeError(f"OCR V23 P1 config field mismatch: {key}")
    if config.get("selection_thresholds") != list(THRESHOLDS):
        raise RuntimeError("OCR V23 P1 thresholds changed")
    if int(config["expected_optimizer_steps"]) > 1280:
        raise RuntimeError("OCR V23 P1 optimizer budget exceeds the preregistration")
    if config.get("expected_runner_source_bundle_sha256") != source_bundle_sha256(
        REPO_ROOT, RUNNER_SOURCE_PATHS,
    ):
        raise RuntimeError("OCR V23 P1 runner source bundle changed")
    if sha256_file(REPO_ROOT / SEAL_PATH) != config.get("split_seal_sha256"):
        raise RuntimeError("OCR V23 split seal changed before P1")
    ...
    for split, (path, config_key) in archive_bindings.items():
        ...
        if actual != seal["splits"][split]["archive_sha256"] or actual != config.get(config_key):
            raise RuntimeError(f"OCR V23 {split} archive changed before P1")
    exact_inputs = {
    ...
    if (REPO_ROOT / CANONICAL_OUTPUT).exists():
        raise RuntimeError("OCR V23 P1 output already exists")
    return {"config": config, "head": head, "seal": seal}


def _feature_groups(records: tuple[ProposalRecord, ...], scene_count: int) -> tuple[np.ndarray, ...]:
    groups = tuple(
        np.asarray(
    ...
    if any(len(indices) == 0 for indices in groups):
        raise RuntimeError("OCR V23 production proposal stream omitted an entire scene")
    flattened = np.concatenate(groups)
    if not np.array_equal(flattened, np.arange(len(records), dtype=np.int64)):
        raise RuntimeError("OCR V23 production proposal stream is not scene-contiguous")
    return groups


def _role_targets(scenes: tuple[Any, ...], records: tuple[ProposalRecord, ...]) -> np.ndarray:
...
def _calibrated_records(
    ...
    if outputs.shape != (len(records), 2 + len(ROLE_ORDER)):
        raise RuntimeError("OCR V23 calibrator output stream shape changed")
    return tuple(
...
def train_candidate(output_dir: Path) -> dict[str, object]:
    if output_dir.exists():
        raise RuntimeError(f"OCR V23 P1 output exists: {output_dir}")
    evidence = preflight()
    ...
    try:
        ...
        if train_values.shape[1:] != (FEATURE_COUNT,):
            raise RuntimeError("OCR V23 P1 training feature width changed")
        ...
        for key in ("proposal_count", "positive_proposal_count", "negative_proposal_count"):
            if training_evidence[key] != registered_training[key]:
                raise RuntimeError(f"OCR V23 P1 incomplete training proposal stream: {key}")
        ...
        if int(train_labels.sum()) != expected_truths or np.any(train_roles[train_labels == 1] < 0):
            raise RuntimeError("OCR V23 P1 production stream omitted a training truth")

        phase = "role_anchor_training"
        generator = _configure(int(config["seed"]))
        model = RoleAnchorSetNet(seed=int(config["seed"]))
        ...
        if positives == 0 or negatives == 0:
            raise RuntimeError("OCR V23 P1 requires positive and negative production proposals")
        proposal_weights = _balanced_class_weights(
            torch.from_numpy(train_labels), 2, "proposal",
        )
        positive_role_targets = torch.from_numpy(train_roles[train_labels == 1])
        role_weights = _balanced_class_weights(
            positive_role_targets, len(ROLE_ORDER), "role",
        )
        ...
        for epoch in range(int(config["epochs"])):
            ...
            print(
                f"OCR V23 P1 epoch {epoch + 1}/{config['epochs']} complete; "
                f"optimizer_steps={optimizer_steps}",
                flush=True,
            )
        if optimizer_steps != int(config["expected_optimizer_steps"]):
            raise RuntimeError("OCR V23 P1 optimizer-step count changed")

        phase = "checkpoint_and_onnx_export"
        checkpoint_path = output_dir / "graph-text-role-anchor-set-v23-p1.pt"
        torch.save({"state_dict": model.state_dict()}, checkpoint_path)
        onnx_path = output_dir / "graph-text-role-anchor-set-v23-p1.onnx"
        ...
        for key in ("proposal_count", "positive_proposal_count", "negative_proposal_count"):
            if selection_evidence[key] != registered_selection[key]:
                raise RuntimeError(f"OCR V23 P1 incomplete selection proposal stream: {key}")
        ...
        if sum(record.truth_index >= 0 for record in selection_records) != expected_truths:
            raise RuntimeError("OCR V23 P1 production stream omitted a validation truth")
        ...
        with torch.inference_mode():
            for indices in selection_groups:
                ...
                torch_outputs.append(expected_output[0])
