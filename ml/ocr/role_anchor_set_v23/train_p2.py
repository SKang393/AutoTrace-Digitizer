...
"""Single-use CPU training and visible selection for OCR V23 P2."""
...
def _proposal_objective(
    ...
    if negative.numel() == 0 or positive.numel() == 0:
        raise RuntimeError("OCR V23 P2 requires positive and negative proposals in every scene")
...
def preflight() -> dict[str, Any]:
    ...
    expected = {
        ...
        "objective": "class_balanced_cross_entropy_plus_strict_negative_scene_extrema_margin_v1",
        "model_license": "Apache-2.0",
        "seed": 2_608_162_302,
        "learning_rate": 0.0003,
        "weight_decay": 0.0001,
        "negative_acceptance_probability_maximum": 0.01,
        "negative_scene_extrema_margin_weight": 4.0,
    ...
    for key, value in expected.items():
        if config.get(key) != value:
            raise RuntimeError(f"OCR V23 P2 config field mismatch: {key}")
    if config.get("selection_thresholds") != list(THRESHOLDS):
        raise RuntimeError("OCR V23 P2 thresholds changed")
    if int(config["expected_optimizer_steps"]) > 1280:
        raise RuntimeError("OCR V23 P2 optimizer budget exceeds the preregistration")
    if config.get("expected_runner_source_bundle_sha256") != source_bundle_sha256(
        REPO_ROOT, RUNNER_SOURCE_PATHS,
    ):
        raise RuntimeError("OCR V23 P2 runner source bundle changed")
    if sha256_file(REPO_ROOT / PROTOCOL_PATH) != config.get("protocol_sha256"):
        raise RuntimeError("OCR V23 P2 protocol changed")
    if sha256_file(REPO_ROOT / P1_RESULT_PATH) != P1_RESULT_SHA256:
        raise RuntimeError("OCR V23 P1 aggregate result changed before P2")
    ...
    for key, value in p1_expected.items():
        if p1_result.get(key) != value:
            raise RuntimeError(f"OCR V23 P1 result is not a fail-closed P2 trigger: {key}")
    if sha256_file(REPO_ROOT / SEAL_PATH) != config.get("split_seal_sha256"):
        raise RuntimeError("OCR V23 split seal changed before P2")
    ...
    for split, (path, config_key) in archive_bindings.items():
        ...
        if actual != seal["splits"][split]["archive_sha256"] or actual != config.get(config_key):
            raise RuntimeError(f"OCR V23 {split} archive changed before P2")
    exact_inputs = {
...
def train_candidate(output_dir: Path) -> dict[str, object]:
    ...
    try:
        ...
        if train_values.shape[1:] != (FEATURE_COUNT,):
            raise RuntimeError("OCR V23 P2 training feature width changed")
        ...
        for key in ("proposal_count", "positive_proposal_count", "negative_proposal_count"):
            if training_evidence[key] != registered_training[key]:
                raise RuntimeError(f"OCR V23 P2 incomplete training proposal stream: {key}")
        ...
        if int(train_labels.sum()) != expected_truths or np.any(train_roles[train_labels == 1] < 0):
            raise RuntimeError("OCR V23 P2 production stream omitted a training truth")
        ...
        if positives == 0 or negatives == 0:
            raise RuntimeError("OCR V23 P2 requires positive and negative production proposals")
        ...
        role_weights = common._balanced_class_weights(
        ...
        for epoch in range(int(config["epochs"])):
            ...
            print(
                f"OCR V23 P2 epoch {epoch + 1}/{config['epochs']} complete; "
                f"optimizer_steps={optimizer_steps}",
                flush=True,
            )
        if optimizer_steps != int(config["expected_optimizer_steps"]):
            raise RuntimeError("OCR V23 P2 optimizer-step count changed")

        phase = "checkpoint_and_onnx_export"
        checkpoint_path = output_dir / "graph-text-role-anchor-set-v23-p2.pt"
        torch.save({"state_dict": model.state_dict()}, checkpoint_path)
        onnx_path = output_dir / "graph-text-role-anchor-set-v23-p2.onnx"
        ...
        for key in ("proposal_count", "positive_proposal_count", "negative_proposal_count"):
            if selection_evidence[key] != registered_selection[key]:
                raise RuntimeError(f"OCR V23 P2 incomplete selection proposal stream: {key}")
        ...
        if sum(record.truth_index >= 0 for record in selection_records) != expected_truths:
            raise RuntimeError("OCR V23 P2 production stream omitted a validation truth")
