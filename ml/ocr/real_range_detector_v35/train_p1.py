...
"""Canonical-budget synthetic train/dev runner for the V35 detector."""
...
from .protocol import (
    BATCH_SIZE, CANONICAL_OUTPUT, EPOCHS, LEARNING_RATE, ONNX_PARITY_TOLERANCE,
    MODEL_LICENSE, REVISION, SEED, TASK,
    V32_RESULT_PATH, V32_RESULT_SHA256, V33_RESULT_PATH, V33_RESULT_SHA256,
    V34_DIAGNOSTIC, V34_DIAGNOSTIC_SHA256, WEIGHT_DECAY,
)
from ml.ocr.real_range_classifier_finetune_v32.dataset import build_split, split_fingerprint


REPO_ROOT = Path(__file__).resolve().parents[3]
ROOT = Path("ml/ocr/real_range_detector_v35")
...
def _export(model: nn.Module, sample: torch.Tensor, path: Path) -> None:
    torch.onnx.export(
        ...
        input_names=["source_tiles"], output_names=["text_logits"],
...
def train_candidate(output_dir: Path) -> dict[str, object]:
    if output_dir.exists():
        raise RuntimeError(f"V35 output already exists: {output_dir}")
    config = json.loads((REPO_ROOT / CONFIG_PATH).read_text(encoding="utf-8"))
    if config["expected_runner_source_bundle_sha256"] == "ROOT_TO_FILL_SOURCE_BUNDLE_SHA256":
        raise RuntimeError("V35 source bundle hash must be filled after source review")
    ...
    try:
        for path, expected in (
            (V34_DIAGNOSTIC, V34_DIAGNOSTIC_SHA256),
            (V32_RESULT_PATH, V32_RESULT_SHA256),
            (V33_RESULT_PATH, V33_RESULT_SHA256),
        ):
            if sha256_file(REPO_ROOT / path) != expected:
                raise RuntimeError(f"V35 trigger evidence changed: {path.as_posix()}")
        ...
        if best_state is None:
            raise RuntimeError("V35 training produced no selected train-loss state")
        model.load_state_dict(best_state)
        model.eval()
        phase = "export"
        checkpoint_path = output_dir / "graph-text-real-range-detector-v35-p1.pt"
        torch.save({"state_dict": model.state_dict(), "seed": SEED, "best_train_epoch": best_epoch}, checkpoint_path)
        onnx_path = output_dir / "graph-text-real-range-detector-v35-p1.onnx"
        _export(model, tensors, onnx_path)
        session = ort.InferenceSession(str(onnx_path), providers=["CPUExecutionProvider"])
        if session.get_providers() != ["CPUExecutionProvider"]:
            raise RuntimeError("V35 parity requires CPUExecutionProvider")
        ...
        def run_model(input_values: np.ndarray) -> np.ndarray:
            return np.asarray(session.run(["text_logits"], {"source_tiles": np.ascontiguousarray(input_values, dtype=np.float32)})[0], dtype=np.float32)
        ...
        report = {
            "schema": "graphreader.ocr-real-range-detector-v35-result.v1",
        ...
        failure = {
            "schema": "graphreader.ocr-real-range-detector-v35-failure.v1",
            "task": TASK, "revision": REVISION, "candidate_id": "P1",
