...
"""Single-use P3 regularized training for component-context detector V7."""
...
from .model import ComponentContextNet
...
CANDIDATE_ID = "P3"
CONFIG_PATH = Path("ml/ocr/component_context_detector_v7/training/p3.json")
CANONICAL_OUTPUT = Path("ml/ocr/component_context_detector_v7/artifacts/P3-run")
RUNNER_SOURCE_PATHS = (
    Path("ml/ocr/component_context_detector_v7/dataset.py"),
    Path("ml/ocr/component_context_detector_v7/model.py"),
    Path("ml/ocr/component_context_detector_v7/pipeline.py"),
    Path("ml/ocr/component_context_detector_v7/protocol.py"),
    Path("ml/ocr/component_context_detector_v7/train_p1.py"),
    Path("ml/ocr/component_context_detector_v7/train_p2.py"),
    Path("ml/ocr/component_context_detector_v7/train_p3.py"),
...
def train_candidate(output_dir: Path) -> dict[str, object]:
    ...
    try:
        ...
        if sha256_file(selection_path) != config["selection_manifest_sha256"]:
            raise RuntimeError("OCR V7 selection manifest checksum changed")
        ...
        if sha256_file(sealed_path) != config["sealed_public_test_seal_sha256"]:
            raise RuntimeError("OCR V7 public seal checksum changed")
        sealed = json.loads(sealed_path.read_text(encoding="utf-8"))
        if sha256_file(REPO_ROOT / sealed["fixture_archive_path"]) != sealed["fixture_archive_sha256"]:
            raise RuntimeError("OCR V7 sealed-public archive changed before P3")
        p2_result_path = REPO_ROOT / config["p2_result_path"]
        if sha256_file(p2_result_path) != config["p2_result_sha256"]:
            raise RuntimeError("OCR V7 P2 result checksum changed")
        p2_result = json.loads(p2_result_path.read_text(encoding="utf-8"))
        if p2_result.get("status") != "failed_selection" or p2_result.get("public_gate_archive_opened") is not False:
            raise RuntimeError("OCR V7 P3 requires the exact failed, public-unopened P2 result")
        ...
        if sha256_file(source_checkpoint) != config["source_checkpoint_sha256"]:
            raise RuntimeError("OCR V7 P1 source checkpoint checksum changed")
        ...
        for epoch in range(int(config["epochs"])):
            ...
            if epoch in {0, int(config["epochs"]) // 2, int(config["epochs"]) - 1}:
                checkpoints.append({"epoch": epoch + 1, "label_smoothed_cross_entropy": sum(losses) / len(losses)})
        phase = "selection"
        model.eval()
        comparisons = [
        ...
        phase = "export"
        checkpoint_path = output_dir / "graph-text-component-context-v7-p3.pt"
        torch.save({"state_dict": model.state_dict(), "selected_threshold": selected["threshold"]}, checkpoint_path)
        onnx_path = output_dir / "graph-text-component-context-v7-p3.onnx"
        ...
        report: dict[str, object] = {
            "schema": "graphreader.ocr-component-context-training-report.v1",
            "task": TASK,
            "revision": REVISION,
            "candidate_id": CANDIDATE_ID,
            ...
            "loss_revision": config["loss_revision"],
        ...
        failure = {
            "schema": "graphreader.ocr-component-context-training-failure.v1",
            "task": TASK,
            "revision": REVISION,
            "candidate_id": CANDIDATE_ID,
