...
"""Single-use public gate for the selected OCR component-context V7 candidate."""
...
RESULT_PATH = Path("ml/ocr/component_context_detector_v7/P2_RESULT.json")
SPLIT_CONFIG_PATH = Path("ml/ocr/component_context_detector_v7/gates/sealed-public-v1.json")
EVALUATOR_SOURCE_PATHS = (
    Path("ml/ocr/component_context_detector_v7/dataset.py"),
    Path("ml/ocr/component_context_detector_v7/pipeline.py"),
    Path("ml/ocr/component_context_detector_v7/protocol.py"),
    Path("ml/ocr/component_context_detector_v7/sealed_gate.py"),
...
def _entry(ledger: dict[str, object]) -> dict[str, object]:
    ...
    if entry is None:
        raise RuntimeError("OCR component-context V7 budget entry is missing")
    return entry
...
def evaluate_candidate(*, onnx_path: Path, selection_report_path: Path, output_path: Path) -> dict[str, object]:
    ...
    if output_path.exists():
        raise RuntimeError(f"OCR V7 public output already exists: {output_path}")
    ...
    if (
        entry.get("status") != "candidate_2_selected_public_gate_pending"
        ...
        or entry.get("public_gate_authorized_candidate_id") != "P2"
        or entry.get("public_gate_evaluations") != 0
        or entry.get("public_gate_archive_opened") is not False
        or entry.get("candidate_onnx_sha256", {}).get("P2") != onnx_sha256
        or entry.get("p2_training_report_sha256") != selection_report_sha256
        or entry.get("p2_result_sha256") != sha256_file(REPO_ROOT / RESULT_PATH)
    ):
        raise RuntimeError("OCR component-context V7 public gate is not authorized by the canonical ledger")
    ...
    if (
        ...
        raise RuntimeError("Only the exact validation-selected OCR V7 candidate may open the public gate")
    ...
    if sha256_file(seal_path) != split_config["sealed_public_test_seal_sha256"]:
        raise RuntimeError("OCR V7 public seal differs from the frozen gate configuration")
    ...
    if sha256_file(archive_path) != seal_data["fixture_archive_sha256"]:
        raise RuntimeError("OCR V7 sealed fixture archive checksum mismatch")
    if sha256_file(private_manifest_path) != seal_data["private_manifest_sha256"]:
        raise RuntimeError("OCR V7 sealed private manifest checksum mismatch")
    ...
    if (
        ...
        raise RuntimeError("OCR V7 sealed private manifest violates the frozen data contract")

    session = ort.InferenceSession(str(onnx_path), providers=["CPUExecutionProvider"])
    if session.get_providers() != ["CPUExecutionProvider"]:
        raise RuntimeError("OCR V7 public gate requires CPUExecutionProvider only")
    if [item.name for item in session.get_inputs()] != ["region_proposals"]:
        raise RuntimeError("OCR V7 public gate found an unexpected ONNX input contract")
    ...
    if (
        ...
        raise RuntimeError("OCR V7 sealed fixture bytes do not reproduce the frozen scene contract")
    ...
    passed = (
        ...
        and metrics["inference_calls"] == direct_calls == seal_data["scene_count"]
    )
    report: dict[str, object] = {
        "schema": "graphreader.ocr-component-context-public-gate.v1",
        "task": TASK,
        "revision": PUBLIC_REVISION,
