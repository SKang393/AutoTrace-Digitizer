...
"""Single-use P2 loss-calibration training for component-context detector V7."""
...
CANDIDATE_ID = "P2"
CONFIG_PATH = Path("ml/ocr/component_context_detector_v7/training/p2.json")
CANONICAL_OUTPUT = Path("ml/ocr/component_context_detector_v7/artifacts/P2-run")
RUNNER_SOURCE_PATHS = (
...
def train_candidate(output_dir: Path) -> dict[str, object]:
    ...
    try:
        ...
        if sha256_file(REPO_ROOT / sealed["fixture_archive_path"]) != sealed["fixture_archive_sha256"]:
            raise RuntimeError("OCR V7 sealed-public archive changed before P2")
        p1_result_path = REPO_ROOT / config["p1_result_path"]
        if sha256_file(p1_result_path) != config["p1_result_sha256"]:
            raise RuntimeError("OCR V7 P1 result checksum changed")
        p1_result = json.loads(p1_result_path.read_text(encoding="utf-8"))
        if p1_result.get("status") != "failed_selection" or p1_result.get("public_gate_archive_opened") is not False:
            raise RuntimeError("OCR V7 P2 requires the exact failed, public-unopened P1 result")
        ...
        for epoch in range(int(config["epochs"])):
            ...
            if epoch in {0, int(config["epochs"]) // 2, int(config["epochs"]) - 1}:
                checkpoints.append({"epoch": epoch + 1, "unweighted_cross_entropy": sum(losses) / len(losses)})
        phase = "selection"
        model.eval()
        comparisons = [
        ...
        phase = "export"
        checkpoint_path = output_dir / "graph-text-component-context-v7-p2.pt"
        torch.save({"state_dict": model.state_dict(), "selected_threshold": selected["threshold"]}, checkpoint_path)
        onnx_path = output_dir / "graph-text-component-context-v7-p2.onnx"
        ...
        report: dict[str, object] = {
            ...
            "loss_revision": "unweighted-cross-entropy-finetune-v1",
