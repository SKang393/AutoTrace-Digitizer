...
"""Single-use P1 training for the preregistered component-context detector V7."""
...
CANDIDATE_ID = "P1"
CONFIG_PATH = Path("ml/ocr/component_context_detector_v7/training/p1.json")
CANONICAL_OUTPUT = Path("ml/ocr/component_context_detector_v7/artifacts/P1-run")
RUNNER_SOURCE_PATHS = (
...
def train_candidate(output_dir: Path) -> dict[str, object]:
    ...
    try:
        ...
        if sha256_file(REPO_ROOT / sealed["fixture_archive_path"]) != sealed["fixture_archive_sha256"]:
            raise RuntimeError("OCR V7 sealed-public archive changed before training")
        ...
        model = ComponentContextNet(seed=seed)
        ...
        counts = np.bincount(training_labels, minlength=2).astype(np.float32)
        class_weights = len(training_labels) / (2.0 * counts)
        ...
        for epoch in range(int(config["epochs"])):
            ...
            if epoch in {0, int(config["epochs"]) // 2, int(config["epochs"]) - 1}:
                checkpoints.append({"epoch": epoch + 1, "weighted_cross_entropy": sum(losses) / len(losses)})
        phase = "selection"
        model.eval()
        comparisons = [
        ...
        phase = "export"
        checkpoint_path = output_dir / "graph-text-component-context-v7-p1.pt"
        torch.save({"state_dict": model.state_dict(), "selected_threshold": selected["threshold"]}, checkpoint_path)
        onnx_path = output_dir / "graph-text-component-context-v7-p1.onnx"
        ...
        report: dict[str, object] = {
            ...
            "training_class_weights": class_weights.tolist(),
