...
"""Freeze V9 train, validation, and truth-hidden public evidence."""
...
DEFAULT_PRIVATE_ROOT = Path("ml/ocr/component_recall_detector_v9/artifacts/split-freeze")
DEFAULT_PROTOCOL = Path("ml/ocr/component_recall_detector_v9/PROTOCOL.json")
DEFAULT_SELECTION = Path("ml/ocr/component_recall_detector_v9/SELECTION_MANIFEST.json")
DEFAULT_SEAL = Path("ml/ocr/component_recall_detector_v9/SEALED_PUBLIC_TEST_SEAL.json")
DEFAULT_GATE = Path("ml/ocr/component_recall_detector_v9/gates/sealed-public-v1.json")
SPLIT_SOURCE_PATHS = (
    Path("ml/ocr/component_recall_detector_v9/dataset.py"),
    Path("ml/ocr/component_recall_detector_v9/prepare_split.py"),
...
def freeze_split(*, private_root: Path, protocol_path: Path, selection_path: Path, seal_path: Path, gate_path: Path) -> dict[str, str]:
    ...
    if existing:
        raise RuntimeError("OCR V9 split freeze refuses to overwrite evidence: " + ", ".join(existing))
    ...
    selection = {
        "schema": "graphreader.ocr-component-recall-selection.v1",
        "task": TASK,
        "revision": REVISION,
    ...
    seal = {
        "schema": "graphreader.ocr-component-recall-sealed-test-seal.v1",
        "task": TASK,
        "revision": REVISION,
    ...
    gate = {
        "schema": "graphreader.ocr-component-recall-gate-config.v1",
        "task": TASK,
