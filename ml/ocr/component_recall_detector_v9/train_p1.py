...
"""Single-use P1 training for OCR component-recall detector V9."""
...
CANDIDATE_ID = "P1"
CONFIG_PATH = Path("ml/ocr/component_recall_detector_v9/training/p1.json")
CANONICAL_OUTPUT = Path("ml/ocr/component_recall_detector_v9/artifacts/P1-run")
RUNNER_SOURCE_PATHS = (
    ...
    Path("ml/ocr/component_recall_detector_v9/train_p1.py"),
...
def train_candidate(output_dir: Path) -> dict[str, object]:
    ...
    try:
        ...
        if sha256_file(REPO_ROOT / seal["fixture_archive_path"]) != seal["fixture_archive_sha256"]:
            raise RuntimeError("OCR V9 sealed-public archive changed before training")
        ...
        if preflight_output.shape != (3, 2) or not np.isfinite(preflight_output).all():
            raise RuntimeError("OCR V9 export preflight returned an invalid tensor")
        ...
        phase = "export"
        checkpoint_path = output_dir / "graph-text-component-recall-v9-p1.pt"
        torch.save({"state_dict": model.state_dict(), "selected_threshold": selected["threshold"]}, checkpoint_path)
        onnx_path = output_dir / "graph-text-component-recall-v9-p1.onnx"
