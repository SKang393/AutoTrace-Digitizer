...
"""Single-use relational-neighborhood proposal training for OCR V28 P1."""
...
def _validate_stored_split(
    ...
    if (
        ...
        raise RuntimeError(f"OCR V28 {name} stored fixtures violate the seal")
...
def _load_role_parent_state() -> dict[str, torch.Tensor]:
    ...
    if not isinstance(state, dict) or not state:
        raise RuntimeError("OCR V28 role-parent checkpoint state is missing")
    return state
...
def _candidate_session(path: Path) -> ort.InferenceSession:
    ...
    if session.get_providers() != ["CPUExecutionProvider"]:
        raise RuntimeError("OCR V28 requires CPUExecutionProvider only")
    return session
...
def _export(
    ...
    torch.onnx.export(
        ...
        dynamic_axes={
            ...
            "proposal_relations": {1: "proposal_count", 2: "proposal_count"},
...
def _proposal_objective(
    ...
    if not torch.any(positive) or not torch.any(negative):
        raise RuntimeError("OCR V28 requires positive and negative proposals per scene")
    ...
    hard_count = min(int(config["hard_negative_top_k"]), len(negative_margins))
    hardest = torch.topk(negative_margins, hard_count).values
    hard_negative = torch.relu(
        hardest - float(config["negative_logit_margin_ceiling"])
    ).mean()
    losses = {
...
def _trigger_is_terminal(trigger: dict[str, Any]) -> bool:
    ...
    return bool(
        ...
        and trigger.get("onnx_parity_passed") is True
        and trigger.get("parent_role_argmax_preserved") is True
...
def preflight() -> dict[str, Any]:
    ...
    expected = {
        ...
        "candidate_limit": 3,
        "architecture": "pairwise-geometry-relational-neighborhood-v1",
        ...
        "role_parent_argmax_preservation_required": True,
    ...
    for key, value in expected.items():
        if config.get(key) != value:
            raise RuntimeError(f"OCR V28 P1 config field mismatch: {key}")
    if config.get("expected_runner_source_bundle_sha256") != source_bundle_sha256(
        REPO_ROOT, RUNNER_SOURCE_PATHS,
    ):
        raise RuntimeError("OCR V28 P1 runner source bundle changed")
    if sha256_file(REPO_ROOT / SEAL_PATH) != config.get("split_seal_sha256"):
        raise RuntimeError("OCR V28 split seal changed before P1")
    seal = _read_json(REPO_ROOT / SEAL_PATH)
    required_seal = {
        "schema": "graphreader.ocr-relational-neighborhood-split-seal.v1",
        "revision": REVISION,
    ...
    for key, value in required_seal.items():
        if seal.get(key) != value:
            raise RuntimeError(f"OCR V28 split seal field changed: {key}")
    ...
    if seal.get("source_bundle_sha256") != config.get("split_source_bundle_sha256"):
        raise RuntimeError("OCR V28 split source bundle changed")
    sealed_sources = seal.get("source_sha256")
    if not isinstance(sealed_sources, dict) or not sealed_sources:
        raise RuntimeError("OCR V28 split source inventory is absent")
    ...
    for split, (path, key) in archive_bindings.items():
        ...
        if actual != seal["splits"][split]["archive_sha256"] or actual != config.get(key):
            raise RuntimeError(f"OCR V28 {split} archive changed before P1")
    exact_inputs = {
    ...
    for relative, expected_hash in exact_inputs.items():
        if sha256_file(REPO_ROOT / relative) != expected_hash:
            raise RuntimeError(f"OCR V28 frozen input changed: {relative}")
    if not _trigger_is_terminal(_read_json(REPO_ROOT / TRIGGER_RESULT_PATH)):
        raise RuntimeError("OCR V28 aggregate-only V27 trigger changed")
    if (REPO_ROOT / CANONICAL_OUTPUT).exists():
        raise RuntimeError("OCR V28 P1 output already exists")
    ...
    if (
        ...
        raise RuntimeError("OCR V28 P1 canonical authorization changed")
    return {"config": config, "head": head, "seal": seal}


def train_candidate(output_dir: Path) -> dict[str, object]:
    if output_dir.exists():
        raise RuntimeError(f"OCR V28 P1 output exists: {output_dir}")
    evidence = preflight()
    ...
    try:
        ...
        if train_values.shape[1:] != (FEATURE_COUNT,):
            raise RuntimeError("OCR V28 training evidence width changed")
        if train_crops.shape[1:] != (CROP_CHANNELS, CROP_HEIGHT, CROP_WIDTH):
            raise RuntimeError("OCR V28 training crop shape changed")
        if len(train_relations) != len(train_scenes) or len(train_slices) != len(train_scenes):
            raise RuntimeError("OCR V28 training scene relation stream changed")
        ...
        for key in ("proposal_count", "positive_proposal_count", "negative_proposal_count"):
            if training_evidence[key] != registered[key]:
                raise RuntimeError(f"OCR V28 incomplete training stream: {key}")

        phase = "relational_neighborhood_proposal_training"
        generator = _configure(int(config["seed"]))
        model = RelationalNeighborhoodProposalNet(seed=int(config["seed"]))
        model.load_role_parent_state_dict(_load_role_parent_state())
        frozen_before = _parameter_stream_sha256(model.role_parent)
        trainable = model.trainable_parameters()
        trainable_names = sorted(
            name for name, parameter in model.named_parameters() if parameter.requires_grad
        )
        if not trainable or any(name.startswith("role_parent.") for name in trainable_names):
            raise RuntimeError("OCR V28 frozen role-parent boundary changed")
        ...
        for epoch in range(int(config["epochs"])):
            ...
            for scene_index in torch.randperm(
                ...
                candidate = model(values, crops, relations)[0, :, :2]
            ...
            print(
                f"OCR V28 P1 epoch {epoch + 1}/{config['epochs']} complete; "
                f"optimizer_steps={optimizer_steps}",
                flush=True,
            )
        if optimizer_steps != int(config["expected_optimizer_steps"]):
            raise RuntimeError("OCR V28 optimizer-step count changed")
        frozen_after = _parameter_stream_sha256(model.role_parent)
        if frozen_after != frozen_before:
            raise RuntimeError("OCR V28 modified the frozen V24 role parent")

        phase = "checkpoint_and_onnx_export"
        checkpoint_path = output_dir / "graph-text-relational-neighborhood-proposal-v28-p1.pt"
        torch.save({"state_dict": model.state_dict()}, checkpoint_path)
        onnx_path = output_dir / "graph-text-relational-neighborhood-proposal-v28-p1.onnx"
        ...
        if {item.name for item in candidate_session.get_inputs()} != {
            ...
            raise RuntimeError("OCR V28 ONNX input identity changed")
        ...
        for key in ("proposal_count", "positive_proposal_count", "negative_proposal_count"):
            if selection_evidence[key] != registered[key]:
                raise RuntimeError(f"OCR V28 incomplete selection stream: {key}")
        ...
        selection_evidence.update({
            ...
            "v24_role_parent_output_tensor_stream_sha256": parent_outputs.hexdigest(),
            "candidate_onnx_sha256": sha256_file(onnx_path),
            "candidate_onnx_graph_optimization_level": "ORT_DISABLE_ALL",
            "role_parent_argmax_mismatch_count": role_argmax_mismatches,
        })
        ...
        report: dict[str, object] = {
            ...
            "role_parent_checkpoint_sha256": ROLE_PARENT_CHECKPOINT_SHA256,
            "role_parent_argmax_mismatch_count": role_argmax_mismatches,
            "role_parent_argmax_preserved": roles_preserved,
...
def main() -> int:
    ...
    print(json.dumps({
        ...
        "role_parent_argmax_mismatch_count": report[
            "role_parent_argmax_mismatch_count"
        ],
