...
"""Single-use public gate for a selected line-aware marker candidate."""
...
from ml.markers.center.line_aware_v1.dataset import load_sealed_public_archive
...
from ml.markers.gate_seal import (
    acquire_gate_seal, canonical_json_bytes, complete_gate_seal, sha256_file,
)


REPO_ROOT = Path(__file__).resolve().parents[4]
TASK = "marker-center"
REVISION = "marker-center-line-aware-public-v1"
SPLIT_CONFIG_PATH = Path("ml/markers/center/line_aware_v1/gates/sealed-public-v1.json")
EVALUATOR_SOURCE_PATHS = (
    ...
    Path("ml/markers/center/line_aware_v1/sealed_gate.py"),
)
GATE_CONFIG = {
...
def evaluate_candidate(*, onnx_path: Path, training_report_path: Path, output_path: Path) -> dict[str, object]:
    ...
    if training_report.get("status") != "selected" or training_report.get("selection_gate_passed") is not True:
        raise RuntimeError("Only a selection-passing line-aware candidate may open the public gate")
    candidate_hash = sha256_file(onnx_path)
    if candidate_hash != training_report.get("onnx_sha256"):
        raise RuntimeError("Line-aware candidate ONNX differs from its selection report")
    ...
    if sha256_file(seal_path) != split_config["sealed_public_test_seal_sha256"]:
        raise RuntimeError("Line-aware public-test seal differs from gate configuration")
    ...
    if sha256_file(archive) != sealed["fixture_archive_sha256"]:
        raise RuntimeError("Line-aware fixture archive checksum mismatch")
    if sha256_file(private_manifest) != sealed["private_manifest_sha256"]:
        raise RuntimeError("Line-aware private manifest checksum mismatch")
    ...
    if session.get_providers() != ["CPUExecutionProvider"]:
        raise RuntimeError("Line-aware public gate requires CPUExecutionProvider only")
    ...
    report: dict[str, object] = {
        "schema": "graphreader.marker-center-line-aware-public-gate.v1",
        "task": TASK, "revision": REVISION, "status": "pass" if passed else "fail",
        "release_eligible": False, "production_approval": False, "evaluation_count": 1,
        ...
        "threshold": threshold, "provider": "CPUExecutionProvider", "metrics": metrics,
        "gate_requirements": GATE_CONFIG, "seal_binding": gate.binding,
        "canonical_seal_key": gate.key,
        "elapsed_ms": round((time.perf_counter() - started) * 1000, 3),
        "remaining_blockers": [
            "independent artifact-mask public gate", "production runtime adapter",
            "model manifest and model-store approval", "packaging and clean-machine proof",
        ],
    }
