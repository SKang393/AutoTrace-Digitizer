...
"""Single-use P1 training and selection for the line-aware defect class."""
...
CANDIDATE_ID = "P1"
CONFIG_PATH = Path("ml/markers/center/line_aware_v1/training/p1.json")
RUNNER_SOURCE_PATHS = (
    ...
    Path("ml/markers/center/line_aware_v1/train_p1.py"),
...
def _runner(model: LineAwarePatchNet):
...
def _export(model: LineAwarePatchNet, example: torch.Tensor, path: Path) -> None:
    torch.onnx.export(
        model,
        example,
        path,
...
def train_candidate(output_dir: Path) -> dict[str, object]:
    ...
    try:
        ...
        model = LineAwarePatchNet(LineAwareModelConfig(seed=seed))
        ...
        phase = "export"
        checkpoint_path = output_dir / "marker-center-line-aware-p1.pt"
        torch.save({
            "state_dict": model.state_dict(), "config": model.export_contract(),
            "selected_threshold": selected["threshold"],
        }, checkpoint_path)
        onnx_path = output_dir / "marker-center-line-aware-p1.onnx"
        ...
        report: dict[str, object] = {
            ...
            "training_example_count": len(examples.labels), "epochs": config["epochs"], "seed": seed,
        ...
        failure = {
            ...
            "public_gate_evaluations": 0, "synthetic_only": True, "private_or_article_images": False,
            "chandler_included": False, "sealed_public_archive_opened": False,
            "phase": phase, "exception_type": type(error).__name__, "exception_message": str(error),
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=Path("ml/markers/center/artifacts/line-aware-v1/P1-run"))
