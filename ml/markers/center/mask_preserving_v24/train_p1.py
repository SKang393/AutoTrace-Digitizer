...
from ml.markers.center.real_range_generator_v1.negative_sampler import CONNECTOR_ANCHOR_FRACTIONS, CONNECTOR_ANCHOR_MAX_DISTANCE_PX, TOPOLOGY_RADIUS_PX, SampledNegatives, sample_negatives
...
def run(output_dir: Path, checkpoint: Path, v21_onnx: Path) -> dict:
    ...
    try:
        ...
        if float(topology_config["radius_px"]) != TOPOLOGY_RADIUS_PX:
        ...
        if tuple(float(value) for value in connector_config["fractions"]) != CONNECTOR_ANCHOR_FRACTIONS:
            raise RuntimeError("connector anchor fractions contract changed")
        ...
        if sampling.connector_anchor_target_count != connector_config["target_count"] or sampling.connector_anchor_capacity != connector_config["expected_capacity"] or sampling.connector_anchor_selected != connector_config["expected_selected"] or sampling.connector_anchor_selected_index_sha256 != connector_config["selected_index_sha256"]:
