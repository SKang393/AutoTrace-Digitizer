...
"""Authorization-ready V21 runner; execution is intentionally deferred."""
...
from ml.markers.center.scale_classifier_v16.train import _evaluate
from ml.markers.center.metric_aligned_v17.train import _examples
from ml.markers.gate_seal import canonical_json_bytes, sha256_file
from ml.markers.training_budget import acquire_training_candidate, complete_training_candidate

from .focal_loss import v21_loss
from .protocol import (
    ARCHITECTURE, CLASSIFICATION_LOSS, FOCAL_ALPHA, FOCAL_GAMMA,
    GEOMETRY_VETO_GUARD, V13_MANIFEST_SHA256, V20_DIAGNOSTIC_PATH,
    V20_DIAGNOSTIC_SHA256, V20_RESULT_PATH, V20_RESULT_SHA256,
)
...
CANDIDATE_ID = "P1"
CONFIG_PATH = Path("ml/markers/center/focal_confidence_v21/training/p1.json")
RUNNER_SOURCE_PATHS = (
    Path("ml/markers/center/focal_confidence_v21/protocol.py"),
    Path("ml/markers/center/focal_confidence_v21/focal_loss.py"),
    Path("ml/markers/center/focal_confidence_v21/train.py"),
    ...
    Path("ml/markers/center/tail_coverage_v20/P1_RESULT.json"),
    Path("ml/markers/center/tail_coverage_v20/diagnostics/V20_DIAGNOSTIC.json"),
...
def _verify_inputs(config: dict[str, object]) -> None:
    result_path = REPO_ROOT / V20_RESULT_PATH
    diagnostic_path = REPO_ROOT / V20_DIAGNOSTIC_PATH
    if sha256_file(result_path) != V20_RESULT_SHA256:
        raise RuntimeError("V20 aggregate result changed")
    if sha256_file(diagnostic_path) != V20_DIAGNOSTIC_SHA256:
        raise RuntimeError("V20 diagnostic changed")
    result = json.loads(result_path.read_text(encoding="utf-8"))
    diagnostic = json.loads(diagnostic_path.read_text(encoding="utf-8"))
    if result.get("status") != "failed_dev_retired_unconsumed" or result.get("dev_gate_passed") is not False:
        raise RuntimeError("V21 requires the failed V20 dev result")
    if result.get("private_data") is not False or diagnostic.get("scope", {}).get("private_or_article_images") is not False:
        raise RuntimeError("V21 cannot use private data")
    if config.get("v20_result_sha256") != V20_RESULT_SHA256 or config.get("v20_diagnostic_sha256") != V20_DIAGNOSTIC_SHA256:
        raise RuntimeError("V20 evidence binding changed")
    if config.get("architecture") != ARCHITECTURE or config.get("geometry_veto_guard") != GEOMETRY_VETO_GUARD:
        raise RuntimeError("V21 frozen architecture or geometry guard changed")
    counts = diagnostic.get("miss_stage", {}).get("counts", {})
    if (
        counts.get("marker_geometry_veto") != GEOMETRY_VETO_GUARD["diagnostic_marker_geometry_veto_count"]
        or counts.get("unmasked_artifact_veto") != GEOMETRY_VETO_GUARD["diagnostic_unmasked_artifact_veto_count"]
    ):
        raise RuntimeError("V20 downstream veto counts changed")
    if config.get("input_proposal_manifest_sha256") != V13_MANIFEST_SHA256:
        raise RuntimeError("V13 proposal manifest binding changed")
    if config.get("classification_loss") != CLASSIFICATION_LOSS or config.get("focal_alpha") != FOCAL_ALPHA or config.get("focal_gamma") != FOCAL_GAMMA:
        raise RuntimeError("V21 focal-loss constants changed")
    if config.get("sealed_candidate_budget") != 1 or config.get("sealed_runs") != 0:
        raise RuntimeError("V21 candidate budget changed")
...
def run(output_dir: Path) -> dict[str, object]:
    ...
    try:
        ...
        if len(train_scenes) != int(config["train_scene_count"]):
            raise RuntimeError("V20 synthetic train-scene count changed")
        generator = torch.Generator().manual_seed(int(config["seed"]) + 1)
        patches, labels, offsets, radii, hard = _examples(
        ...
        for _ in range(int(config["epochs"])):
            ...
            for start in range(0, len(order), int(config["batch_size"])):
                ...
                loss = v21_loss(
                    ...
                    alpha=float(config["focal_alpha"]), gamma=float(config["focal_gamma"]),
                )
        ...
        phase = "export"
        checkpoint = output_dir / "marker-center-focal-confidence-v21-p1.pt"
        ...
        onnx_path = output_dir / "marker-center-focal-confidence-v21-p1.onnx"
        ...
        if session.get_providers() != ["CPUExecutionProvider"]:
            raise RuntimeError("V21 parity requires CPUExecutionProvider")
        parity_rows = []
