...
"""One-use P1 training and visible selection for marker-center V7."""
...
CANDIDATE_ID = "P1"
CONFIG_PATH = Path("ml/markers/center/domain_randomized_v7/training/p1.json")
RUNNER_SOURCE_PATHS = (
    ...
    Path("ml/markers/center/domain_randomized_v7/train_p1.py"),
)
EPOCHS = 48
...
def _execute_candidate(
    ...
    for key, expected in expected_configuration.items():
        if config.get(key) != expected:
            raise RuntimeError(f"Marker-center V7 P1 configuration changed: {key}")
    ...
    for epoch in range(EPOCHS):
        epoch_indices = np.roll(indices, (epoch * 17) % len(indices))
    ...
    if optimizer_steps != int(config["expected_optimizer_steps"]):
        raise RuntimeError("Marker-center V7 P1 optimizer step count changed")
    progress["phase"] = "export"
    checkpoint_path = output_dir / "marker-center-domain-randomized-v7-p1.pt"
    onnx_path = output_dir / "marker-center-domain-randomized-v7-p1.onnx"
    ...
    if session.get_providers() != ["CPUExecutionProvider"]:
        raise RuntimeError("Marker-center V7 P1 selection requires CPUExecutionProvider only")
...
def run(output_dir: Path) -> dict[str, object]:
    if output_dir.exists():
        raise RuntimeError(f"Marker-center V7 P1 output exists: {output_dir}")
