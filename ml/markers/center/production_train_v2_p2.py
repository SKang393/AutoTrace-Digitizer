...
"""Preregistered P2 runner isolated to the procedural tick/joint dataset change."""
...
from .dataset_v2_p2 import build_p2_selection_dataset, seal_p2_dataset_manifest
...
CANDIDATE_ID = "P2"
CONFIG_PATH = Path("ml/markers/center/training/production-repair-v2-p2.json")
RUNNER_SOURCE_PATHS = (
    Path("ml/markers/center/production_train_v2_p2.py"),
    Path("ml/markers/center/dataset_v2_p2.py"),
...
def train_candidate(output_dir: Path) -> tuple[Path, Path, dict[str, object]]:
    ...
    manifest_path, manifest_sha256 = seal_p2_dataset_manifest(output_dir)
    if manifest_sha256 != config["selection_dataset_manifest_sha256"]:
        raise RuntimeError("P2 selection dataset manifest differs from preregistration")
    ...
    training_scenes = build_p2_selection_dataset("train")
    validation_scenes = build_p2_selection_dataset("validation")
    loss_checkpoints = []
    model.train()
