...
"""One-time P2 validation of the exact radial payload with calibrated postprocessing."""
...
from ml.markers.center.runtime_consistency_v2.pipeline_p2 import (
    MINIMUM_CENTER_SEPARATION,
    POSTPROCESS_REVISION,
    evaluate_scenes,
)
from ml.markers.center.runtime_consistency_v2.public_gate_p2 import evaluate_candidate
...
CANDIDATE_ID = "P2"
CONFIG_PATH = Path("ml/markers/center/runtime_consistency_v2/training/p2.json")
RUNNER_SOURCE_PATHS = (
    ...
    Path("ml/markers/center/runtime_consistency_v2/candidate_runner_p2.py"),
    Path("ml/markers/center/runtime_consistency_v2/public_gate_p2.py"),
...
def validate_candidate(output_dir: Path) -> dict[str, object]:
    ...
    try:
        if int(config["optimizer_steps"]) != 0 or config["weights_changed"] is not False:
            raise RuntimeError("Runtime-consistency P2 must perform zero training")
        if config["postprocess_revision"] != POSTPROCESS_REVISION:
            raise RuntimeError("P2 postprocess revision differs from its preregistration")
        if float(config["minimum_center_separation"]) != MINIMUM_CENTER_SEPARATION:
            raise RuntimeError("P2 minimum center separation differs from its preregistration")
        diagnosis_path = REPO_ROOT / config["diagnosis_path"]
        if sha256_file(diagnosis_path) != config["diagnosis_sha256"]:
            raise RuntimeError("P2 bounded diagnosis differs from its preregistration")
        diagnosis = json.loads(diagnosis_path.read_text(encoding="utf-8"))
        if (
            diagnosis.get("scope") != "visible validation selection only"
            or diagnosis.get("public_archive_opened") is not False
            or diagnosis.get("sweep_count") != 1
        ):
            raise RuntimeError("P2 diagnosis is not the frozen selection-only sweep")
        ...
        if sha256_file(archive) != sealed["fixture_archive_sha256"]:
            raise RuntimeError("Sealed public archive changed before P2 execution")
        ...
        if (
            ...
            or source_report.get("postprocess_revision")
            != config["source_payload_postprocess_revision"]
            or float(source_report.get("selected_threshold", -1.0))
            != float(config["source_payload_threshold"])
        ):
            raise RuntimeError("Source report does not bind the exact radial P3 payload")

        checkpoint_path = output_dir / "marker-center-runtime-consistency-p2.pt"
        onnx_path = output_dir / "marker-center-runtime-consistency-p2.onnx"
        ...
        if session.get_providers() != ["CPUExecutionProvider"]:
            raise RuntimeError("P2 validation requires CPUExecutionProvider only")
        ...
        report: dict[str, object] = {
            "schema": "graphreader.marker-center-runtime-consistency-candidate-p2.v2",
            "task": TASK,
            "revision": REVISION,
            "candidate_id": CANDIDATE_ID,
        ...
        summary = {
            "schema": "graphreader.marker-center-runtime-consistency-run-summary-p2.v2",
            "task": TASK,
            "revision": REVISION,
            "candidate_id": CANDIDATE_ID,
        ...
        failure = {
            "schema": "graphreader.marker-center-runtime-consistency-failure-p2.v2",
            "task": TASK,
            "revision": REVISION,
            "candidate_id": CANDIDATE_ID,
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("ml/markers/center/artifacts/runtime-consistency-v2/P2-run"),
    )
