...
"""Materialize fresh V9 splits before any model execution."""
...
from ml.markers.center.mask_consensus_v9.protocol import DESIGN_SOURCE_PATHS, REVISION, ROOT
from ml.markers.gate_seal import canonical_json_bytes, sha256_file, source_bundle_sha256


REPO_ROOT = Path(__file__).resolve().parents[4]
OUTPUT_ROOT = REPO_ROOT / "ml/markers/center/artifacts/mask-consensus-v9/split-freeze"
...
def prepare() -> dict[str, object]:
    ...
    if any(path.exists() for path in tracked_outputs) or OUTPUT_ROOT.exists():
        raise RuntimeError("Marker-center V9 splits or seals already exist")
    ...
    if len(all_hashes) != len(set(all_hashes)):
        raise RuntimeError("Marker-center V9 source bytes overlap across splits")
    ...
    public_dataset = {
        "schema": "graphreader.marker-center-mask-consensus-public-dataset.v9",
        "revision": REVISION,
    ...
    public_seal = {
        "schema": "graphreader.marker-center-mask-consensus-public-seal.v9",
        "revision": REVISION,
    ...
    selection = {
        "schema": "graphreader.marker-center-mask-consensus-selection.v9",
        "revision": REVISION,
        "train": {
            **SPLITS["train"],
    ...
    report = {
        "schema": "graphreader.marker-center-mask-consensus-split-freeze.v9",
        "revision": REVISION,
