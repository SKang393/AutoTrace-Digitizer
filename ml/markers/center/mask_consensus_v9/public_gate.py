...
"""One-use truth-hidden public gate for marker-center mask-consensus V9."""
...
from ml.markers.center.mask_consensus_v8.public_gate import (
    GATE_CONFIGURATION,
    _run_opened_gate,
)
from ml.markers.center.mask_consensus_v9.protocol import REVISION, TASK
from ml.markers.gate_seal import acquire_gate_seal, sha256_file


REPO_ROOT = Path(__file__).resolve().parents[4]
ROOT = REPO_ROOT / "ml/markers/center/mask_consensus_v9"
GATE_PATH = Path("ml/markers/center/mask_consensus_v9/gates/sealed-public-v1.json")
EVALUATOR_SOURCE_PATHS = (
    ...
    Path("ml/markers/center/mask_consensus_v9/public_gate.py"),
)
...
def run(candidate_report_path: Path, output_path: Path) -> dict[str, object]:
    ...
    if (
        ...
        raise RuntimeError("Marker-center V9 public gate is not separately authorized")
    ...
    if sha256_file(split_seal_path) != gate["expected_split_seal_sha256"]:
        raise RuntimeError("Truth-hidden marker-center V9 split seal changed")
    ...
    if sha256_file(archive_path) != gate["expected_public_fixture_archive_sha256"]:
        raise RuntimeError("Truth-hidden marker-center V9 public archive changed")
