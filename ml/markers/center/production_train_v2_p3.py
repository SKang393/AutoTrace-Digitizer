...
"""Preregistered P3 runner isolated to the non-overlapping dataset placement."""
...
from .dataset_v2_p3 import build_p3_selection_dataset, seal_p3_dataset_manifest
...
from .production_train_v2 import _selection_score
...
CANDIDATE_ID = "P3"
CONFIG_PATH = Path("ml/markers/center/training/production-repair-v2-p3.json")
RUNNER_SOURCE_PATHS = (
    Path("ml/markers/center/production_train_v2_p3.py"),
    Path("ml/markers/center/dataset_v2_p3.py"),
...
def train_candidate(output_dir: Path) -> tuple[Path, Path, dict[str, object]]:
    ...
    manifest_path, manifest_sha256 = seal_p3_dataset_manifest(output_dir)
    if manifest_sha256 != config["selection_dataset_manifest_sha256"]:
        raise RuntimeError("P3 selection dataset manifest differs from preregistration")
    ...
    training_scenes = build_p3_selection_dataset("train")
    validation_scenes = build_p3_selection_dataset("validation")
    loss_checkpoints = []
    model.train()
    for epoch in range(int(config["epochs"])):
        ...
        radius_loss = functional.smooth_l1_loss(
            heads[:, 1:2][radius_mask], radius_target[radius_mask]
        )
        ...
        loss = (
            ...
            + float(config["changes"]["hard_negative_artifact_weight"])
            * hard_negative_artifact_loss
            + float(config["changes"]["hard_negative_center_suppression_weight"])
            * hard_negative_center_loss
        )
        loss.backward()
    ...
    parity = export_onnx(checkpoint, onnx_path, candidate_dir / "onnx-parity.json")
    ...
    gates = {
        ...
        "zero_duplicates": standard["metrics_5px"]["duplicate_rate"] == 0
        and robust["metrics_5px"]["duplicate_rate"] == 0,
        "zero_hard_negative_hits": not any(standard["hard_negative_hits"].values())
        and not any(robust["hard_negative_hits"].values()),
