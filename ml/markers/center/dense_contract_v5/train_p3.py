...
"""One-use P3 spatial-margin and inference-graph repair for dense-contract V5."""
...
CANDIDATE_ID = "P3"
CONFIG_PATH = Path("ml/markers/center/dense_contract_v5/training/p3.json")
RUNNER_SOURCE_PATHS = (
...
FUSION_SEMANTIC_TOLERANCE = 1e-5
        ...
        for point in archive["hard_points"][scene_index, :hard_count]:
            negative_values.append(
                _disk_maximum(
                    ...
                    x=float(point[0]),
                    y=float(point[1]),
                    radius=HARD_NEGATIVE_TOLERANCE,
                )
            )
    zero = center_prediction.sum() * 0.0
    if positive_values:
        positives = torch.stack(positive_values).clamp(1e-5, 1 - 1e-5)
        positive_loss = functional.binary_cross_entropy(positives, torch.ones_like(positives))
    else:
        positive_loss = zero
    if negative_values:
        negatives = torch.stack(negative_values).clamp(1e-5, 1 - 1e-5)
...
def _fuse_inference_model(model: torch.nn.Module) -> torch.nn.Module:
    fused = deepcopy(model).eval()
    for module in tuple(fused.modules()):
        if (
            ...
            module[0] = fuse_conv_bn_eval(module[0], module[1])
            module[1] = torch.nn.Identity()
    return fused
...
def _execute_candidate(
    ...
    expected_configuration = {
        ...
        "fusion_semantic_tolerance": FUSION_SEMANTIC_TOLERANCE,
        "hard_negative_margin_loss_weight": HARD_NEGATIVE_MARGIN_LOSS_WEIGHT,
        "hard_negative_margin_radius_px": HARD_NEGATIVE_TOLERANCE,
        "learning_rate": LEARNING_RATE,
        "onnx_parity_tolerance": PARITY_TOLERANCE,
        "positive_margin_loss_weight": POSITIVE_MARGIN_LOSS_WEIGHT,
        "positive_margin_radius_px": MATCH_TOLERANCE,
        "selection_thresholds": list(THRESHOLDS),
    }
    for key, expected in expected_configuration.items():
        if config.get(key) != expected:
            raise RuntimeError(f"Dense-contract V5 P3 configuration changed: {key}")
    ...
    if sha256_file(ROOT / "SELECTION_MANIFEST.json") != config["selection_manifest_sha256"]:
        raise RuntimeError("Dense-contract V5 P3 selection manifest changed")
    ...
    if optimizer_steps != int(config["expected_optimizer_steps"]):
        raise RuntimeError("Dense-contract V5 P3 optimizer step count changed")
    progress["phase"] = "export"
    checkpoint_path = output_dir / "marker-center-dense-contract-v5-p3.pt"
    onnx_path = output_dir / "marker-center-dense-contract-v5-p3.onnx"
    ...
    if session.get_providers() != ["CPUExecutionProvider"]:
        raise RuntimeError("Dense-contract V5 P3 selection requires CPUExecutionProvider only")
    ...
    for index in range(validation["inputs"].shape[0]):
        ...
        fusion_semantic_error = max(
            fusion_semantic_error,
            _maximum_output_difference(raw_outputs, fused_outputs),
        )
    ...
    selection_passed = (
        ...
        and fusion_semantic_error <= FUSION_SEMANTIC_TOLERANCE
    )
    save_checkpoint(
        checkpoint_path,
        model,
    ...
    return {
        ...
        "checkpoint_to_inference_graph_tolerance": FUSION_SEMANTIC_TOLERANCE,
        "checkpoint_to_inference_graph_passed": fusion_semantic_error <= FUSION_SEMANTIC_TOLERANCE,
...
def run(output_dir: Path) -> dict[str, object]:
    if output_dir.exists():
        raise RuntimeError(f"Dense-contract V5 P3 output exists: {output_dir}")
