...
"""One-use truth-hidden public gate for marker-center mask-consensus V8."""
...
from ml.markers.center.mask_consensus_v8.dataset import PROHIBITED_KINDS, read_archive
from ml.markers.center.mask_consensus_v8.protocol import REVISION, TASK
...
GATE_PATH = Path("ml/markers/center/mask_consensus_v8/gates/sealed-public-v1.json")
EVALUATOR_SOURCE_PATHS = (
    ...
    Path("ml/markers/center/mask_consensus_v8/public_gate.py"),
)
GATE_CONFIGURATION = {
...
def _evaluate_opened_gate(
    *,
    ...
    if session.get_providers() != ["CPUExecutionProvider"]:
        raise RuntimeError("Marker-center V8 public gate requires CPUExecutionProvider only")
...
def run(candidate_report_path: Path, output_path: Path) -> dict[str, object]:
    ...
    if entry.get("public_gate_authorized") is not True or entry.get("public_gate_authorized_candidate_id") != candidate_id:
        raise RuntimeError("Marker-center V8 public gate is not separately authorized")
    candidate_report_sha256 = sha256_file(candidate_report_path)
