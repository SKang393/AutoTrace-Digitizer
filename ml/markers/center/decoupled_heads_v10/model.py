...
"""Gradient-decoupled marker-center and artifact-mask network."""
...
class DecoupledHeadsConfig:
    architecture: str = "marker-center-decoupled-heads-v10"
...
class IndependentTower(nn.Module):
    def __init__(self, config: DecoupledHeadsConfig) -> None:
        super().__init__()
    ...
    def forward(self, value: Tensor) -> Tensor:
        ink = value[:, 0:1]
        exclusion = torch.maximum(value[:, 1:2].clamp(0.0, 1.0), value[:, 2:3].clamp(0.0, 1.0))
        safe_ink = ink * (1.0 - exclusion)
...
class DecoupledMarkerArtifactNet(nn.Module):
    """Use separate towers so artifact specificity cannot consume center capacity."""

    contract = TensorContract()

    def __init__(self, config: DecoupledHeadsConfig | None = None) -> None:
        super().__init__()
        self.config = config or DecoupledHeadsConfig()
        self.center_tower = IndependentTower(self.config)
        self.artifact_tower = IndependentTower(self.config)
    ...
    def _initialize_weights(self) -> None:
        ...
        nn.init.constant_(self.artifact_head.bias, -4.0)
    ...
    def forward(self, value: Tensor) -> Tensor:
        ...
        learned_artifact = torch.sigmoid(self.artifact_head(self.artifact_tower(value)))
...
def create_model() -> DecoupledMarkerArtifactNet:
    return DecoupledMarkerArtifactNet()


def save_checkpoint(
    path: Path,
    model: DecoupledMarkerArtifactNet,
    *,
    ...
    torch.save(
        {
            "format_version": 1,
            "kind": "trained-pytorch-decoupled-marker-artifact-checkpoint",
...
def load_checkpoint(path: Path, *, device: str = "cpu") -> tuple[DecoupledMarkerArtifactNet, dict[str, Any]]:
    payload = torch.load(path, map_location=device, weights_only=False)
    config_data = dict(payload["config"])
    config_data["channels"] = tuple(config_data["channels"])
    model = DecoupledMarkerArtifactNet(DecoupledHeadsConfig(**config_data))
...
__all__ = [
    "DecoupledHeadsConfig",
    "DecoupledMarkerArtifactNet",
    "create_model",
    "load_checkpoint",
    "save_checkpoint",
]
