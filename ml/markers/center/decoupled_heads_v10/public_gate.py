...
"""One-use truth-hidden public gate for marker-center V10."""
...
from ml.markers.center.mask_consensus_v8.public_gate import GATE_CONFIGURATION, _run_opened_gate
from ml.markers.center.decoupled_heads_v10.protocol import REVISION, TASK
...
GATE_PATH = Path("ml/markers/center/decoupled_heads_v10/gates/sealed-public-v1.json")
EVALUATOR_SOURCE_PATHS = (
    ...
    Path("ml/markers/center/decoupled_heads_v10/public_gate.py"),
)
...
def run(candidate_report_path: Path, output_path: Path) -> dict[str, object]:
    ...
    if (
        ...
        raise RuntimeError("Marker-center V10 public gate is not separately authorized")
    ...
    if sha256_file(split_seal_path) != gate["expected_split_seal_sha256"]:
        raise RuntimeError("Truth-hidden marker-center V10 split seal changed")
    ...
    if sha256_file(archive_path) != gate["expected_public_fixture_archive_sha256"]:
        raise RuntimeError("Truth-hidden marker-center V10 public archive changed")
