...
"""One-use truth-hidden public gate for marker-center seed-refinement V11."""
...
from ml.markers.center.seed_refinement_v11.dataset import PROHIBITED_KINDS, read_archive
...
GATE_PATH = Path("ml/markers/center/seed_refinement_v11/gates/sealed-public-v1.json")
EVALUATOR_SOURCE_PATHS = (
    ...
    Path("ml/markers/center/seed_refinement_v11/public_gate.py"),
)
GATE_CONFIGURATION = {
    "profile": "marker-center-seed-refinement-public-gate-v1",
    ...
    "maximum_marker_artifact_hits": 0,
...
def _evaluate_opened_gate(
    *,
    ...
    if session.get_providers() != ["CPUExecutionProvider"]:
        raise RuntimeError("Marker-center V11 public gate requires CPUExecutionProvider only")
    ...
    for index in range(archive["inputs"].shape[0]):
        ...
        output = _onnx_output(session, value)
        outputs.append(output)
        input_stream.update(value.tobytes(order="C"))
        output_stream.update(output.tobytes(order="C"))
    ...
    return {
        "schema": "graphreader.marker-seed-refinement-gate.v1",
        "profile": "marker-center-seed-refinement-public-gate-v1",
        "status": "pass" if passed else "fail",
        "scope": "public_synthetic_sealed",
        "provider": "cpu",
        "seed_mask_scope": "checksum_bound_proposal_prior_refined_nonmonotonically",
        "runtime_pass_count": 1,
        "runtime_postprocess_profile": "nonmonotonic_seed_refinement_v1",
        ...
        "seed_removed_pixels": aggregate["seed_removed_pixels"],
        "seed_added_pixels": aggregate["seed_added_pixels"],
        "direct_execution_inference_calls": archive["inputs"].shape[0],
...
def _run_opened_gate(
    *,
    ...
    try:
        ...
        failure = {
            "schema": "graphreader.marker-seed-refinement-gate-failure.v1",
            "profile": "marker-center-seed-refinement-public-gate-v1",
        ...
        complete_gate_seal(seal, status="failed_runner", report_sha256=sha256_file(output_path))
        raise
    output_path.parent.mkdir(parents=True, exist_ok=True)
    output_path.write_bytes(canonical_json_bytes(report))
    complete_gate_seal(seal, status=str(report["status"]), report_sha256=sha256_file(output_path))
    return report
...
def run(candidate_report_path: Path, output_path: Path) -> dict[str, object]:
    ...
    if entry.get("public_gate_authorized") is not True or entry.get("public_gate_authorized_candidate_id") != candidate_id:
        raise RuntimeError("Marker-center V11 public gate is not separately authorized")
    candidate_report_sha256 = sha256_file(candidate_report_path)
    if entry.get("public_gate_authorized_candidate_report_sha256") != candidate_report_sha256:
        raise RuntimeError("Authorized candidate report identity changed")
    ...
    if onnx_sha256 != candidate["onnx_sha256"] or entry.get("public_gate_authorized_onnx_sha256") != onnx_sha256:
        raise RuntimeError("Authorized candidate ONNX identity changed")
    ...
    if sha256_file(archive_path) != gate["expected_public_fixture_archive_sha256"]:
        raise RuntimeError("Truth-hidden marker-center V8 public archive changed")
