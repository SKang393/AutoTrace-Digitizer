...
"""Single-use P2 training and visible selection for marker-center V11."""
...
CANDIDATE_ID = "P2"
CONFIG_PATH = Path("ml/markers/center/seed_refinement_v11/training/p2.json")
P1_RESULT_PATH = Path("ml/markers/center/seed_refinement_v11/P1_RESULT.json")
P1_RESULT_SHA256 = "8fb959d27f11d501ba6769acdb5e7230a011b751f731cc0bc2382a427e12e190"
...
def _verify_config_and_inputs(
    ...
    for key, value in expected.items():
        if config.get(key) != value:
            raise RuntimeError(f"Marker-center V11 P2 configuration changed: {key}")
    p1_result_path = REPO_ROOT / P1_RESULT_PATH
    if sha256_file(p1_result_path) != P1_RESULT_SHA256:
        raise RuntimeError("Marker-center V11 P1 aggregate result changed")
    p1_result = json.loads(p1_result_path.read_text(encoding="utf-8"))
    if (
        p1_result.get("status") != "failed_selection_consumed"
        or p1_result.get("selection_exact_scene_count") != 122
        or p1_result.get("selection_false_positives") != 0
        or p1_result.get("selection_false_negatives") != 26
        or p1_result.get("artifact_precision") != 0.7815472274567087
        or p1_result.get("artifact_recall") != 0.9687703003413912
        or p1_result.get("seed_added_pixels") != 250166
        or p1_result.get("seed_removed_pixels") != 1175
        or p1_result.get("public_gate_archive_opened") is not False
        or p1_result.get("case_detail_or_pixels_inspected") is not False
    ):
        raise RuntimeError("Marker-center V11 P2 requires the consumed aggregate P1 failure")
...
def _execute_candidate(
    ...
    if optimizer_steps != int(config["expected_optimizer_steps"]):
        raise RuntimeError("Marker-center V11 P2 optimizer step count changed")
    ...
    checkpoint_path = output_dir / "marker-center-seed-refinement-v11-p2.pt"
    onnx_path = output_dir / "marker-center-seed-refinement-v11-p2.onnx"
    ...
    if session.get_providers() != ["CPUExecutionProvider"]:
        raise RuntimeError("Marker-center V11 P2 selection requires CPUExecutionProvider only")
...
def run(output_dir: Path) -> dict[str, object]:
    if output_dir.exists():
        raise RuntimeError(f"Marker-center V11 P2 output exists: {output_dir}")
...
def main() -> int:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=REPO_ROOT / "ml/markers/center/artifacts/seed-refinement-v11/P2-run",
    )
