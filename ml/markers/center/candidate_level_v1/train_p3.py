...
from ml.markers.center.candidate_level_v1.pipeline import evaluate_scenes, extract_proposals
...
CANDIDATE_ID = "P3"
CONFIG_PATH = Path("ml/markers/center/candidate_level_v1/training/p3.json")
RUNNER_SOURCE_PATHS = (
    ...
    Path("ml/markers/center/candidate_level_v1/train_p3.py"),
...
def calibrate_candidate(output_dir: Path) -> dict[str, object]:
    ...
    try:
        ...
        model = CandidatePatchNetP2(CandidateP2ModelConfig(seed=int(config["source_seed"])))
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("ml/markers/center/artifacts/candidate-level-v1/P3-run"),
    )
