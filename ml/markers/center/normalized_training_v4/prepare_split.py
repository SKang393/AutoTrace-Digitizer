...
"""Freeze normalized-training V4 selection and truth-hidden public evidence."""
...
from ml.markers.center.normalized_training_v4.public_gate import (
...
DEFAULT_PRIVATE_ROOT = Path("ml/markers/center/artifacts/normalized-training-v4/split-freeze")
DEFAULT_SELECTION_MANIFEST = Path("ml/markers/center/normalized_training_v4/SELECTION_MANIFEST.json")
DEFAULT_SEALED_TEST_SEAL = Path("ml/markers/center/normalized_training_v4/SEALED_PUBLIC_TEST_SEAL.json")
DEFAULT_GATE_CONFIG = Path("ml/markers/center/normalized_training_v4/gates/sealed-public-p1.json")
DEFAULT_TRAINING_CONFIG = Path("ml/markers/center/normalized_training_v4/training/p1.json")
SOURCE_PATHS = (
    Path("ml/markers/center/normalized_training_v4/dataset.py"),
    Path("ml/markers/center/normalized_training_v4/prepare_split.py"),
)
RUNNER_SOURCE_PATHS = (
    Path("ml/markers/center/normalized_training_v4/dataset.py"),
...
def freeze_split(
    *,
    ...
    seed_path.write_bytes(
        canonical_json_bytes(
            {
                "schema": "graphreader.marker-center-normalized-training-private-seed.v4",
    ...
    seal = {
        "schema": "graphreader.marker-center-normalized-training-sealed-test-seal.v4",
        "task": "marker-center",
    ...
    gate = {
        "schema": "graphreader.marker-center-normalized-training-gate-config.v4",
        "task": "marker-center",
    ...
    training = {
        "schema": "graphreader.marker-center-normalized-training-candidate-config.v4",
        "task": "marker-center",
        "revision": "marker-center-normalized-training-v4",
        "candidate_id": "P1",
        "experiment_ordinal": 1,
        "experiment_budget": 3,
        "trigger": "The consumed zero-training background-invariance P1 passed CPU parity but failed 8 of 16 fresh selection scenes with five false positives, three misses, and three duplicates. The unchanged weights were not calibrated for normalized inputs.",
        "isolated_change": "train new radial-topology MLP head weights from a new seed on fresh smooth-background procedural families after applying the exact median-normalized ink preprocessing; retain proposal eligibility, masks, geometry refinement, duplicate suppression, and public gates",
        ...
        "learning_rate": 0.002,
        "weight_decay": 0.0001,
        "positive_loss_weight": 4.5,
        "maximum_negative_per_positive": 16,
        "selection_thresholds": list(GATE_CONFIG["allowed_thresholds"]),
    ...
    return {
        "selection_manifest_sha256": sha256_file(selection_path),
        "selection_scene_count": selection_manifest()["scene_count"],
        "sealed_public_test_seal_sha256": sha256_file(seal_path),
        "sealed_public_scene_count": len(scenes),
        "fixture_archive_sha256": sha256_file(archive_path),
        "private_manifest_sha256": sha256_file(private_manifest_path),
        ...
        "evaluator_source_bundle_sha256": source_bundle_sha256(REPO_ROOT, EVALUATOR_SOURCE_PATHS),
    }
