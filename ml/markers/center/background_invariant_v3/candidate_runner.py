...
"""Execute the single preregistered marker background-invariance candidate."""
...
from ml.markers.center.background_invariant_v3.public_gate import evaluate_candidate
from ml.markers.center.runtime_consistency_v2.candidate_runner import _load_exact_model
...
CANDIDATE_ID = "P1"
CONFIG_PATH = Path("ml/markers/center/background_invariant_v3/training/p1.json")
RUNNER_SOURCE_PATHS = (
...
def validate_candidate(output_dir: Path) -> dict[str, object]:
    ...
    try:
        if int(config["optimizer_steps"]) != 0 or config["weights_changed"] is not False:
            raise RuntimeError("Background-invariance P1 must perform zero training")
        ...
        if sha256_file(archive) != sealed["fixture_archive_sha256"]:
            raise RuntimeError("Sealed public archive changed before P1 execution")
        ...
        if (
            ...
            raise RuntimeError("Source report does not bind the selected exact ONNX")

        checkpoint_path = output_dir / "marker-center-background-invariant-p1.pt"
        onnx_path = output_dir / "marker-center-background-invariant-p1.onnx"
        ...
        if sha256_file(checkpoint_path) != config["source_checkpoint_sha256"]:
            raise RuntimeError("Copied checkpoint bytes differ from source")
        if sha256_file(onnx_path) != config["source_onnx_sha256"]:
            raise RuntimeError("Copied ONNX bytes differ from source")

        phase = "selection"
        validation_scenes = build_selection_scenes()
        session = ort.InferenceSession(str(onnx_path), providers=["CPUExecutionProvider"])
        if session.get_providers() != ["CPUExecutionProvider"]:
            raise RuntimeError("P1 validation requires CPUExecutionProvider only")
        ...
        metrics = evaluate_scenes(validation_scenes, onnx_runner, threshold=selected_threshold)
        selection_passed = (
        ...
        phase = "parity"
        model = _load_exact_model(checkpoint_path, int(config["source_model_seed"]))
        ...
        report: dict[str, object] = {
            "schema": "graphreader.marker-center-background-invariant-candidate-p1.v3",
            "task": TASK,
            "revision": REVISION,
            "candidate_id": CANDIDATE_ID,
            ...
            "background_statistic": "lower-middle median over each 33x33 ink patch",
        ...
        summary = {
            "schema": "graphreader.marker-center-background-invariant-run-summary-p1.v3",
            "task": TASK,
            "revision": REVISION,
            "candidate_id": CANDIDATE_ID,
        ...
        failure = {
            "schema": "graphreader.marker-center-background-invariant-run-failure-p1.v3",
            "task": TASK,
            "revision": REVISION,
            "candidate_id": CANDIDATE_ID,
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("ml/markers/center/artifacts/background-invariant-v3/P1-run"),
    )
