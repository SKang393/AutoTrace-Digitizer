...
"""Single-use P2 training and selection with stronger center-offset supervision."""
...
from ml.markers.center.line_aware_v1.pipeline import (
    concatenate_examples, evaluate_scenes, extract_proposals, sample_training_examples,
)
...
CANDIDATE_ID = "P2"
CONFIG_PATH = Path("ml/markers/center/radial_feature_v1/training/p2.json")
RUNNER_SOURCE_PATHS = (
    Path("ml/markers/center/radial_feature_v1/dataset.py"),
    Path("ml/markers/center/radial_feature_v1/model.py"),
    Path("ml/markers/center/radial_feature_v1/train_p2.py"),
...
def _export(model: RadialFeatureNet, sample: torch.Tensor, path: Path) -> None:
    torch.onnx.export(
        model, sample, path, input_names=[model.contract.input_name],
        output_names=[model.contract.output_name], opset_version=18,
        dynamic_axes={model.contract.input_name: {0: "candidate_count"}, model.contract.output_name: {0: "candidate_count"}},
        dynamo=False,
    )
...
def _candidate_loss(
    ...
    *,
    ...
    if torch.any(positive):
        ...
        radius = raw[:, 3].sum() * 0
    total = marker + (offset_loss_weight * offset) + (0.35 * radius)
    return total, {
    ...
    try:
        ...
        examples = concatenate_examples(
            sample_training_examples(
                scene, maximum_negative_per_positive=int(config["maximum_negative_per_positive"]),
                generator=generator,
            )
        ...
        _export(model.eval(), examples.patches[:8], preflight)
        ...
        for epoch in range(int(config["epochs"])):
            ...
            for start in range(0, len(order), batch_size):
                ...
                loss, components = _candidate_loss(
                    raw, examples.labels.index_select(0, indices),
                    examples.offsets.index_select(0, indices), examples.radii.index_select(0, indices),
                    positive_weight=float(config["positive_loss_weight"]),
                    offset_loss_weight=float(config["offset_loss_weight"]),
                )
            ...
            if epoch in {0, int(config["epochs"]) // 2, int(config["epochs"]) - 1}:
                checkpoints.append({
                    "epoch": epoch + 1,
                    **{key: sum(item[key] for item in losses) / len(losses) for key in ("total", "marker", "offset", "radius")},
                })
        phase = "selection"
        model.eval()
        threshold_results = [
            {"threshold": float(threshold), "metrics": evaluate_scenes(validation_scenes, _runner(model), threshold=float(threshold))}
        ...
        selected = max(threshold_results, key=lambda item: (
            ...
            item["metrics"]["f1"], item["threshold"],
        ))
        metrics = selected["metrics"]
        selection_passed = (
            metrics["exact_scene_count"] == metrics["scene_count"]
            and metrics["false_positives"] == 0 and metrics["false_negatives"] == 0
            and metrics["duplicate_count"] == 0 and metrics["prohibited_structure_hits"] == 0
        )
        phase = "export"
        checkpoint_path = output_dir / "marker-center-radial-feature-p2.pt"
        torch.save({"state_dict": model.state_dict(), "config": model.export_contract(), "selected_threshold": selected["threshold"]}, checkpoint_path)
        onnx_path = output_dir / "marker-center-radial-feature-p2.onnx"
        ...
        report: dict[str, object] = {
            ...
            "release_eligible": False, "production_approval": False, "public_gate_evaluations": 0,
            "synthetic_only": True, "private_or_article_images": False, "chandler_included": False,
            "training_authorization": authorization.binding, "training_example_count": len(examples.labels),
            "epochs": config["epochs"], "seed": seed, "optimizer_steps": optimizer_steps,
            "loss_weights": {"marker_positive": config["positive_loss_weight"], "offset": config["offset_loss_weight"], "radius": 0.35},
            "loss_checkpoints": checkpoints, "onnx_preflight_sha256": preflight_sha256,
            "threshold_comparisons": threshold_results, "selected_threshold": selected["threshold"],
            ...
            "onnx_provider": "CPUExecutionProvider", "onnx_parity_maximum_absolute_error": maximum_error,
            "onnx_parity_tolerance": config["onnx_parity_tolerance"], "onnx_parity_passed": parity_passed,
        ...
        failure = {
            ...
            "task": TASK, "revision": REVISION, "candidate_id": CANDIDATE_ID,
            "status": "failed_runner", "release_eligible": False, "production_approval": False,
            "public_gate_evaluations": 0, "synthetic_only": True,
            "private_or_article_images": False, "chandler_included": False,
            "sealed_public_archive_opened": False, "phase": phase,
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=Path("ml/markers/center/artifacts/radial-feature-v1/P2-run"))
