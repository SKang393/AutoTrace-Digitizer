...
"""Single-use sealed public gate for a selected radial-feature candidate."""
...
from ml.markers.center.radial_feature_v1.dataset import load_sealed_public_archive
from ml.markers.gate_seal import acquire_gate_seal, canonical_json_bytes, complete_gate_seal, sha256_file


REPO_ROOT = Path(__file__).resolve().parents[4]
TASK = "marker-center"
REVISION = "marker-center-radial-feature-public-v1"
SPLIT_CONFIG_PATH = Path("ml/markers/center/radial_feature_v1/gates/sealed-public-v1.json")
EVALUATOR_SOURCE_PATHS = (
    Path("ml/markers/center/radial_feature_v1/dataset.py"),
    Path("ml/markers/center/radial_feature_v1/model.py"),
    Path("ml/markers/center/radial_feature_v1/sealed_gate.py"),
...
def evaluate_candidate(*, onnx_path: Path, training_report_path: Path, output_path: Path) -> dict[str, object]:
    ...
    if training_report.get("status") != "selected" or training_report.get("selection_gate_passed") is not True:
        raise RuntimeError("Only a selection-passing radial-feature candidate may open the public gate")
    candidate_hash = sha256_file(onnx_path)
    if candidate_hash != training_report.get("onnx_sha256"):
        raise RuntimeError("Radial-feature candidate ONNX differs from its selection report")
    ...
    if sha256_file(seal_path) != split_config["sealed_public_test_seal_sha256"]:
        raise RuntimeError("Radial-feature public-test seal differs from gate configuration")
    ...
    if sha256_file(archive) != sealed["fixture_archive_sha256"]:
        raise RuntimeError("Radial-feature fixture archive checksum mismatch")
    if sha256_file(private_manifest) != sealed["private_manifest_sha256"]:
        raise RuntimeError("Radial-feature private manifest checksum mismatch")
    gate = acquire_gate_seal(
        repo_root=REPO_ROOT, task=TASK, revision=REVISION,
    ...
    if session.get_providers() != ["CPUExecutionProvider"]:
        raise RuntimeError("Radial-feature public gate requires CPUExecutionProvider only")
    ...
    report: dict[str, object] = {
        "schema": "graphreader.marker-center-radial-feature-public-gate.v1",
...
def main() -> int:
    ...
    result = evaluate_candidate(
        onnx_path=REPO_ROOT / args.onnx, training_report_path=REPO_ROOT / args.training_report,
