...
"""Authorization-ready V16 training and synthetic dev runner."""
...
from ml.markers.center.line_aware_v1.pipeline import MarkerPrediction, extract_proposals, postprocess_predictions
...
from .model import ModelConfig, ScaleClassifierNet

REPO_ROOT = Path(__file__).resolve().parents[4]
TASK = "marker-center"
REVISION = "marker-center-scale-classifier-v16"
CANDIDATE_ID = "P1"
CONFIG_PATH = Path("ml/markers/center/scale_classifier_v16/training/p1.json")
RUNNER_SOURCE_PATHS = (
    Path("ml/markers/center/scale_classifier_v16/model.py"),
    Path("ml/markers/center/scale_classifier_v16/train.py"),
    Path("ml/markers/center/scale_classifier_v16/protocol.py"),
...
def _loss(raw, labels, offsets, radii, hard, *, positive_weight: float, hard_weight: float) -> torch.Tensor:
    weights = torch.where(labels > 0.5, torch.full_like(labels, positive_weight), torch.where(hard, torch.full_like(labels, hard_weight), torch.ones_like(labels)))
...
def run(output_dir: Path) -> dict[str, object]:
    ...
    try:
        ...
        phase = "export"
        checkpoint = output_dir / "marker-center-scale-classifier-v16-p1.pt"
        ...
        onnx_path = output_dir / "marker-center-scale-classifier-v16-p1.onnx"
        example = torch.zeros((1, 3, 33, 33), dtype=torch.float32)
        torch.onnx.export(model.eval(), example, onnx_path, input_names=["candidate_patches"], output_names=["candidate_predictions"], dynamic_axes={"candidate_patches": {0: "candidate_count"}, "candidate_predictions": {0: "candidate_count"}}, opset_version=18, dynamo=False)
        onnx.checker.check_model(onnx.load(onnx_path))
        session = ort.InferenceSession(str(onnx_path), providers=["CPUExecutionProvider"])
        if session.get_providers() != ["CPUExecutionProvider"]:
            raise RuntimeError("V16 parity requires CPUExecutionProvider")
        parity_rows = []
