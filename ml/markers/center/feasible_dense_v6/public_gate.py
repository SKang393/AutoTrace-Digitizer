...
"""One-use truth-hidden public gate for feasible dense V6."""
...
from ml.markers.center.feasible_dense_v6.dataset import PROHIBITED_KINDS, read_archive
...
GATE_PATH = Path("ml/markers/center/feasible_dense_v6/gates/sealed-public-v1.json")
...
def _evaluate_opened_gate(
    *,
    ...
    if session.get_providers() != ["CPUExecutionProvider"]:
        raise RuntimeError("Feasible dense V6 public gate requires CPUExecutionProvider only")
...
def run(candidate_report_path: Path, output_path: Path) -> dict[str, object]:
    ...
    if entry.get("public_gate_authorized") is not True or entry.get("public_gate_authorized_candidate_id") != candidate_id:
        raise RuntimeError("Feasible dense V6 public gate is not separately authorized")
    candidate_report_sha256 = sha256_file(candidate_report_path)
