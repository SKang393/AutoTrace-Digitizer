...
"""One-use P1 training and visible selection for feasible dense V6."""
...
from ml.markers.center.feasible_dense_v6.dataset import read_archive
...
CANDIDATE_ID = "P1"
CONFIG_PATH = Path("ml/markers/center/feasible_dense_v6/training/p1.json")
RUNNER_SOURCE_PATHS = (
    ...
    Path("ml/markers/center/feasible_dense_v6/model.py"),
    Path("ml/markers/center/feasible_dense_v6/train_p1.py"),
)
EPOCHS = 48
...
def _execute_candidate(
    ...
    expected_configuration = {
        ...
        "epochs": EPOCHS,
        "expected_optimizer_steps": EPOCHS * (192 // BATCH_SIZE),
    ...
    for key, expected in expected_configuration.items():
        if config.get(key) != expected:
            raise RuntimeError(f"Feasible dense V6 P1 configuration changed: {key}")
    ...
    if sha256_file(ROOT / "SELECTION_MANIFEST.json") != config["selection_manifest_sha256"]:
        raise RuntimeError("Feasible dense V6 selection manifest changed")
    ...
    if sha256_file(train_path) != selection["train"]["archive_sha256"]:
        raise RuntimeError("Frozen feasible dense V6 training archive changed")
    if sha256_file(validation_path) != selection["validation"]["archive_sha256"]:
        raise RuntimeError("Frozen feasible dense V6 validation archive changed")
    ...
    for epoch in range(EPOCHS):
        epoch_indices = np.roll(indices, (epoch * 11) % len(indices))
    ...
    if optimizer_steps != int(config["expected_optimizer_steps"]):
        raise RuntimeError("Feasible dense V6 P1 optimizer step count changed")
    progress["phase"] = "export"
    checkpoint_path = output_dir / "marker-center-feasible-dense-v6-p1.pt"
    onnx_path = output_dir / "marker-center-feasible-dense-v6-p1.onnx"
    ...
    if session.get_providers() != ["CPUExecutionProvider"]:
        raise RuntimeError("Feasible dense V6 P1 selection requires CPUExecutionProvider only")
    ...
    return {
        "schema": "graphreader.marker-center-feasible-dense-candidate.v6",
        "task": TASK,
        "revision": REVISION,
...
def run(output_dir: Path) -> dict[str, object]:
    if output_dir.exists():
        raise RuntimeError(f"Feasible dense V6 P1 output exists: {output_dir}")
    ...
    try:
        ...
        failure = {
            "schema": "graphreader.marker-center-feasible-dense-failure.v6",
            "task": TASK,
            "revision": REVISION,
            "candidate_id": CANDIDATE_ID,
...
def main() -> int:
    ...
    print(json.dumps(run(arguments.output.resolve()), sort_keys=True))
    return 0
