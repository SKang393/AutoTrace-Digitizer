...
Windows - Steps to build a portable package:
--------------------------------------------
These steps build a folder that can be copied to another Windows computer and run without installing Engauge.
Run them from a Qt command prompt so qmake, the selected compiler, windeployqt and the Qt help tools are in PATH.
...
2) Open the output folder:
   ...
   > release\EngaugeDigitizer-Windows-Portable

3) Launch Engauge from the visible launcher:

   > Start Engauge Digitizer.cmd

The same folder can be zipped and moved to another Windows computer. The portable build includes
portable-settings.ini, which makes Engauge store settings in the package's settings directory instead of the
Windows registry. The script also writes release\EngaugeDigitizer-Windows-Portable.zip unless -NoZip is used.
