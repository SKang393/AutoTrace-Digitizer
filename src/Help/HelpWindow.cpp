...
#define _QUOTE(string) #string
  ...
  for (itr = paths.begin(); itr != paths.end(); itr++) {
    ...
    const QDir applicationDir(QDir::cleanPath(QApplication::applicationDirPath()));
    const QString relativePath = pathAsIs.startsWith("/") ?
                                 pathAsIs.mid(1) :
                                 pathAsIs;
    QString pathWithPrefix = QDir::cleanPath(applicationDir.absoluteFilePath(relativePath));
