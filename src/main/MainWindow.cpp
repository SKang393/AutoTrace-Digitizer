...
bool documentHasGraphPoints(const Document &document)
{
  const QStringList curveNames = document.curvesGraphsNames();
  for (int index = 0; index < curveNames.count(); ++index) {
    if (document.curvesGraphsNumPoints(curveNames.at(index)) > 0) {
      return true;
    }
  }
...
bool graphBoundsFromAxisPoints(const Document &document,
                               ...
                               double &yMaximum)
{
  ...
  const Points axisPoints = document.curveAxes().points();
  if (axisPoints.count() < 3) {
    return false;
  }

  bool initialized = false;
  for (int index = 0; index < axisPoints.count(); ++index) {
    const QPointF graphPoint = axisPoints.at(index).posGraph();
    if (!initialized) {
      xMinimum = graphPoint.x();
      xMaximum = graphPoint.x();
      yMinimum = graphPoint.y();
      yMaximum = graphPoint.y();
      initialized = true;
    } else {
      xMinimum = qMin(xMinimum, graphPoint.x());
      xMaximum = qMax(xMaximum, graphPoint.x());
      yMinimum = qMin(yMinimum, graphPoint.y());
      yMaximum = qMax(yMaximum, graphPoint.y());
    }
  }

  yMinimum = qMax(0.0, yMinimum);
  if (xMaximum <= xMinimum || yMaximum <= yMinimum) {
    return false;
  }

  QList<QPointF> screenCorners;
  QPointF screen;
  transformation.transformRawGraphToScreen(QPointF(xMinimum, yMinimum), screen);
  screenCorners << screen;
  transformation.transformRawGraphToScreen(QPointF(xMaximum, yMinimum), screen);
  screenCorners << screen;
  transformation.transformRawGraphToScreen(QPointF(xMinimum, yMaximum), screen);
  screenCorners << screen;
  transformation.transformRawGraphToScreen(QPointF(xMaximum, yMaximum), screen);
  screenCorners << screen;

  double left = screenCorners.first().x();
  double right = screenCorners.first().x();
  double top = screenCorners.first().y();
  double bottom = screenCorners.first().y();
  for (int index = 1; index < screenCorners.count(); ++index) {
    left = qMin(left, screenCorners.at(index).x());
    right = qMax(right, screenCorners.at(index).x());
    top = qMin(top, screenCorners.at(index).y());
    bottom = qMax(bottom, screenCorners.at(index).y());
  }

  const QRect imageRect(QPoint(0, 0), imageSize);
  plotRect = QRect(qFloor(left),
                   qFloor(top),
                   qCeil(right - left) + 1,
                   qCeil(bottom - top) + 1).intersected(imageRect);
...
#if defined(OSX_DEBUG) || defined(OSX_RELEASE)
  qApp->setApplicationName ("AutoTrace Digitizer");
...
QString MainWindow::fileNameForExportOnly () const
{
  ...
  if (m_isErrorReportRegressionTest) {
    ...
    QDir dir = QFileInfo (m_currentFileWithPathAndFileExtension).absoluteDir();
    fileName = QDir::cleanPath (dir.absoluteFilePath (QString ("%1.%2")
                                                      .arg (m_currentFile)
...
QString MainWindow::fileNameForExtractImageOnly () const
{
  // User requested export-only mode so just change file extension
  QDir dir = QFileInfo (m_currentFileWithPathAndFileExtension).absoluteDir();
  QString fileName = QDir::cleanPath (dir.absoluteFilePath (QString ("%1.%2")
                                                            .arg (m_currentFile)
...
void MainWindow::loadToolTips()
{
  if (m_actionViewToolTips->isChecked ()) {
    ...
    m_actionDigitizeAutoAxis->setToolTip (m_actionDigitizeAutoAxis->text());
    m_actionDigitizeScale->setToolTip (m_actionDigitizeScale->text());
    m_actionDigitizeCurve->setToolTip (m_actionDigitizeCurve->text());
    m_actionDigitizeAutoCurve->setToolTip (m_actionDigitizeAutoCurve->text());
    ...
    m_actionDigitizeAutoAxis->setToolTip ("");
    m_actionDigitizeScale->setToolTip ("");
    m_actionDigitizeCurve->setToolTip ("");
    m_actionDigitizeAutoCurve->setToolTip ("");
...
void MainWindow::setCurrentPathFromFile (const QString &fileName)
{
  ...
  if (dir.exists ()) {
    ...
    bool success = QDir::setCurrent (QDir::cleanPath (dir.absolutePath ())); // Return to chosen directory the next time
...
void MainWindow::slotDigitizeAutoAxis ()
{
  LOG4CPP_INFO_S ((*mainCat)) << "MainWindow::slotDigitizeAutoAxis";

  if (m_cmdMediator == nullptr || m_currentFile.isEmpty()) {
    QMessageBox::warning(this,
                         tr("Auto Axis"),
                         tr("Import an image before using Auto Axis."));
    return;
  }
  ...
  if (!modeGraph()) {
    QMessageBox::warning(this,
                         tr("Auto Axis"),
                         tr("Auto Axis is available for graph images."));
    return;
  }

  Document &document = m_cmdMediator->document();
  if (document.documentAxesPointsRequired() != DOCUMENT_AXES_POINTS_REQUIRED_3) {
    QMessageBox::warning(this,
                         tr("Auto Axis"),
                         tr("Auto Axis supports the standard three-axis-point graph import."));
    return;
  }

  if (document.curveAxes().numPoints() > 0 || documentHasGraphPoints(document)) {
    QMessageBox::warning(this,
                         tr("Auto Axis"),
                         tr("Auto Axis can rotate the image, so use it before adding axis or curve points."));
    return;
  }

  bool accepted = false;
  const double yMaximum = QInputDialog::getDouble(this,
                                                  tr("Auto Axis"),
                                                  tr("Y-axis maximum"),
                                                  100.0,
                                                  0.000001,
  ...
  const AutoAxisResult result = AutoDigitize::detectAxes(document.pixmap().toImage(),
                                                        yMaximum);
  if (!result.success) {
    QMessageBox::warning(this,
                         tr("Auto Axis"),
  ...
  if (result.rotated) {
    document.setPixmap(result.image);
    setPixmap(selectedGraphCurve(),
              m_cmdMediator->pixmap());
  }

  for (int index = 0; index < result.pointsScreen.count(); ++index) {
    bool isError = false;
    QString errorMessage;
    document.checkAddPointAxis(result.pointsScreen.at(index),
                               result.pointsGraph.at(index),
                               isError,
                               errorMessage,
                               false);
    if (isError) {
    ...
    CmdAddPointAxis *cmd = new CmdAddPointAxis(*this,
                                               document,
                                               result.pointsScreen.at(index),
                                               result.pointsGraph.at(index),
                                               document.nextOrdinalForCurve(AXIS_CURVE_NAME),
                                               false);
    m_cmdMediator->push(cmd);
  }
  ...
  showTemporaryMessage(result.rotated ?
                       tr("Auto Axis created 3 points and rotated the image %1 degrees.").arg(result.rotationDegrees, 0, 'f', 2) :
                       tr("Auto Axis created 3 points."));
}
  ...
  if (cycleKey != m_autoCurveCycleState.key ||
    ...
    if (result.groups.isEmpty()) {
      QMessageBox::warning(this,
                           tr("Auto Curve"),
                           result.message);
      m_autoCurveCycleState.key.clear();
      return;
    }
...
void MainWindow::updateControls ()
{
  ...
  m_actionDigitizeAutoAxis->setEnabled (modeGraph ());
  m_actionDigitizeScale->setEnabled (modeMap ());
  m_actionDigitizeCurve->setEnabled (!m_currentFile.isEmpty ());
  m_actionDigitizeAutoCurve->setEnabled (!m_currentFile.isEmpty () && m_transformation.transformIsDefined ());
