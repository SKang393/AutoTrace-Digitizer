...
const QString ENGAUGE_PORTABLE_ENVIRONMENT ("ENGAUGE_PORTABLE");
const QString ENGAUGE_PORTABLE_MARKER ("portable-settings.ini");

// Prototypes
bool checkFileExists (const QString &file);
void configureDefaultLocale ();
...
void configurePortableSettings ()
{
  ...
  const QString portableEnvironment = environment.value (ENGAUGE_PORTABLE_ENVIRONMENT).trimmed().toLower();
  const bool isPortableEnvironment = (portableEnvironment == "1" ||
                                      portableEnvironment == "true" ||
                                      portableEnvironment == "yes");
  const QString applicationDirectory = QDir::cleanPath (QCoreApplication::applicationDirPath());
  const QDir applicationDir (applicationDirectory);
  const QString portableMarker = QDir::cleanPath (applicationDir.absoluteFilePath (ENGAUGE_PORTABLE_MARKER));
  const bool hasPortableMarker = QFileInfo (portableMarker).exists();

  if (isPortableEnvironment || hasPortableMarker) {
    const QString settingsDirectory = QDir::cleanPath (applicationDir.absoluteFilePath ("settings"));
                   ...
                   QStringList &commandLineWithoutLoadStartupFiles)
{
  ...
  for (int i = 1; i < arguments.count(); i++) {
    ...
    if (nextIsErrorReportFile) {
      sanityCheckValue (checkFileExists (argument),
                        argument,
                        QObject::tr ("is not a valid file name"));
      errorReportFile = QDir::cleanPath (QFileInfo (argument).absoluteFilePath());
      nextIsErrorReportFile = false;
    } else if (nextIsExtractImageOnly) {
      sanityCheckValue (importImageExtensions.offers (argument),
                        argument,
                        QObject::tr ("is not a valid image file extension"));
      extractImageOnlyExtension = argument;
      nextIsExtractImageOnly = false;
    } else if (nextIsFileCmdScript) {
      sanityCheckValue (checkFileExists (argument),
                        argument,
                        QObject::tr ("is not a valid file name"));
      fileCmdScriptFile = QDir::cleanPath (QFileInfo (argument).absoluteFilePath());
      nextIsFileCmdScript = false;
    } else if (argument == DASH_DEBUG) {
      isDebug = true;
    } else if (argument == DASH_DROP_REGRESSION) {
      isDropRegression = true;
    } else if (argument == DASH_ERROR_REPORT) {
      nextIsErrorReportFile = true;
    } else if (argument == DASH_EXPORT_ONLY) {
      isExportOnly = true;
    } else if (argument == DASH_EXTRACT_IMAGE_ONLY) {
      isExtractImageOnly = true;
      nextIsExtractImageOnly = true;
    } else if (argument == DASH_FILE_CMD_SCRIPT) {
      nextIsFileCmdScript = true;
    } else if (argument == DASH_GNUPLOT) {
      isGnuplot = true;
    } else if (argument == DASH_HELP) {
      showUsage = true; // User requested help
    } else if (argument == DASH_REGRESSION) {
      isErrorReportRegressionTest = true;
    } else if (argument == DASH_RESET) {
      isReset = true;
    } else if (argument == DASH_STYLE) {
    ...
    } else if (argument == DASH_STYLES) {
      showStylesAndQuit ();
    } else if (argument == DASH_UPGRADE) {
      isUpgrade = true;
    } else if (argument.startsWith (DASH)) {
    ...
    if (!isLoadStartupFile) {
      commandLineWithoutLoadStartupFiles << argument;
    }
  }
