...
[TestClass]
public sealed class OcrV9P2CandidateSelectionTests
{
    private const string RunVariable = "GRAPHREADER_RUN_OCR_V9_P2_SELECTION";
    private const string DetectorVariable = "GRAPHREADER_OCR_V9_P2_DETECTOR";
    private const string OfficialVariable = "GRAPHREADER_OCR_V9_P2_OFFICIAL";
    private const string NumericVariable = "GRAPHREADER_OCR_V9_P2_NUMERIC";
    private const string AmbiguityVariable = "GRAPHREADER_OCR_V9_P2_AMBIGUITY";
    private const string YamlVariable = "GRAPHREADER_OCR_V9_P2_OFFICIAL_YAML";
    private const string ReportVariable = "GRAPHREADER_OCR_V9_P2_SELECTION_REPORT";
    private const string SourceCommitVariable = "GRAPHREADER_OCR_V9_P2_SOURCE_COMMIT";
    ...
    private const string ExactTest =
        "OcrV9P2CandidateSelectionTests.FreshP2SelectionExecutesOnceThroughCSharpCpuCandidate";
    ...
    [TestMethod]
    public async Task FreshP2SelectionExecutesOnceThroughCSharpCpuCandidate()
    {
        if (!string.Equals(Environment.GetEnvironmentVariable(RunVariable), "1", StringComparison.Ordinal))
        {
            Assert.Inconclusive(
                $"Set {RunVariable}=1 only after the tracked one-run P2 authorization exists.");
        }

        string root = FindRepositoryRoot();
        string contractRoot = Path.Combine(root, "ml", "ocr", "selected_confidence_acceptance_v9_p2");
        string sealPath = Path.Combine(contractRoot, "SELECTION_SEAL.json");
        string authorizationPath = Path.Combine(contractRoot, "SELECTION_AUTHORIZATION.json");
        Assert.IsTrue(File.Exists(sealPath), "The P2 selection identity has not been frozen.");
        Assert.IsTrue(File.Exists(authorizationPath), "The P2 selection execution is not authorized.");
        ...
        Assert.IsTrue(File.Exists(archivePath), $"The frozen P2 archive is missing: {archivePath}");
        AssertHash(archivePath, authorization.FixtureArchiveSha256, "P2 selection archive");
        string reportPath = RequiredOutputPath(ReportVariable);
        Assert.AreEqual(
            Path.GetFullPath(Path.Combine(root, authorization.ResultPath)),
            reportPath,
            true,
            "The P2 report path is not authorization-bound.");
        ...
        CancellationToken cancellationToken)
    {
        ...
        ZipArchiveEntry manifestEntry = archive.GetEntry("manifest.json") ??
            throw new InvalidDataException("P2 selection archive has no manifest.json.");
        byte[] manifestBytes = await OcrV8DirectPublicCorpusTests.ReadEntryAsync(
            manifestEntry,
            cancellationToken);
        Assert.AreEqual(seal.FixtureManifestSha256, Sha256(manifestBytes), "P2 manifest changed.");
        FixtureManifest manifest = JsonSerializer.Deserialize<FixtureManifest>(manifestBytes, JsonOptions) ??
            throw new InvalidDataException("P2 selection manifest is invalid.");
        Assert.AreEqual("graphreader.ocr-selected-confidence-selection-fixtures.v9-p2", manifest.Schema);
        Assert.AreEqual("graphreader-v10-selected-confidence-acceptance-v9-p2", manifest.Revision);
        ...
        Assert.IsFalse(manifest.P1FixtureBytesReused);
        Assert.IsFalse(manifest.P1TruthOrSceneIdsReused);
        ...
        foreach (FixtureCase fixture in manifest.Cases)
        {
            ...
            ZipArchiveEntry imageEntry = archive.GetEntry(fixture.ImagePath) ??
                throw new InvalidDataException($"P2 selection image is missing: {fixture.ImagePath}");
        ...
        return new SelectionReport(
            "graphreader.ocr-selected-confidence-selection-report.v1",
            "P2",
    ...
    {
        Assert.AreEqual("graphreader.ocr-selected-confidence-selection-seal.v1", seal.Schema);
        Assert.AreEqual("P2", seal.CandidateId);
        Assert.AreEqual(128, seal.SceneCount);
        Assert.AreEqual(640, seal.TruthRegionCount);
        ...
        Assert.AreEqual("graphreader.ocr-selected-confidence-selection-authorization.v1", authorization.Schema);
    ...
    private static string RequiredOutputPath(string variable)
    {
        ...
        Assert.IsFalse(string.IsNullOrWhiteSpace(value), $"{variable} is required.");
        string path = Path.GetFullPath(value);
        Assert.IsFalse(File.Exists(path), $"The one-time P2 report already exists: {path}");
        return path;
    }
    ...
    private static string RequiredSourceCommit()
    {
        ...
        Assert.IsTrue(value.Length == 40 && value.All(Uri.IsHexDigit));
    ...
    private sealed record FixtureManifest(
        ...
        bool P1FixtureBytesReused,
        bool P1TruthOrSceneIdsReused,
